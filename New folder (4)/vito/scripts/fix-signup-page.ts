 ...
 * FIX SIGNUP PAGE
 *
 * Regenerate signup page with proper types
 */
...
async function main() {
  ...
  console.log('FIXING SIGNUP PAGE');
  console.log('='.repeat(60));
  ...
  try {
    ...
    if (!wire) throw new Error('WIRE output not found');

    let artifacts = wire.artifacts || [];
    artifacts = artifacts.filter((a: any) => a.path !== 'src/app/signup/page.tsx');
    artifacts.push({
      type: 'code',
    ...
    console.log('[Fix] Signup page replaced with clean version');
    console.log('='.repeat(60));
  } catch (error) {
