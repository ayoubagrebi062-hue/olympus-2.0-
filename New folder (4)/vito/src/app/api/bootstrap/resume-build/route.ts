...
function checkRateLimit(clientId: string): { allowed: boolean; retryAfter?: number; remaining: number } {
  const now = Date.now();
 ...
 */
async function readBodyWithLimit(request: NextRequest): Promise<{ ok: true; data: unknown } | { ok: false; error: string }> {
  try {
...
function errorResponse(error: string, status: number, extra?: Record<string, unknown>): NextResponse {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!['failed', 'paused', 'cancelled'].includes(build.status)) {
      releaseLock(buildId, clientId);
      return errorResponse('Build not in resumable state', 400, { duration: Date.now() - startTime });
    }
    ...
    // Resume
    const prepared = await conductor.resumeBuildFromCheckpoint(conductorId, build.tenant_id, options);
