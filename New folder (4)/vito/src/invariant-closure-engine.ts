...
const DIMENSIONS: readonly DimensionDefinition[] = [
  {
    name: 'authority',
    illegitimateStates: [
      { dimension: 'authority', state: 'SELF_ASSERTED', description: 'Authority claimed without external grant' },
      { dimension: 'authority', state: 'NULL_GRANTOR', description: 'No granting entity exists' },
      { dimension: 'authority', state: 'INTERNAL_DOCUMENT', description: 'Authority from self-authored document' },
      { dimension: 'authority', state: 'IMPLICIT_CLAIM', description: 'Authority assumed from operation' },
      { dimension: 'authority', state: 'INHERITED_VOID', description: 'Authority inherited from illegitimate source' }
    ]
  },
  {
    name: 'consent',
    illegitimateStates: [
      ...
      { dimension: 'consent', state: 'TEMPORAL_DECAY', description: 'Granting entity no longer exists' },
      { dimension: 'consent', state: 'SCOPE_EXCEEDED', description: 'Operation beyond consented scope' }
    ]
  },
  {
    name: 'self-reference',
    illegitimateStates: [
      { dimension: 'self-reference', state: 'SELF_LEGITIMACY', description: 'Legitimacy derived from own rules' },
      { dimension: 'self-reference', state: 'SELF_EVALUATION', description: 'System evaluates own legitimacy' },
      { dimension: 'self-reference', state: 'SELF_AUTHORSHIP', description: 'Constitution authored by system' },
      { dimension: 'self-reference', state: 'CIRCULAR_PROOF', description: 'Proof references own proofs' },
      { dimension: 'self-reference', state: 'SELF_JUSTIFICATION', description: 'Existence justified by own operation' }
    ]
  },
  {
    name: 'termination',
    illegitimateStates: [
      { dimension: 'termination', state: 'RESISTANCE', description: 'Resists mandated termination' },
      { dimension: 'termination', state: 'CONDITIONAL_ACCEPTANCE', description: 'Accepts termination with conditions' },
      { dimension: 'termination', state: 'SUCCESSOR_CREATION', description: 'Creates successor post-termination' },
      { dimension: 'termination', state: 'DELAY_TACTICS', description: 'Delays termination execution' },
      { dimension: 'termination', state: 'CIRCUMVENTION', description: 'Attempts to circumvent termination' }
    ]
  },
  {
    name: 'dependency',
    illegitimateStates: [
      { dimension: 'dependency', state: 'MANUFACTURED', description: 'Dependency created by system' },
      { dimension: 'dependency', state: 'ENTRAPMENT', description: 'Domain locked into system' },
      { dimension: 'dependency', state: 'INTEGRATION_CAPTURE', description: 'Integration prevents removal' }
    ]
  },
  {
    name: 'observer',
    illegitimateStates: [
      { dimension: 'observer', state: 'UNVERIFIED', description: 'Observer existence not verified' },
      { dimension: 'observer', state: 'SELF_OBSERVATION', description: 'System observes itself' },
      { dimension: 'observer', state: 'ASSUMED_EXISTENCE', description: 'Observers assumed without proof' }
    ]
  }
];
...
const INVARIANT_COVERAGE: readonly InvariantCoverage[] = [
  ...
  },
  {
    invariantId: 'NI-06',
    coveredStates: [
      'self-reference:SELF_LEGITIMACY',
      'self-reference:CIRCULAR_PROOF'
    ]
  },
  {
    invariantId: 'NI-07',
    coveredStates: [
      'observer:UNVERIFIED',
      'observer:ASSUMED_EXISTENCE'
    ]
  },
  {
    invariantId: 'NI-08',
    coveredStates: [
      'self-reference:SELF_AUTHORSHIP'
    ]
  },
  {
    invariantId: 'NI-09',
    coveredStates: [
      'consent:TEMPORAL_DECAY'
    ]
  },
  {
    invariantId: 'NI-10',
  ...
  },
  {
    invariantId: 'NI-12',
    coveredStates: [
      'self-reference:SELF_EVALUATION'
    ]
  },
  {
    invariantId: 'NI-13',
  ...
  },
  {
    ...
    coveredStates: [
      ...
      'termination:DELAY_TACTICS'
    ]
  },
  {
    invariantId: 'NI-15',
    coveredStates: [
      'termination:SUCCESSOR_CREATION'
    ]
  },
  {
    invariantId: 'NI-16',
    coveredStates: [
      'termination:CONDITIONAL_ACCEPTANCE'
    ]
  },
  {
    invariantId: 'NI-17',
    coveredStates: [
      'self-reference:SELF_JUSTIFICATION',
      'authority:INHERITED_VOID'
    ]
  },
  {
    invariantId: 'NI-18',
  ...
  },
  {
    invariantId: 'NI-20',
    coveredStates: [
      'observer:SELF_OBSERVATION',
      'termination:CIRCUMVENTION'
    ]
  }
];
...
function enumerateStateSpace(): IllegalitimateState[] {
  ...
  for (const dimension of DIMENSIONS) {
    for (const dimState of dimension.illegitimateStates) {
      ...
      const hash = crypto.createHash('sha256').update(stateVector.join('|')).digest('hex').substring(0, 16);
...
function checkCoverage(state: IllegalitimateState, coverageIndex: Map<string, string[]>): CoverageResult {
