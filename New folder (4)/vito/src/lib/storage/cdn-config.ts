 ...
 */
export function transformForVercel(
  url: string,
 ...
 */
export function transformForCloudflare(
  url: string,
  options: ImageTransformOptions
): string {
  const params: string[] = [];
 ...
 */
export function transformForSupabase(
  url: string,
  options: ImageTransformOptions
): string {
...
export function generatePreloadHeaders(
  ...
  return urls
    .map(({ url, type }) => generatePreloadHeader(url, { type }))
    .join(', ');
}
