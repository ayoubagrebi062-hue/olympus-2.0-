...
export function hashDecision(decision: Omit<DecisionTrace, 'id' | 'hash' | 'parentHash'>): DecisionTrace {
...
export function authorizeIrreversibleAction(request: IrreversibleActionRequest): DecisionTrace {
  if (!request.reason || request.reason.trim().length < 10) {
    HARD_STOP(`INSUFFICIENT_REASON: Action "${request.action}" requires written reason (min 10 chars)`)
  }
  if (!request.actorId) {
...
export function checkTrustDegradation(
  ...
  if (degradation >= threshold) {
    console.warn(`[OLYMPUS] TRUST_DEGRADATION: ${degradation} points (${previousScore} → ${currentScore})`)
    return true
  }
  return false
}
