...
function keywordRelevanceCheck(output: GeneratedOutput): RelevanceResult {
  ...
  const code = output.files.map(f => f.content).join('\n').toLowerCase();
  ...
  const score = keyTerms.length > 0
    ? Math.round((matchedTerms / keyTerms.length) * 100)
    : 70; // Default score if no key terms
...
function extractKeyTerms(prompt: string): string[] {
  ...
  const stopWords = new Set([
    ...
    'above', 'create', 'make', 'build', 'generate', 'add', 'include',
    'and', 'or', 'but', 'if', 'then', 'else', 'when', 'where', 'why',
    'how', 'all', 'each', 'every', 'both', 'few', 'more', 'most', 'other',
    'some', 'such', 'no', 'nor', 'not', 'only', 'own', 'same', 'so',
    'than', 'too', 'very', 'just', 'also', 'now', 'page', 'website', 'app',
    'application', 'section', 'component', 'feature', 'modern', 'simple',
    'want', 'need', 'please', 'help', 'me', 'my', 'i', 'you', 'your',
...
export function checkCodeStructure(output: GeneratedOutput): ValidationIssue[] {
  ...
  const _hasReactFiles = output.files.some(f =>
    f.path.endsWith('.tsx') || f.path.endsWith('.jsx')
  );
  const hasApiFiles = output.files.some(f =>
    f.path.includes('/api/') || f.path.includes('route.')
  );
  const hasStyleFiles = output.files.some(f =>
    f.path.endsWith('.css') || f.path.endsWith('.scss')
  );

  // If prompt mentions API but no API files
  if ((prompt.includes('api') || prompt.includes('backend') || prompt.includes('endpoint'))
      && !hasApiFiles) {
    issues.push({
      code: 'MISSING_API',
  ...
  if ((prompt.includes('style') || prompt.includes('design') || prompt.includes('css'))
      && !hasStyleFiles
