...
const KNOWN_PACKAGES = new Set([
  'react', 'react-dom', 'next', 'next/image', 'next/link', 'next/navigation',
  'next/font', 'next/headers', 'next/server',
  'tailwindcss', 'lucide-react', 'framer-motion', 'clsx', 'class-variance-authority',
  ...
  'zod', 'react-hook-form', '@hookform/resolvers', 'zustand', 'swr',
  'date-fns', 'lodash', 'axios', 'uuid', 'nanoid',
  '@supabase/supabase-js', '@supabase/ssr', 'mongodb', 'redis', 'ioredis',
  '@anthropic-ai/sdk', 'openai', 'stripe', '@stripe/stripe-js',
  'tailwind-merge', 'sharp', 'neo4j-driver', '@qdrant/js-client-rest',
]);

// Packages that are known to NOT exist (hallucinated)
const HALLUCINATED_PACKAGES = new Set([
  'react-magic-ui', 'next-super-image', 'tailwind-animate-extra',
  '@shadcn/button', '@shadcn/ui', 'shadcn-ui',
  'super-icons', 'mega-utils', 'awesome-helpers',
  'react-awesome-component', 'next-magic', 'ai-utils',
]);

// Node.js built-in modules
const NODE_BUILTINS = new Set([
  'fs', 'path', 'os', 'http', 'https', 'crypto', 'util', 'stream',
  'events', 'buffer', 'querystring', 'url', 'child_process', 'cluster',
  ...
  'timers', 'trace_events', 'v8', 'wasi', 'worker_threads',
]);
