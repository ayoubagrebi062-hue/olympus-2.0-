...
export class VisualRegressionEngine {
  ...
  private async pixelCompare(
    ...
    for (let i = 0; i < minLength; i += 100) { // Sample every 100th byte for performance
    ...
    return {
      ...
      changedRegions,
      brokenElements: [],
      analysisNotes: [`Size diff: ${(sizeDiffPercent * 100).toFixed(2)}%`, `Byte diff: ${(byteDiffPercent * 100).toFixed(2)}%`],
    };
  }
