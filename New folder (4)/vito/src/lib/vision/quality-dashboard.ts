...
export class QualityDashboard {
  ...
  recordGeneration(result: GenerationResult, provider: string): void {
    const record: GenerationRecord = {
      ...
      stubRate: result.quality.stubMetrics.hasStubs
        ? result.quality.stubMetrics.stubCount /
          (result.code?.split('\n').length || 1)
        : 0,
  ...
  getMetrics(): DashboardMetrics {
    ...
    const avgQualityScore =
      recentRecords.reduce((sum, r) => sum + r.qualityScore, 0) /
      Math.max(totalRequests, 1);
    const avgGenerationTime =
      recentRecords.reduce((sum, r) => sum + r.generationTimeMs, 0) /
      Math.max(totalRequests, 1);
  ...
  getQualityTrend(hours: number = 24): { improving: boolean; delta: number } {
    ...
    const recentScores = this.trends.qualityScores.filter(
  ...
  getProviderRecommendation(): string {
    ...
    for (const [name, metrics] of Array.from(this.providerMetrics)) {
      ...
      const successRate =
        metrics.successfulRequests / Math.max(metrics.totalRequests, 1);
      const avgLatency =
        metrics.totalLatencyMs / Math.max(metrics.totalRequests, 1);
