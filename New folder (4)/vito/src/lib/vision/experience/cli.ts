...
const sanitize = (s: string): string =>
  s.replace(/[\x00-\x1F\x7F]/g, '').replace(/<[^>]*>/g, '').slice(0, 5000);
...
const highlight = (line: string): string => {
  return line
    .replace(/\b(import|export|from|const|let|var|function|return|if|else|try|catch|finally|async|await|throw|interface|type|extends)\b/g, `${t.pink}$1${t.reset}`)
...
const typeCode = async (code: string): Promise<void> => {
  ...
  for (const line of lines) {
    ...
    if (line.includes('interface') || line.includes('export function') || line.includes('export const')) {
...
`);
  ...
  console.log();
  if (copied) {
    console.log(`${t.sparkle} ${t.bold}${demo.name}${t.reset} ${t.dim}·${t.reset} ${lines} lines ${t.dim}·${t.reset} ${t.green}copied${t.reset}\n`);
  } else {
