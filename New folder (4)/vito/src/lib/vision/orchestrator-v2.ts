...
export class VisionOrchestratorV2 {
  ...
  async generate(
    ...
    return this.dedup.execute(
      () => this.executeGeneration(request, ctx),
      ctx,
      dedupKey
    );
  }
  ...
  private async executeGeneration(
    ...
    try {
      ...
      if (isErr(codeResult)) {
        ...
        return Ok(this.createResponse(ctx, {
          code: null,
          images: [],
          quality: null,
  ...
  private async generateCodeWithResilience(
    ...
    return this.codeCircuit.execute(async () => {
      ...
      return withRetry(
        () => this.generateCodeOnce(request, ctx),
        {
