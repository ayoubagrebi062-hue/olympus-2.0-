...
const HEALING_MODEL = 'claude-3-5-sonnet-20241022';
...
export class SelfHealingGenerator {
  ...
  async healCode(
    ...
    const confidenceScore = appliedFixes.length > 0
...
Provide targeted fixes for each issue.`;
  ...
  recordFixOutcome(learningId: string, success: boolean): void {
    ...
    const pattern = this.learningDB.patterns.find(p =>
      p.lastUsed && new Date(p.lastUsed).getTime() > Date.now() - 60000
    );
