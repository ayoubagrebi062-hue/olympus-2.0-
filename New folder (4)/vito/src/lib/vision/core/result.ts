...
export async function fromPromise<T>(
  ...
  try {
    ...
    return Err(mapper?.(e) ?? createError(VisionErrorCode.INTERNAL_ERROR, e instanceof Error ? e.message : 'Unknown'));
  }
}
...
export async function withRetry<T>(
  ...
  const { maxAttempts = 3, baseDelayMs = 1000, maxDelayMs = 30000, shouldRetry = e => e.retryable, onRetry } = opts;
  ...
  for (let i = 1; i <= maxAttempts; i++) {
    ...
    const delay = Math.min((r.error.retryAfterMs ?? baseDelayMs) * Math.pow(2, i - 1) * (1 + Math.random() * 0.1), maxDelayMs);
