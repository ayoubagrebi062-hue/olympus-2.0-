...
class SimplePredictionModel implements PredictionModel {
  ...
  predictDuration(task: AgentTask, history: TaskHistory[]): PredictionResult {
    ...
    const variance = agentHistory.reduce(
      (sum, h) => sum + Math.pow(h.duration - ema, 2),
      0
    ) / agentHistory.length;
  ...
  private hashInputs(inputs: Map<string, unknown>): string {
    const sorted = Array.from(inputs.entries()).sort((a, b) =>
...
export class IntelligentOrchestrator {
  ...
  createPlan(tasks: AgentTask[]): ExecutionPlan {
    ...
    const speculativeSavings = speculativeStarts.reduce(
  ...
  private cleanupOldData(): void {
    ...
    if (this.circuitBreakers.size > this.maxCircuitBreakers) {
      const now = Date.now();
      const oldEntries = Array.from(this.circuitBreakers.entries())
        .filter(([_, cb]) => cb.state === 'closed' && now - cb.lastFailure > 3600000); // 1 hour old
  ...
  private async startSpeculativeExecution(
    ...
    try {
      ...
      this.state.speculativeTasks = this.state.speculativeTasks.filter(
        id => id !== task.id
      );
    }
  }
  ...
  private createExecutionWaves(
    ...
    while (remaining.size > 0) {
      ...
      for (const id of remaining) {
        const node = graph.get(id)!;
        const depsCompleted = Array.from(node.deps).every(
          d => completed.has(d) || !graph.has(d)
        );
      ...
      const estimatedDuration = Math.max(
        ...waveTaskIds.map(id => predictions.get(id)?.value || 0)
      );
  ...
  private identifySpeculativeStarts(
    ...
    for (const task of tasks) {
      ...
      const prediction = this.predictionModel.predictOutput(
        depNode.task,
        this.outputs
      );

      if (prediction.confidence >= this.config.speculationThreshold) {
        const depPrediction = this.predictionModel.predictDuration(
          depNode.task,
          this.taskHistory
        );
