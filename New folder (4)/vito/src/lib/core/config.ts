...
function loadConfigFromEnv(): Partial<OlympusConfig> {
  return {
    guardrails: {
      ...
      enabledLayers: getEnvArray('OLYMPUS_GUARDRAILS_LAYERS', defaultConfig.guardrails.enabledLayers) as GuardrailLayer[],
      parallelSecurity: getEnvBoolean('OLYMPUS_GUARDRAILS_PARALLEL', defaultConfig.guardrails.parallelSecurity),
      ...
      blockThreshold: getEnvFloat('OLYMPUS_GUARDRAILS_BLOCK_THRESHOLD', defaultConfig.guardrails.blockThreshold),
      warnThreshold: getEnvFloat('OLYMPUS_GUARDRAILS_WARN_THRESHOLD', defaultConfig.guardrails.warnThreshold),
      bypassRoles: getEnvArray('OLYMPUS_GUARDRAILS_BYPASS_ROLES', defaultConfig.guardrails.bypassRoles),
    },

    handoffs: {
      enabled: getEnvBoolean('OLYMPUS_HANDOFFS_ENABLED', defaultConfig.handoffs.enabled),
      maxChainDepth: getEnvNumber('OLYMPUS_HANDOFFS_MAX_DEPTH', defaultConfig.handoffs.maxChainDepth),
      confidenceThreshold: getEnvFloat('OLYMPUS_HANDOFFS_CONFIDENCE', defaultConfig.handoffs.confidenceThreshold),
      defaultCompression: getEnvString('OLYMPUS_HANDOFFS_COMPRESSION', defaultConfig.handoffs.defaultCompression) as 'none' | 'semantic' | 'neural',
      timeoutMs: getEnvNumber('OLYMPUS_HANDOFFS_TIMEOUT_MS', defaultConfig.handoffs.timeoutMs),
      circuitBreaker: {
        enabled: getEnvBoolean('OLYMPUS_CIRCUIT_BREAKER_ENABLED', defaultConfig.handoffs.circuitBreaker.enabled),
        failureThreshold: getEnvNumber('OLYMPUS_CIRCUIT_BREAKER_FAILURES', defaultConfig.handoffs.circuitBreaker.failureThreshold),
        successThreshold: getEnvNumber('OLYMPUS_CIRCUIT_BREAKER_SUCCESSES', defaultConfig.handoffs.circuitBreaker.successThreshold),
        resetTimeoutMs: getEnvNumber('OLYMPUS_CIRCUIT_BREAKER_RESET_MS', defaultConfig.handoffs.circuitBreaker.resetTimeoutMs),
      },
    },

    mcp: {
      enabled: getEnvBoolean('OLYMPUS_MCP_ENABLED', defaultConfig.mcp.enabled),
      defaultTransport: getEnvString('OLYMPUS_MCP_TRANSPORT', defaultConfig.mcp.defaultTransport) as MCPTransport,
      connectionTimeoutMs: getEnvNumber('OLYMPUS_MCP_CONNECT_TIMEOUT_MS', defaultConfig.mcp.connectionTimeoutMs),
      executionTimeoutMs: getEnvNumber('OLYMPUS_MCP_EXEC_TIMEOUT_MS', defaultConfig.mcp.executionTimeoutMs),
      maxServersPerTenant: getEnvNumber('OLYMPUS_MCP_MAX_SERVERS', defaultConfig.mcp.maxServersPerTenant),
      retry: {
      ...
      healthCheckIntervalMs: getEnvNumber('OLYMPUS_MCP_HEALTH_INTERVAL_MS', defaultConfig.mcp.healthCheckIntervalMs),
    },

    context: {
      maxBaggageSizeBytes: getEnvNumber('OLYMPUS_MAX_BAGGAGE_BYTES', defaultConfig.context.maxBaggageSizeBytes),
      idempotencyTtlMs: getEnvNumber('OLYMPUS_IDEMPOTENCY_TTL_MS', defaultConfig.context.idempotencyTtlMs),
      enableDegradation: getEnvBoolean('OLYMPUS_ENABLE_DEGRADATION', defaultConfig.context.enableDegradation),
    },

    observability: {
      logLevel: getEnvString('OLYMPUS_LOG_LEVEL', defaultConfig.observability.logLevel) as 'debug' | 'info' | 'warn' | 'error',
      metricsEnabled: getEnvBoolean('OLYMPUS_METRICS_ENABLED', defaultConfig.observability.metricsEnabled),
      eventsEnabled: getEnvBoolean('OLYMPUS_EVENTS_ENABLED', defaultConfig.observability.eventsEnabled),
      tracingEnabled: getEnvBoolean('OLYMPUS_TRACING_ENABLED', defaultConfig.observability.tracingEnabled),
      traceSampleRate: getEnvFloat('OLYMPUS_TRACE_SAMPLE_RATE', defaultConfig.observability.traceSampleRate),
    },
  };
}
...
export function getConfig(): OlympusConfig {
  if (!configInstance) {
    ...
    configInstance = deepMerge(defaultConfig as unknown as Record<string, unknown>, envConfig as unknown as Record<string, unknown>) as unknown as OlympusConfig;
  }
...
export function setConfig(overrides: Partial<OlympusConfig>): void {
  ...
  configInstance = deepMerge(current as unknown as Record<string, unknown>, overrides as unknown as Record<string, unknown>) as unknown as OlympusConfig;
}
