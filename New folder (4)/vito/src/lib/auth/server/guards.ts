...
export async function requireAuth(redirectTo?: string): Promise<AuthSession> {
  ...
  if (!authSession) {
    const loginUrl = new URL(REDIRECTS.UNAUTHENTICATED, process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3001');
...
export async function hasPermission(permission: Permission): Promise<boolean> {
  ...
  return authSession.permissions.includes(permission) ||
