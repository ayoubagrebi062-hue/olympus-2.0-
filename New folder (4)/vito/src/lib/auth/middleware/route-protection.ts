      ...
      try {
        ...
        if (parts.length === 3) {
          ...
          if (payload.olympus && typeof payload.olympus === 'object') {
            tenantId = typeof payload.olympus.tenant_id === 'string' ? payload.olympus.tenant_id : null;
            tenantSlug = typeof payload.olympus.tenant_slug === 'string' ? payload.olympus.tenant_slug : null;
          }
        }
      } catch (jwtError) {
        // Log JWT parsing errors for debugging but don't fail the request
        // User is still authenticated via getUser() above
        console.warn('[Auth] Failed to parse JWT claims:', jwtError instanceof Error ? jwtError.message : 'Unknown error');
      }
    }
  }
