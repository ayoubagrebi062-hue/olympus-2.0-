...
import { LOCKOUT_CONFIG, getProgressiveDelay, shouldLockAccount, getLockoutEndTime, isLockoutExpired } from '../constants';
...
export async function isLockedOut(identifier: string): Promise<{
  ...
  if (!record) {
    return { isLocked: false, lockoutEndsAt: null, attemptsRemaining: LOCKOUT_CONFIG.MAX_FAILED_ATTEMPTS };
  }
