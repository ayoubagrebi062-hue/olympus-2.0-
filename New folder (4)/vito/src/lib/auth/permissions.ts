...
export function getEffectivePermissions(
  ...
  const planFilteredPermissions = rolePermissions.filter(
    (permission) => isPlanAllowed(permission, planTier)
  );

  if (customPermissions) {
    const validCustomPermissions = customPermissions.filter(
      (permission) => isPlanAllowed(permission, planTier)
    );
...
export function getAssignableRoles(role: TenantRole): TenantRole[] {
  ...
  return (Object.keys(ROLE_HIERARCHY) as TenantRole[]).filter(
...
export const PERMISSION_GROUPS = {
  tenant: {
    label: 'Organization',
    permissions: [
      { permission: 'tenant:read', label: 'View organization', description: 'View organization details' },
      { permission: 'tenant:update', label: 'Update organization', description: 'Edit organization settings' },
      { permission: 'tenant:delete', label: 'Delete organization', description: 'Permanently delete the organization' },
      { permission: 'tenant:manage_billing', label: 'Manage billing', description: 'Update payment methods and subscription' },
      { permission: 'tenant:manage_members', label: 'Manage members', description: 'Invite, remove, and change member roles' },
      { permission: 'tenant:manage_settings', label: 'Manage settings', description: 'Configure organization settings' },
      { permission: 'tenant:view_audit_logs', label: 'View audit logs', description: 'Access security and activity logs' },
    ],
  },
  project: {
    label: 'Projects',
    permissions: [
      { permission: 'project:create', label: 'Create projects', description: 'Create new projects' },
      { permission: 'project:read', label: 'View projects', description: 'View project details' },
      { permission: 'project:update', label: 'Edit projects', description: 'Modify project settings and code' },
      { permission: 'project:delete', label: 'Delete projects', description: 'Permanently delete projects' },
      { permission: 'project:manage_collaborators', label: 'Manage collaborators', description: 'Add or remove project collaborators' },
      { permission: 'project:manage_env_vars', label: 'Manage environment', description: 'Configure environment variables' },
      { permission: 'project:export', label: 'Export projects', description: 'Download project source code' },
    ],
  },
  build: {
    label: 'Builds',
    permissions: [
      ...
      { permission: 'build:read', label: 'View builds', description: 'View build status and details' },
      ...
      { permission: 'build:view_costs', label: 'View costs', description: 'See build cost information' },
    ],
  },
  deployment: {
    label: 'Deployments',
    permissions: [
      { permission: 'deployment:create', label: 'Deploy', description: 'Deploy projects to production' },
      { permission: 'deployment:read', label: 'View deployments', description: 'View deployment status' },
      { permission: 'deployment:rollback', label: 'Rollback', description: 'Revert to previous deployments' },
      { permission: 'deployment:manage_domains', label: 'Manage domains', description: 'Configure custom domains' },
    ],
  },
  file: {
    label: 'Files',
  ...
  analytics: {
    label: 'Analytics',
    permissions: [
      { permission: 'analytics:read', label: 'View analytics', description: 'Access usage analytics' },
      { permission: 'analytics:export', label: 'Export analytics', description: 'Download analytics reports' },
    ],
  },
  api: {
    label: 'API Access',
    permissions: [
      { permission: 'api:read_keys', label: 'View API keys', description: 'View API key information' },
      { permission: 'api:create_keys', label: 'Create API keys', description: 'Generate new API keys' },
