...
export function withPromptProtection(
  ...
  return (handler) => async (request: NextRequest, ctx: any) => {
    try {
      ...
      if (!prompt) {
        return handler(request, ctx, { valid: true, blocked: false, sanitizedInput: '', detections: [], warnings: [] });
      }
...
export function withBruteForceProtection(): (
  ...
  return (handler) => async (request: NextRequest, ctx: any) => {
    ...
    if (!result.allowed) {
      ...
      const response = error('AUTH_002', 'Too many login attempts. Please try again later.', 429) as NextResponse;
