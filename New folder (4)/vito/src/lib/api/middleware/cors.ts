 ...
 */
export function addCorsHeaders(response: NextResponse, request: NextRequest, config: CorsConfig = DEFAULT_CORS): NextResponse {
  ...
  response.headers.set('Access-Control-Allow-Methods', (config.methods || DEFAULT_CORS.methods!).join(', '));
  response.headers.set('Access-Control-Allow-Headers', (config.headers || DEFAULT_CORS.headers!).join(', '));
 ...
 */
export function handlePreflight(request: NextRequest, config: CorsConfig = DEFAULT_CORS): NextResponse {
