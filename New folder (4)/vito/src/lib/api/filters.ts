...
export function sanitizeFilters(resource: string, filters: Record<string, unknown>): Record<string, unknown> {
...
export function parseCommonFilters(searchParams: URLSearchParams): {
  status?: string[]; createdAfter?: Date; createdBefore?: Date; updatedAfter?: Date; updatedBefore?: Date;
} {
...
export function buildSupabaseFilters(query: any, filters: Record<string, unknown>): any {
  ...
  for (const [key, value] of Object.entries(filters)) {
    if (['status', 'createdAfter', 'createdBefore', 'updatedAfter', 'updatedBefore'].includes(key)) continue;
...
/** Sort options */
export interface SortOptions { column: string; ascending: boolean }

/** Parse sort from query */
export function parseSortParams(searchParams: URLSearchParams, allowedColumns: string[]): SortOptions {
