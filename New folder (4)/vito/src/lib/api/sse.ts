...
export function createSSEStream(options: SSEOptions = {}): {
  ...
  const stream = new ReadableStream({
    start(ctrl) {
      ...
      ctrl.enqueue(encoder.encode(formatSSEMessage({ event: 'connected', data: { timestamp: Date.now() } })));
...
function loadSSEConfig(): SSEConfig {
  try {
    ...
    if (maxReconnectStr) {
      ...
      if (!isNaN(maxReconnect) && maxReconnect >= 0 && maxReconnect <= 10) { // Reasonable bounds
...
function validateSSEConfig(config: SSEConfig): { valid: boolean; errors: string[] } {
  ...
  Object.entries(config.baseTimeEstimates).forEach(([context, time]) => {
    if (typeof time !== 'number' || time < 10 || time > 3600) { // 10 seconds to 1 hour
      errors.push(`Invalid baseTimeEstimates.${context}: ${time}`);
    }
  });

  // Validate error recovery settings
  if (config.errorRecovery.maxReconnectAttempts < 0 || config.errorRecovery.maxReconnectAttempts > 20) {
    errors.push(`Invalid maxReconnectAttempts: ${config.errorRecovery.maxReconnectAttempts}`);
  }
  if (config.errorRecovery.baseReconnectDelay < 100 || config.errorRecovery.baseReconnectDelay > 10000) {
...
// Input validation and sanitization with chaos engineering defenses
  const sanitizeInput = (input: string, maxLength: number = sseConfig.security.maxInputLength || 500): string => {
    if (typeof input !== 'string') return '';
    if (input.length > maxLength * 2) { // Early rejection for extremely large inputs
  ...
  return {
    response,
    sendLog: (level: 'info' | 'warn' | 'error', message: string, meta?: Record<string, unknown>) => {
      try {
        const sanitizedMessage = sanitizeInput(message);
        send({ event: 'log', data: { level, message: sanitizedMessage, timestamp: new Date().toISOString(), ...meta } });
      } catch (error) {
        console.error('Failed to send log:', error);
        // Don't close connection on log failure - continue streaming
      }
    },
    sendProgress: async (step: string, progress: number, context?: keyof typeof BASE_TIME_ESTIMATES, customMessage?: string) => {
      try {
        ...
        if (context && !validateContext(context)) {
          console.warn(`SSE: Invalid progress context "${context}". Falling back to basic progress.`);
        ...
        if (context) {
          sendEnhancedProgress(send, sanitizedStep, clampedProgress, context, customMessage ? sanitizeInput(customMessage) : undefined);
        } else {
          const sanitizedMessage = customMessage ? sanitizeInput(customMessage) : undefined;
          send({ event: 'progress', data: { step: sanitizedStep, progress: clampedProgress, message: sanitizedMessage, timestamp: new Date().toISOString() } });
        }
      } catch (error) {
        console.error('Failed to send progress:', error);
        // Attempt recovery by sending error status
        try {
          this.sendError({ code: 'STREAM_ERROR', message: 'Progress update failed' });
        } catch (recoveryError) {
    ...
    sendStatus: (status: string, message?: string) => {
      try {
        ...
        send({ event: 'status', data: { status: sanitizedStatus, message: sanitizedMessage, timestamp: new Date().toISOString() } });
      } catch (error) {
      ...
      try {
        ...
        if (sanitizedError.canRetry && ['NETWORK_ERROR', 'TIMEOUT'].includes(error.code)) {
          ...
          reconnectTimeout = setTimeout(() => {
            send({ event: 'reconnecting', data: { attempt: connectionAttempts, timestamp: new Date().toISOString() } });
