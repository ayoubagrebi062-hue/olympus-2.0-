 ...
 */
function isPostgresError(error: unknown): error is { code: string; message: string; detail?: string } {
  return (
    typeof error === 'object' &&
    error !== null &&
    'code' in error &&
 ...
 */
function handlePostgresError(error: { code: string; message: string; detail?: string }, requestId: string): NextResponse<ApiErrorResponse> {
