...
export class ApiError extends Error {
  ...
  constructor(code: string, options?: { message?: string; status?: number; details?: unknown; requestId?: string; cause?: Error }) {
  ...
  toResponse(requestId?: string): ApiErrorResponse {
    return { success: false, error: { code: this.code, message: this.message, details: this.details, requestId: requestId || this.requestId || 'unknown' } };
  }
}
...
export class NotFoundError extends ApiError {
  constructor(resource: string, options?: { details?: unknown }) {
    super(`${resource.toUpperCase()}_001`, { message: `${resource} not found`, status: 404, details: options?.details });
...
export class RateLimitError extends ApiError {
  ...
  constructor(retryAfter: number, options?: { message?: string }) {
    super('RATE_001', { message: options?.message || 'Rate limit exceeded', status: 429, details: { retryAfter } });
...
export class ServiceUnavailableError extends ApiError {
  constructor(options?: { message?: string; details?: unknown }) {
    super('INT_002', { message: options?.message || 'Service temporarily unavailable', status: 503, details: options?.details });
