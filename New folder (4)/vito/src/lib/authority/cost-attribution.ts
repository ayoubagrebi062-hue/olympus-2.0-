...
export function getBuildCostSummary(buildId: string): BuildCostSummary | null {
  ...
  const burnRate = elapsedMs > 0
    ? ((state.totalInput + state.totalOutput) / (elapsedMs / 60000))
    : 0;

  const estimatedFinalCost = completionPercentage > 0
    ? totalCost / (completionPercentage / 100)
    : totalCost * 6; // Rough estimate
...
export function formatCostSummary(summary: BuildCostSummary): string {
  ...
  lines.push('BY PHASE:');
  for (const phase of summary.phases) {
    const percentage = summary.total.cost > 0
      ? ((phase.totalCost / summary.total.cost) * 100).toFixed(1)
      : '0.0';
  ...
  lines.push(`  Burn rate: ${Math.round(summary.projections.burnRate).toLocaleString()} tokens/min`);
  lines.push('');
