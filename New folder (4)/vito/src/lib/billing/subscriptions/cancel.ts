...
export async function cancelSubscription(
  ...
  const resolvedParams: CancelSubscriptionParams = typeof params === 'string'
    ? { tenantId: params, immediate: immediate ?? false }
    : params;
 ...
 */
export async function resumeSubscription(params: ResumeSubscriptionParams | string): Promise<Subscription> {
  ...
  await withStripeErrorHandling(
    () => stripe.subscriptions.update(subscription.stripe_subscription_id!, { cancel_at_period_end: false }),
    'Resume subscription'
  );
...
export async function pauseSubscription(tenantId: string): Promise<void> {
  const subscription = await requireSubscription(tenantId);
  if (!subscription.stripe_subscription_id) throw new BillingError(BILLING_ERROR_CODES.NO_ACTIVE_SUBSCRIPTION);

  const stripe = getStripe();
  await withStripeErrorHandling(
    () => stripe.subscriptions.update(subscription.stripe_subscription_id!, { pause_collection: { behavior: 'mark_uncollectible' } }),
    'Pause subscription'
  );
  await (createServiceRoleClient().from('subscriptions') as any).update({ status: 'paused' }).eq('id', subscription.id);
}
...
export async function unpauseSubscription(tenantId: string): Promise<void> {
  ...
  await (createServiceRoleClient().from('subscriptions') as any).update({ status: 'active' }).eq('id', subscription.id);
}
