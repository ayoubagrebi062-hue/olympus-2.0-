...
export async function isTrialEligible(tenantId: string): Promise<boolean> {
  ...
  const { data: rawData } = await supabase.from('subscriptions').select('id').eq('tenant_id', tenantId).limit(1);
...
export async function getTrialStatus(tenantId: string): Promise<Trial | null> {
  ...
  return {
    tenantId, planTier: subscription.plan.tier as PlanTier,
    startDate: subscription.trial_start || subscription.created_at,
    endDate: subscription.trial_end, daysRemaining,
    isExpired: now > trialEnd, wasExtended: (subscription as any).trial_extended || false,
  };
}
...
export async function extendTrial(params: ExtendTrialParams): Promise<Trial> {
  ...
  if (!subscription || subscription.status !== 'trialing') {
    throw new BillingError(BILLING_ERROR_CODES.NO_ACTIVE_SUBSCRIPTION, { reason: 'No active trial' });
  }
...
export async function getExpiringTrials(daysAhead: number = 3): Promise<string[]> {
  const futureDate = new Date(); futureDate.setDate(futureDate.getDate() + daysAhead);
  const { data: rawData } = await createServiceRoleClient().from('subscriptions')
    .select('tenant_id').eq('status', 'trialing')
    .lte('trial_end', futureDate.toISOString()).gt('trial_end', new Date().toISOString());
  const data = rawData as any;
...
export async function handleTrialExpired(tenantId: string): Promise<void> {
  const supabase = createServiceRoleClient();
  const { data: rawFreePlan } = await supabase.from('plans').select('id').eq('tier', 'free').single();
  const freePlan = rawFreePlan as any;
  if (!freePlan) return;

  await (supabase.from('subscriptions') as any).update({
    plan_id: freePlan.id, status: 'canceled', trial_end: new Date().toISOString(),
  }).eq('tenant_id', tenantId).eq('status', 'trialing');
}
...
export function getTrialConfig() {
  return { durationDays: TRIAL_CONFIG.DURATION_DAYS, planTier: TRIAL_CONFIG.PLAN_TIER, requiresPaymentMethod: TRIAL_CONFIG.REQUIRE_PAYMENT_METHOD };
}
