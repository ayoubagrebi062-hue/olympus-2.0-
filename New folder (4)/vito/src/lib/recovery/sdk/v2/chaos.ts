...
class ChaosEngine {
  ...
  private shouldAffect(operationName: string): boolean {
    ...
    if (this.config.excludedOperations.length > 0) {
      if (this.config.excludedOperations.some(op =>
        operationName.includes(op) || op.includes(operationName)
      )) {
        return false;
      }
    }
    ...
    return this.config.affectedOperations.some(op =>
      operationName.includes(op) || op.includes(operationName)
    );
  }
  ...
  private emit(event: ChaosEvent): void {
    ...
    // Verbose logging
    if (this.config.verbose) {
      const emoji = event.type === 'failure' ? '💥' :
                    event.type === 'latency' ? '🐢' :
                    event.type === 'timeout' ? '⏰' : '⏭️';
      console.warn(`[CHAOS] ${emoji} ${event.type.toUpperCase()} on ${event.operation}`, event.details);
    }
