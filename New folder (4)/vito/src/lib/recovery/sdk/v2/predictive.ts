...
export class PredictiveCircuit {
  ...
  constructor(name: string, config: PredictiveConfig = {}) {
    ...
    this.config = {
      ...
      latencyIncreaseThreshold: validatePositiveInt('latencyIncreaseThreshold', config.latencyIncreaseThreshold, 1, 1000, 50),
      errorRateThreshold: validatePositiveInt('errorRateThreshold', config.errorRateThreshold, 1, 100, 10),
      confidenceThreshold: validateProbability('confidenceThreshold', config.confidenceThreshold, 0.7),
  ...
  shouldDegrade(): boolean {
    ...
    return prediction.recommendation === 'DEGRADE_NOW' ||
           prediction.recommendation === 'CIRCUIT_OPEN';
  }
  ...
  getStats(): PredictorStats {
    ...
    return {
      ...
      currentLatencyAvg: latencies.length > 0
        ? Math.round(latencies.reduce((a, b) => a + b, 0) / latencies.length)
        : 0,
      currentErrorRate: recentSamples.length > 0
        ? Math.round((errors / recentSamples.length) * 100)
        : 0,
      latencyTrend,
      errorTrend,
  ...
  private analyzeErrorTrend(): { risk: number; reason?: string } {
    ...
    const olderErrorRate = older.length > 0
      ? older.filter(s => !s.success).length / older.length
      : 0;
  ...
  private updateTimePattern(sample: MetricSample, timestamp: number): void {
    ...
    const alpha = 0.1;
    pattern.avgLatency = pattern.count === 0
      ? sample.latency
      : pattern.avgLatency * (1 - alpha) + sample.latency * alpha;

    pattern.errorRate = pattern.count === 0
      ? (sample.success ? 0 : 1)
