...
// Advanced Modules
// ============================================================================
export { AdaptiveBackoff, getAdaptiveBackoff, parseRetryAfter, clearBackoffRegistry } from './adaptive-backoff';
...
export type { FallbackOptions, FallbackHealth, ChainResult, ChainHealthReport } from './fallback-chain';

export { chaos, withChaos, chaosify, ChaosErrors } from './chaos';
export type { ChaosConfig, ChaosEvent, ChaosEventHandler } from './chaos';

export { RequestCoalescer, getCoalescer, getAllCoalescerStats, clearCoalescerRegistry, createApiCoalescer, createDbCoalescer } from './coalescing';
export type { CoalescerConfig, CoalescerStats } from './coalescing';

export { PredictiveCircuit, getPredictiveCircuit, getAllPredictorStats, clearPredictorRegistry } from './predictive';
...
export { metrics, ConsoleExporter, PrometheusExporter, OpenTelemetryExporter, named, generateTraceId } from '../metrics';
export type { MetricsExporter, RetryMetricData, SuccessMetricData, FailureMetricData, CircuitMetricData } from '../metrics';
...
import { PredictiveCircuit, getPredictiveCircuit, PredictiveConfig, Prediction } from './predictive';
...
function getSystemHealth(): SystemHealth {
  ...
  for (const p of predictors) {
    if (p.prediction.recommendation === 'CIRCUIT_OPEN') {
      ...
      alerts.push(`Predictor ${p.name}: Degradation recommended - ${p.prediction.reasons.join(', ')}`);
      score -= 15;
  ...
  for (const c of coalescers) {
    if (c.dedupRatio > 0.5) {
      recommendations.push(`Coalescer ${c.name} has ${Math.round(c.dedupRatio * 100)}% dedup rate - consider caching`);
    }
  }
