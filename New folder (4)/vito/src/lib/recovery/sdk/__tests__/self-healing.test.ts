...
function failNTimes<T>(n: number, successValue: T, errorMessage = 'Simulated failure'): () => Promise<T> {
  let failures = 0;
  return async () => {
    if (failures < n) {
      failures++;
...
describe('Simple API', () => {
  describe('retry()', () => {
    ...
    it('throws on undefined operation', async () => {
      await expect(retry(undefined as any)).rejects.toThrow('withRetry: operation cannot be undefined');
    });
  ...
  describe('fallback()', () => {
    ...
    it('throws when both fail', async () => {
      await expect(
        fallback(alwaysFails('primary'), alwaysFails('fallback'))
      ).rejects.toThrow('Both primary and fallback');
    });
  });
});
...
describe('Input Validation (Chaos Engineering)', () => {
  ...
  it('truncates huge error messages', async () => {
    ...
    const result = await withRetry(() => { throw new Error(hugeMessage); }, { maxAttempts: 1 });
