...
export class SelfHealing {
  ...
  static async execute<T>(
    ...
    const builder = new SelfHealingBuilderImpl<T>()
      .withStrategy('exponential', { maxAttempts: options.maxAttempts ?? 3 });
...
class SelfHealingExecutorImpl<T> implements SelfHealingExecutor<T> {
  ...
  constructor(config: SelfHealingConfig<T>, eventHandlers: RecoveryEventHandler[], operationName: string) {
    this.config = config;
  ...
  async execute(operation: Operation<T>): Promise<RecoveryResult<T>> {
    ...
    for (attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (this.config.signal?.aborted) {
        return this.createError(RecoveryErrorCode.CANCELLED, 'Operation cancelled', startTime, attempt - 1);
      }
    ...
    metrics.emitFailure({
      ...
      labels: { operation: this.getOperationName(operation), error_code: RecoveryErrorCode.MAX_RETRIES_EXCEEDED, trace_id: traceId },
      timestamp: Date.now(),
      traceId,
    });
  ...
  private async executeFallback(
    ...
    try {
      ...
      metrics.emitFailure({
        ...
        labels: { operation: opName, error_code: RecoveryErrorCode.FALLBACK_FAILED, trace_id: traceId },
        timestamp: Date.now(),
        traceId,
      });
      ...
      return this.createError(
        ...
        attempts,
        lastError,
        { fallbackError: fallbackError instanceof Error ? fallbackError.message : String(fallbackError), traceId }
      );
    }
  }
  ...
  private handleCircuitOpen(startTime: number, operation?: Operation<T>, traceId?: string): RecoveryResult<T> {
  ...
  private recordSuccess(): void {
    ...
    if (this.circuitState.state === 'half-open') {
      ...
      if (halfOpen.testAttempts >= (this.config.circuitBreaker.successThreshold ?? DEFAULTS.CIRCUIT_SUCCESS_THRESHOLD)) {
  ...
  private createError(
    ...
    return Err(createRecoveryError(code, message, {
      cause,
      attempts,
      elapsed: Date.now() - startTime,
      context,
