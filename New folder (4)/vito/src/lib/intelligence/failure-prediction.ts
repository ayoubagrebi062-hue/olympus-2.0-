...
const AGENT_RISK_FACTORS: Record<string, { complexity: number; failureRate: number; riskKeywords: string[] }> = {
  schemaforge: {
    complexity: 0.7,
    failureRate: 0.25,
...
export class FailurePredictionEngine {
  ...
  predict(input: PredictionInput): FailurePrediction {
    ...
    const improvedSuccessRate = this.estimateImprovedRate(
      failureProbability,
      preventiveActions
    );
  ...
  private analyzeRiskFactors(input: PredictionInput): RiskFactor[] {
    ...
    const featureIndicators = prompt.match(/(?:feature|functionality|capability|ability to|should be able to)/gi) || [];
    ...
    for (const agent of input.config.agents) {
      ...
      if (agentRisk) {
        const matches = agentRisk.riskKeywords.filter(kw =>
  ...
  private calculateFailureProbability(
    ...
    for (const pred of predictedFailures) {
      const severityMultiplier =
        pred.severity === 'critical' ? 0.3 :
        pred.severity === 'major' ? 0.2 : 0.1;
  ...
  private suggestClarification(occurrences: PatternMatch[], prompt: string): string {
    ...
    for (const occ of occurrences.slice(0, 3)) {
      ...
      const after = prompt.substring(occ.location.end, Math.min(prompt.length, occ.location.end + 20));
  ...
  private determineRiskLevel(
    ...
    const weightedRisk = failureProbability * confidence + failureProbability * (1 - confidence) * 0.5;
