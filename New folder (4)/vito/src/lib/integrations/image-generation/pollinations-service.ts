...
export interface PollinationsRequest {
  prompt: string;
  type?: 'hero' | 'icon' | 'avatar' | 'product' | 'logo' | 'banner' | 'background' | 'illustration' | 'thumbnail' | 'custom';
  style?: 'cinematic' | 'anime' | 'cyberpunk' | 'fantasy' | 'photorealistic' | 'minimalist' | 'watercolor' | 'oil-painting' | 'pixel-art' | 'comic' | 'neon' | 'vintage' | '3d-render' | 'digital-art' | 'steampunk' | 'none';
  ...
  async generateBatch(prompts: LeonardoPrompt[]): Promise<BatchGenerationResult> {
    ...
    for (let i = 0; i < prompts.length; i += this.concurrency) {
      ...
      const batchResults = await Promise.all(
        batch.map(prompt =>
          this.generateImage(prompt.id, this.mapToPollinationsRequest(prompt))
        )
      );
    ...
    console.log(`[Pollinations] Batch complete: ${success.length}/${prompts.length} succeeded (${totalTime}ms)`);
...
export function getImageService(webhookUrl?: string): PollinationsService {
  if (!serviceInstance) {
    ...
    if (!url) {
      throw new Error('N8N_WEBHOOK_URL environment variable not set. Set it to your n8n webhook URL.');
    }
...
function validatePrompt(prompt: string): string {
  ...
  if (trimmed.length > MAX_PROMPT_LENGTH) {
    console.warn(`[Pollinations] Prompt truncated from ${trimmed.length} to ${MAX_PROMPT_LENGTH} chars`);
