 ...
 * Set user's industry
 */
export async function setUserIndustry(
  userId: string,
  industryName: string
): Promise<void> {
  await writeQuery(
 ...
 */
export async function getUserTechPreferences(userId: string): Promise<Array<{
  name: string;
  category: string;
  weight: number;
}>> {
 ...
 */
export async function getUserDesignPatterns(userId: string): Promise<Array<{
  name: string;
  description: string;
  weight: number;
}>> {
 ...
 */
export async function recordProjectTech(
  projectId: string,
  techNames: string[]
): Promise<void> {
