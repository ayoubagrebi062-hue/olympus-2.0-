...
export class IntentMemoryStore {
  ...
  getMemoriesForSignature(
    ...
    return this.memories.filter(m =>
      m.clarificationSignature === signature &&
  ...
  getSignatureStats(
    ...
    return {
      ...
      mostCommonAnswer,
      confidence,
      lastSeen: memories.reduce((latest, m) =>
        m.timestamp > latest ? m.timestamp : latest, memories[0].timestamp
      ),
      convergedCount,
      blockedCount,
      convergeRate,
...
export function inferDefaultAnswers(
  ...
  console.log(`[IMPL] Config: minOccurrences=${config.minOccurrences}, confidence=${config.confidenceThreshold}`);
...
function validateInferredAnswer(
  ...
  switch (clarification.answerType) {
    case 'ENUM':
      if (clarification.allowedValues &&
          !clarification.allowedValues.includes(inferredValue)) {
        return {
          valid: false,
    ...
    case 'NUMBER':
      ...
      if (clarification.valueRange) {
        if (inferredValue < clarification.valueRange.min ||
 ...
 */
export function inferredToIntentAnswers(
...
export function logPolicyReport(report: PolicyApplicationReport): void {
  ...
  if (report.confidenceScores.length > 0) {
    ...
    for (const score of report.confidenceScores.slice(0, 5)) {
      console.log(`[IMPL]   ${score.clarificationId}: ${(score.confidence * 100).toFixed(0)}% (converge: ${(score.convergeRate * 100).toFixed(0)}%)`);
    }
  }
