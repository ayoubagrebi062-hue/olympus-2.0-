...
export function detectInternalStructureContradictions(
  ...
  for (const chain of chains) {
    ...
    for (const gap of gaps.filter(g => g.severity === 'critical')) {
      if (!contradictions.some(c => c.sourceIntentId === intent.id && c.conflictDescription.includes(gap.message))) {
        contradictions.push({
          id: generateContradictionId(),
          type: 'INTENT_VS_INTERNAL_STRUCTURE',
          sourceIntentId: intent.id,
...
function areTransitionsConflicting(a: string, b: string): boolean {
  ...
  for (const [x, y] of opposites) {
    if ((aLower.includes(x) && bLower.includes(y)) ||
...
export function classifyImpossibilities(
  ...
  for (const assumption of criticalDenied) {
    ...
    impossibilities.push({
      ...
      evidence: [
        ...
        assumption.validationEvidence || 'No evidence available',
      ],
      relatedIntents: [],
  ...
  for (const chain of chains) {
    ...
    const totalScore = axisScores.trigger + axisScores.state + axisScores.effect + axisScores.outcome;
...
export function generateUserFeedback(
  ...
  const requiredClarifications = generateClarifications(
    chains,
    contradictions,
    impossibilities
  );
...
function generateRecommendedActions(
  ...
  if (logicalConflicts.length > 0) {
    actions.push(`Resolve ${logicalConflicts.length} conflicting requirement(s) by choosing which takes priority`);
  }
...
export function runIntentRefinementAnalysis(
  ...
  console.log(`[IRCL]   Extracted ${totalAssumptions} assumptions (${deniedAssumptions} denied, ${criticalDeniedAssumptions} critical)`);

  // Step 2: Detect contradictions
  console.log('[IRCL] Step 2: Detecting contradictions...');
  const externalContradictions = detectExternalRealityContradictions(chains, eraReport, governanceReport);
  ...
  const contradictions = [...externalContradictions, ...internalContradictions, ...intentContradictions];
  const blockingContradictions = contradictions.filter(c => c.severity === 'BLOCKING').length;
  console.log(`[IRCL]   Found ${contradictions.length} contradictions (${blockingContradictions} blocking)`);
  ...
  const proceedBlocker = !canProceed
...
export function logIntentRefinementReport(report: IntentRefinementReport): void {
  ...
  console.log(`[IRCL] Assumptions: ${report.totalAssumptions} (${report.criticalDeniedAssumptions} critical denied)`);
  console.log(`[IRCL] Contradictions: ${report.contradictions.length} (${report.blockingContradictions} blocking)`);
