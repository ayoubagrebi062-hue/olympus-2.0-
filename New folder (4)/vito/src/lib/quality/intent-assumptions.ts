...
const CATEGORY_ASSUMPTION_RULES: Record<IntentCategory, AssumptionCategory[]> = {
  ...
  form_submission: ['API_AVAILABLE', 'PERMISSION_GRANTED', 'FORMAT_COMPATIBLE', 'NETWORK_AVAILABLE'],
...
export function extractAssumptionsFromIntent(
  ...
  for (const [category, patterns] of Object.entries(ASSUMPTION_INDICATORS)) {
    for (const pattern of patterns) {
      ...
      if (match) {
        const subject = match[1] || extractSubjectFromRequirement(requirement, category as AssumptionCategory);
 ...
 */
function extractSubjectFromRequirement(
  requirement: string,
  category: AssumptionCategory
): string {
 ...
 */
export function extractAllAssumptions(
  ...
  for (const chain of chains) {
    ...
    const unknown = assumptions.filter(a => a.validationResult === 'UNKNOWN' || !a.validated).length;
    const criticalDenied = assumptions.filter(a => a.critical && a.validationResult === 'DENIED').length;
 ...
 */
export function getDeniedAssumptions(
...
export function getCriticalDeniedAssumptions(
  ...
  return summaries.flatMap(s => s.assumptions.filter(a => a.critical && a.validationResult === 'DENIED'));
}
...
export function logAssumptionSummary(summary: IntentAssumptionSummary): void {
  ...
  console.log(`[IRCL]   Assumptions: ${summary.totalAssumptions} (✓${summary.confirmedAssumptions} ✗${summary.deniedAssumptions} ?${summary.unknownAssumptions})`);

  if (summary.criticalDenied > 0) {
    console.log(`[IRCL]   ⚠️ ${summary.criticalDenied} CRITICAL assumption(s) denied`);
    for (const assumption of summary.assumptions.filter(a => a.critical && a.validationResult === 'DENIED')) {
...
export function logAllAssumptions(summaries: IntentAssumptionSummary[]): void {
  ...
  console.log(`[IRCL] Confirmed: ${totalConfirmed} | Denied: ${totalDenied} | Critical Denied: ${totalCriticalDenied}`);
  console.log('[IRCL] ------------------------------------------');
