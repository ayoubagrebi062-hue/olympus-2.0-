...
function isCanonicalModule(modulePath: string): boolean {
  const normalized = modulePath.replace(/\\/g, '/').toLowerCase();
  return normalized.includes('/src/lib/quality/') ||
         CANONICAL_MODULES.some(m => normalized.includes(m));
}
  ...
  const scanDir = (dir: string) => {
    ...
    for (const item of items) {
      ...
      if (stat.isDirectory()) {
        ...
        if (content.includes("governanceAuthority: 'CANONICAL'") ||
        ...
        if (content.includes('canShip: true') ||
            content.includes('canShip:true')) {
          breaches.push({
            type: 'RESEARCH_CLAIMING_CANONICAL',
            source: itemPath.replace(projectRoot, ''),
