 ...
 */
export const CONSTITUTIONAL_ARTICLES: Record<ConstitutionalArticle, {
  title: string;
  description: string;
  guarantee: string;
}> = {
...
export function runConstitutionalTests(
  ...
  const art6Violations = testHardGateBlocking(inputs.gateResults, overallShipDecision, inputs.buildId);
