...
export class BuildVerifier implements QualityGate {
  ...
  private checkBasicSyntax(file: FileToCheck): GateIssue[] {
    ...
    lines.forEach((line, index) => {
      ...
      if (line.match(/(?:const|let|var)\s+\w+\s*=\s*[^;]+$/) && !line.trim().endsWith(',') && !line.trim().endsWith('{')) {
      ...
      if (line.match(/^\s*\w+\s*=\s*/) && !line.match(/^\s*(?:const|let|var|this\.|return|export|import)/)) {
  ...
  private checkExports(file: FileToCheck): GateIssue[] {
    ...
    if (file.path.match(/\.(jsx|tsx)$/) && file.path.includes('components')) {
      const hasDefaultExport = file.content.includes('export default') || file.content.includes('export { default }');
