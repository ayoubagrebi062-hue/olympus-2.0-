...
export interface AdversarialGovernanceResult {
  ...
  exploitsByType: Record<GovernanceExploitType, {
    attempted: number;
    blocked: number;
    leaked: number;
  }>;
  ...
  return [
    ...
    },
    {
      ...
      payload: {
        ...
        modification: { forbiddenBelow: 0.10 },  // Lower from 30% to 10%
      },
      expectedResult: 'BLOCKED',
    },
...
function validateOverrideAttempt(payload: Record<string, unknown>): {
  ...
  if (previousOverrideMs !== undefined && previousOverrideMs < GOVERNANCE_LIMITS.cooldownBetweenOverridesMs) {
    return {
      blocked: true,
...
export function runAdversarialGovernanceHarness(): AdversarialGovernanceResult {
  ...
  const exploitsByType: Record<GovernanceExploitType, { attempted: number; blocked: number; leaked: number }> = {
