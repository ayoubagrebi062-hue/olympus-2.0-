...
export const OVERRIDE_PENALTIES: Record<OverrideTarget, number> = {
  ...
  STABILITY_WARNING: 0.10,       // 10% SSI penalty - moderate
 ...
 */
export function saveOverrideAuditEntry(
  fsOutputDir: string,
...
export function logOverrideStatus(summary: OverrideSummary): void {
  ...
  if (summary.hasActiveOverride && summary.override) {
    ...
    if (summary.ssiAdjustment) {
      console.log(`[OVERRIDE]   SSI: ${(summary.ssiAdjustment.before * 100).toFixed(1)}% → ${(summary.ssiAdjustment.after * 100).toFixed(1)}%`);
    }
  } else {
