...
export class IntentGraphStore {
  ...
  save(
    ...
    if (changes) {
      ...
      console.log(`[IntentStore]     W-ISS-D delta: ${changes.wissScoreDelta > 0 ? '+' : ''}${changes.wissScoreDelta}%`);
    }
...
export function exportIntentGraphReadable(graph: VersionedIntentGraph): string {
  ...
  if (graph.changes) {
    ...
    lines.push(`W-ISS-D: ${graph.changes.wissScoreDelta > 0 ? '+' : ''}${graph.changes.wissScoreDelta}%`);
