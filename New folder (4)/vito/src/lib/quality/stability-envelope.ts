...
export const DEFAULT_SSI_WEIGHTS: SSIWeights = {
  wissdTrendStability: 0.30,     // 30% - Most important: is W-ISS-D stable?
  rglTrustConfidence: 0.20,      // 20% - Are external validations trustworthy?
  policyInferenceRatio: 0.15,    // 15% - Are we relying too much on policy?
  itgclCouplingDensity: 0.20,    // 20% - Is the system too tightly coupled?
 ...
 */
function computePolicyRatioScore(policyData: PolicyInferenceData | null): { score: number; reason: string } {
...
function computeCouplingScore(coupling: ITGCLCouplingData): { score: number; reason: string } {
  ...
  const modularity = coupling.nodeCount > 0
    ? coupling.connectedComponents / coupling.nodeCount
    : 1;

  // High conflict edges are bad
  const conflictRatio = coupling.edgeCount > 0
    ? coupling.highConflictEdges / coupling.edgeCount
    : 0;
...
function computeChurnScore(churn: ClarificationChurnData): { score: number; reason: string } {
  ...
  const resolutionRatio = totalClarifications > 0
    ? churn.resolvedClarifications / totalClarifications
    : 1;
...
export function addStabilityEntry(
  ...
  const trimmedEntries = newEntries.length > maxEntries
...
export function runStabilityEnvelopeAnalysis(
  ...
  const averageSSI = history.entries.length > 0
  ...
  console.log(`[SCE]   W-ISS-D Trend: ${(components.wissdTrendScore * 100).toFixed(0)}% - ${components.wissdTrendReason}`);
  console.log(`[SCE]   RGL Trust: ${(components.rglTrustScore * 100).toFixed(0)}% - ${components.rglTrustReason}`);
  console.log(`[SCE]   Policy Ratio: ${(components.policyRatioScore * 100).toFixed(0)}% - ${components.policyRatioReason}`);
  console.log(`[SCE]   Coupling: ${(components.couplingScore * 100).toFixed(0)}% - ${components.couplingReason}`);
  console.log(`[SCE]   Churn: ${(components.churnScore * 100).toFixed(0)}% - ${components.churnReason}`);
  console.log('[SCE] ==========================================');
...
export function logStabilityEnvelopeResult(result: StabilityEnvelopeResult): void {
  ...
  console.log(`[SCE] Trend: ${result.trendDirection} (Δ${(result.trendMagnitude * 100).toFixed(1)}%)`);
  console.log('[SCE] ------------------------------------------');
