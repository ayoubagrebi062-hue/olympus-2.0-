...
function validateCTAs(content: string, path: string): GateIssue[] {
  ...
  const largePrimaryPattern = /size=["']lg["'][^>]*variant=["']primary["']|variant=["']primary["'][^>]*size=["']lg["']/g;
...
function validateDensity(content: string, path: string): GateIssue[] {
  ...
  const interactivePattern = /<(Button|button|a|Link|input|select|textarea|Switch|Checkbox|Radio)/gi;
...
function validatePageStructure(content: string, path: string): GateIssue[] {
  ...
  const hasActions = content.includes('<Button') ||
                     content.includes('<a ') ||
...
export async function layoutGate(files: FileToCheck[]): Promise<GateResult> {
  ...
  for (const file of files) {
    ...
    const isPage = file.path.includes('page.') || file.path.includes('Page.') || file.path.includes('/pages/');
