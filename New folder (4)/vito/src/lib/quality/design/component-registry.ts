...
const ATOMS: Record<string, ComponentSpec> = {
  Button: {
    ...
    optionalProps: ['variant', 'size', 'disabled', 'loading', 'type', 'onClick', 'className', 'asChild'],
    anatomy: {
  ...
  Input: {
    ...
    optionalProps: ['placeholder', 'type', 'disabled', 'error', 'className', 'id', 'name', 'required'],
    anatomy: {
  ...
  Avatar: {
    ...
    rules: [
      'MUST have alt text or aria-label',
      'Fallback MUST be shown if image fails',
    ],
  },
  ...
  Spinner: {
    ...
    rules: [
      'MUST have aria-label="Loading"',
      'Should use prefers-reduced-motion',
    ],
  },
  ...
  Switch: {
    ...
    rules: [
      'MUST have associated label',
      'Visual state must be clear (on/off)',
    ],
    a11y: {
      role: 'switch',
      ariaRequired: ['aria-checked'],
  ...
  Textarea: {
    ...
    rules: [
      'MUST have associated label',
      'Consider showing character count if maxLength',
    ],
  },
  ...
  Skeleton: {
    ...
    rules: [
      'Should use prefers-reduced-motion',
      'Should match dimensions of content being loaded',
    ],
  },
  ...
  Heading: {
    ...
    rules: [
      'MUST follow heading hierarchy (no skipping levels)',
...
const MOLECULES: Record<string, ComponentSpec> = {
  ...
  FormField: {
    ...
    rules: [
      'Vertical stack with space-y-1.5',
      'Error message replaces helper text when in error',
    ],
  },
...
const ORGANISMS: Record<string, ComponentSpec> = {
  ...
  Card: {
    ...
    rules: [
      'Interactive cards MUST have hover state',
...
export function getComponentCategory(name: string): 'atom' | 'molecule' | 'organism' | 'template' | null {
