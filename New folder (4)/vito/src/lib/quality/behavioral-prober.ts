...
function extractInteractiveElements(filePath: string, content: string): InteractiveElement[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Find links
    const linkMatch = line.match(/<a[^>]*href\s*=\s*['"]([^'"]+)['"]/i) ||
...
function hasRealBehavior(handlerCode: string | undefined, fullContent: string): boolean {
  ...
  if (functionMatch) {
    ...
    const funcDefPattern = new RegExp(`(const|function)\\s+${funcName}\\s*=?\\s*[^{]*\\{([^}]+)\\}`, 's');
...
function isCrudElement(element: InteractiveElement): 'create' | 'update' | 'delete' | null {
  const text = (element.label || '').toLowerCase() + ' ' + (element.handlerCode || '').toLowerCase();
...
function probeElement(element: InteractiveElement, fullContent: string): ProbeResult {
  ...
  switch (element.type) {
      ...
      if (hasSubmit) {
        ...
        behaviors.push({
          ...
          message: hasApi
            ...
            : 'Form submission missing API call',
        });
      }
      break;
    }
    ...
    case 'input': {
      ...
      if (hasOnChange) {
        const updatesState = element.handlerCode?.includes('set') ||
...
export function runBehavioralAnalysis(buildDir: string): BehavioralReport {
  ...
  console.log(`[BehavioralProber] Passed: ${passedElements}, Failed: ${failedElements}, Critical: ${criticalFailures}`);
  console.log(`[BehavioralProber] Buttons: ${summary.buttons.withRealActions}/${summary.buttons.total} with real actions`);
  console.log(`[BehavioralProber] Forms: ${summary.forms.withApiCalls}/${summary.forms.total} with API calls`);
