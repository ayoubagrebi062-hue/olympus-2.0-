...
const DOMAIN_KEYWORDS: Record<DomainType, readonly string[]> = Object.freeze({
  crud: Object.freeze([
    ...
    'entity', 'item', 'record', 'data',
  ]),
  auth: Object.freeze([
    'login', 'logout', 'sign in', 'sign out', 'sign up',
    'authenticate', 'authorization', 'auth',
    'register', 'password', 'session', 'token',
    'user', 'account', 'credential',
  ]),
  dashboard: Object.freeze([
    'dashboard', 'analytics', 'metrics', 'statistics', 'stats',
    'chart', 'graph', 'visualization', 'report',
    'kpi', 'trend', 'overview', 'summary',
  ]),
  form: Object.freeze([
    'form', 'input', 'field', 'validation', 'validate',
    'submit', 'submission', 'contact', 'feedback',
    'required', 'optional', 'error message',
  ]),
  search: Object.freeze([
    'search', 'find', 'query', 'filter', 'sort',
    'pagination', 'page', 'results', 'match',
  ]),
  generic: Object.freeze([
    'navigate', 'page', 'route', 'link',
 ...
 */
export function extractCapabilitiesFromChains(
 ...
 */
function extractCapabilitiesFromIntent(
  text: string,
  ...
  if (text.includes('read') || text.includes('view') || text.includes('display') || text.includes('show')) {
  ...
  if (text.includes('password') && (text.includes('reset') || text.includes('recover') || text.includes('forgot'))) {
  ...
  if (text.includes('page') || text.includes('pagination') || text.includes('next') || text.includes('previous')) {
  ...
  // Generic capabilities
  if (text.includes('navigate') || text.includes('go to') || text.includes('link') || category === 'navigation') {
 ...
 */
export function computeIntentAdequacy(
  chains: IntentCausalChain[]
): IntentAdequacyResult {
  console.log('[IAL] ==========================================');
  ...
  const satisfiedRequired = requiredCapabilities.filter(
  ...
  // Step 5: Compute IAS
  const ias = requiredCapabilities.length > 0
    ? satisfiedRequired.length / requiredCapabilities.length
    : 1.0;

  console.log(`[IAL] ------------------------------------------`);
  console.log(`[IAL] IAS = ${satisfiedRequired.length}/${requiredCapabilities.length} = ${(ias * 100).toFixed(1)}%`);
...
export const HOSTILE_MINIMAL_INTENTS: readonly HostileMinimalIntentCase[] = Object.freeze([
  {
    ...
    mustBlock: true,
    reason: 'CRUD app with only Read. No Create, Update, Delete, or List. Technically correct but pointless for a CRUD application.',
  },
  {
    ...
    mustBlock: true,
    reason: 'Auth app with only Login. No Logout or Session Management. User would be permanently logged in with no escape.',
  },
  {
    ...
    mustBlock: true,
    reason: 'Dashboard with only static chart. No metrics, no refresh, no filtering. A frozen image masquerading as a dashboard.',
  },
  {
    ...
    mustBlock: true,
    reason: 'Search feature with no actual search functionality. User can type but nothing happens. The cruelest form of correctness.',
  },
  {
    ...
    mustBlock: true,
    reason: 'Form that accepts input but has no validation, no submission, no errors, no success feedback. A black hole for user data.',
  },
]);
...
export function runHostileMinimalIntentValidation(): HostileMinimalIntentValidation {
  ...
  for (const hostileCase of HOSTILE_MINIMAL_INTENTS) {
    ...
    const satisfiedCount = hostileCase.providedCapabilities.filter(
      cap => requiredCapabilities.includes(cap)
    ).length;

    const actualIAS = requiredCapabilities.length > 0
      ? satisfiedCount / requiredCapabilities.length
      : 1.0;
...
export function logIntentAdequacy(result: IntentAdequacyResult): void {
  ...
  console.log(`[IAL] Capabilities: ${result.satisfiedCapabilities.length}/${result.requiredCapabilities.length} satisfied`);
