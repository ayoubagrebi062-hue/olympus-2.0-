...
import { RequiredClarification, IntentImpossibility, IntentImpossibilityType } from './intent-contradictions';
...
function inferAxisAndMutation(
  ...
  if (questionLower.includes('display') ||
  ...
  if (questionLower.includes('connect') ||
...
function generateAnswerSpec(req: RequiredClarification): {
  ...
  const questionLower = req.question.toLowerCase();
  if (questionLower.includes('should') ||
...
function validateSchemaAnswer(
  ...
  for (const [key, propValue] of Object.entries(value) as [string, any][]) {
    ...
    if (actualType !== propSchema.type) {
      return { valid: false, error: `Property ${key}: expected ${propSchema.type}, got ${actualType}` };
    }
...
export function applyIntentAnswers(
  ...
  for (const answer of sortedAnswers) {
    ...
    if (result.applied) {
      ...
      answersApplied++;
      console.log(`[IRE] ✓ ${answer.clarificationId}: Applied ${clarification.mutationType} on ${clarification.axis}`);
    } else {
      answersRejected++;
...
function applyTriggerMutation(
  ...
  switch (mutationType) {
    ...
    default:
      return { applied: false, rejectionReason: `Unsupported mutation: ${mutationType} for trigger` };
  }
}
...
function applyOutcomeMutation(
  ...
  switch (mutationType) {
    ...
    default:
      return { applied: false, rejectionReason: `Unsupported mutation: ${mutationType} for outcome` };
  }
}
...
function applyRequirementMutation(
  ...
  switch (mutationType) {
    ...
    default:
      return { applied: false, rejectionReason: `Unsupported mutation: ${mutationType} for requirement` };
  }
}
...
export function logClarifications(clarifications: IntentClarification[]): void {
  ...
  for (const c of clarifications) {
    ...
    if (c.allowedValues) {
      console.log(`[IRE]   Options: ${c.allowedValues.slice(0, 3).join(', ')}${c.allowedValues.length > 3 ? '...' : ''}`);
    }
  }
