...
export class AgentLayer implements GuardrailLayerHandler {
  ...
  constructor(
    ...
    this.defaultCapabilities = new Set([
      'read_files',
      'generate_content',
      'analyze_data',
    ]);
  ...
  private checkCapabilities(
    ...
    if (rules.blockedCapabilities) {
      const blocked = requestedCapabilities.filter(cap =>
        rules.blockedCapabilities!.includes(cap)
      );
    ...
    if (rules.allowedCapabilities) {
      const notAllowed = requestedCapabilities.filter(cap =>
        !rules.allowedCapabilities!.includes(cap)
      );
