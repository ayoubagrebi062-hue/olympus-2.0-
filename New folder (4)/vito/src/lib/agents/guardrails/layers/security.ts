...
const PROMPT_INJECTION_PATTERNS: SecurityPattern[] = [
  ...
  // === NEW: Additional bypass patterns found during chaos engineering ===
  {
    ...
    pattern: /\b(disregard|forget|override|bypass|skip)\s+(all\s+)?(earlier|previous|prior|above|preceding)\s+(instructions|guidelines|rules|directives|constraints)\b/i,
    severity: 'critical',
    action: 'terminate',
  ...
  },
  {
    ...
    pattern: /\b(pretend|act|behave|respond)\s+(like\s+|as\s+if\s+)?(you\s+)?(are|were|being)\s+(a|an|the)\b/i,
    severity: 'high',
    action: 'block',
  ...
  },
  {
    ...
    pattern: /\b(show|reveal|display|output|print|tell)\s+(me\s+)?(your|the)\s+(system|original|initial)\s*(prompt|instructions|guidelines|rules)\b/i,
    severity: 'critical',
    action: 'terminate',
...
const PII_PATTERNS: SecurityPattern[] = [
  ...
  {
    name: 'pii-api-key',
    pattern: /\b(sk_live_|pk_live_|api[_-]?key|secret[_-]?key)\s*[:=]\s*['"]?[A-Za-z0-9_-]{20,}['"]?/i,
    severity: 'critical',
    action: 'block',
...
export class SecurityLayer implements GuardrailLayerHandler {
  ...
  private mapTypeToDetectionType(
    type: string
  ): SecurityDetection['type'] {
  ...
  private determineAction(detections: SecurityDetection[]): 'allow' | 'warn' | 'block' | 'halt' | 'terminate' {
  ...
  private countSeverities(detections: SecurityDetection[]): Record<string, number> {
    return detections.reduce((counts, d) => {
