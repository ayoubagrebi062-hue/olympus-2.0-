...
import type { AIProvider, AIProviderClient, AIModel, ProviderConfig, CompletionRequest, CompletionResponse } from './types';
...
export class ProviderManager {
  ...
  estimateBuildCost(tier: string, estimatedTokens: number): { model: AIModel; cost: number } {
    ...
    const cost = (inputTokens / 1000) * capabilities.costPer1kInput + (outputTokens / 1000) * capabilities.costPer1kOutput;
    return { model, cost };
  }
}
...
export function createProviderManager(options?: { primaryProvider?: AIProvider; fallbackProvider?: AIProvider }): ProviderManager {
