...
export class AIRouter {
  ...
  private initializeProviders(): void {
    ...
    const hasCloudProvider = process.env.ANTHROPIC_API_KEY || process.env.OPENAI_API_KEY || process.env.GROQ_API_KEY;
    if (!hasCloudProvider) {
    ...
    const activeProviders = Object.keys(this.providers).filter(k => this.providers[k as AIProviderType]);
    console.log(`[AIRouter] Active providers: ${activeProviders.join(', ') || 'NONE - CHECK API KEYS!'}`);
  }
  ...
  async route(context: RoutingContext): Promise<RoutingDecision> {
    ...
    if (primaryHealthy) {
      return {
        ...
        estimatedCost: this.estimateCost(mapping.primary.provider, mapping.primary.model, context.tokenEstimate),
    ...
    if (fallbackHealthy) {
      return {
        ...
        estimatedCost: this.estimateCost(mapping.fallback.provider, mapping.fallback.model, context.tokenEstimate),
    ...
    if (anyHealthy) {
      ...
      const emergencyModel = anyHealthy.type === AIProviderType.OLLAMA
  ...
  async execute(
    ...
    console.log(`[AIRouter] Checking fallback: enabled=${this.config.fallbackEnabled}, fallbackProvider=${routing.fallbackProvider}`);
    if (this.config.fallbackEnabled && routing.fallbackProvider) {
      const fallbackProvider = this.providers[routing.fallbackProvider];
      console.log(`[AIRouter] Fallback provider found: ${!!fallbackProvider}, providers keys: ${Object.keys(this.providers).join(', ')}`);

      if (fallbackProvider) {
        attempts++;
        providersUsed.push(routing.fallbackProvider);
        console.log(`[AIRouter] Attempting fallback to ${routing.fallbackProvider} with model ${routing.fallbackModel}`);
        ...
        try {
          ...
          console.log(`[AIRouter] Fallback ${routing.fallbackProvider} succeeded! Tokens: ${response.usage.totalTokens}`);
