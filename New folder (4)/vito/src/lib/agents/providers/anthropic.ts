...
import type { AIProviderClient, CompletionRequest, CompletionResponse, ProviderConfig, TokenUsage, AnthropicModel } from './types';
...
export class AnthropicClient implements AIProviderClient {
  ...
  async complete(request: CompletionRequest): Promise<CompletionResponse> {
    ...
    const content: string = response.content.map((block) => (block.type === 'text' ? block.text : '')).join('');
    ...
    return {
      ...
      model,
      usage,
      finishReason: response.stop_reason === 'end_turn' ? 'stop' : response.stop_reason === 'max_tokens' ? 'max_tokens' : 'stop',
...
export class AnthropicProvider implements AIProviderInterface {
  ...
  async complete(request: AIRequest): Promise<AIResponse> {
    ...
    // Model-aware max tokens (Haiku: 4096, Sonnet/Opus: 8192)
    const isHaiku = model.includes('haiku');
    const maxTokensLimit = isHaiku ? 4096 : 8192;
    ...
    try {
      ...
      console.log(`[Anthropic] Response complete: stop_reason=${response.stop_reason}, model=${response.model}, output_tokens=${response.usage.output_tokens}, max_tokens=${maxTokens}`);
      if (response.stop_reason === 'max_tokens') {
        console.warn(`[Anthropic] WARNING: Response was truncated due to max_tokens limit (${maxTokens}). Content length: ${content.length}`);
      }
