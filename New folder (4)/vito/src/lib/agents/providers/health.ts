...
export class HealthMonitor {
  ...
  getProviderHealth(type: AIProviderType): ProviderHealth {
    ...
    const avgLatency = recentHistory.reduce((sum, h) => sum + h.latencyMs, 0) / recentHistory.length;
  ...
  getBestProvider(preferLocal: boolean = true): AIProviderType | null {
    ...
    const sorted = health
      ...
      .sort((a, b) => {
        ...
        if (preferLocal) {
          const aLocal = a.provider === AIProviderType.OLLAMA || a.provider === AIProviderType.LM_STUDIO;
          const bLocal = b.provider === AIProviderType.OLLAMA || b.provider === AIProviderType.LM_STUDIO;
  ...
  printReport(): void {
    ...
    for (const h of health) {
      ...
      console.log(`║ ${statusIcon} ${h.provider.padEnd(12)} │ ${h.status.padEnd(10)} │ ${String(h.latencyMs).padEnd(4)}ms │ ${(h.successRate * 100).toFixed(0).padStart(3)}% ║`);
    }
