...
import type { BuildPlan, BuildProgress, OrchestrationEvent, OrchestrationOptions } from '../orchestrator/types';
...
export const buildService = {
  ...
  async create(params: CreateBuildParams): Promise<BuildServiceResponse<{ buildId: string; plan: BuildPlan; conductor?: boolean }>> {
    ...
    if (existing?.length) {
      return { success: false, error: { code: 'BUILD_IN_PROGRESS', message: 'A build is already in progress for this project' } };
    }
      ...
      try {
        ...
        if (routing.useConductor) {
          ...
          const plan: BuildPlan = {
            ...
            tier: conductorTier,
            phases: analysis.features.length > 0
              ? [
                  { phase: 'discovery', name: 'discovery', agents: [], parallel: true, optional: false, estimatedTokens: Math.floor(analysis.estimatedTokens / 8) },
                  { phase: 'design', name: 'design', agents: [], parallel: true, optional: false, estimatedTokens: Math.floor(analysis.estimatedTokens / 8) },
                  { phase: 'architecture', name: 'architecture', agents: [], parallel: true, optional: false, estimatedTokens: Math.floor(analysis.estimatedTokens / 8) },
                  { phase: 'frontend', name: 'frontend', agents: [], parallel: true, optional: false, estimatedTokens: Math.floor(analysis.estimatedTokens / 4) },
                  { phase: 'backend', name: 'backend', agents: [], parallel: true, optional: false, estimatedTokens: Math.floor(analysis.estimatedTokens / 4) },
                  { phase: 'integration', name: 'integration', agents: [], parallel: true, optional: false, estimatedTokens: Math.floor(analysis.estimatedTokens / 8) },
                  { phase: 'testing', name: 'testing', agents: [], parallel: true, optional: false, estimatedTokens: Math.floor(analysis.estimatedTokens / 10) },
                  { phase: 'deployment', name: 'deployment', agents: [], parallel: true, optional: false, estimatedTokens: Math.floor(analysis.estimatedTokens / 10) },
                ]
          ...
          const { data: buildData, error } = await supabase
            ...
            .insert({
              ...
              target_users: params.targetUsers,
              tech_constraints: params.techConstraints,
              business_requirements: params.businessRequirements,
              design_preferences: params.designPreferences,
          ...
          if (error || !build) {
            console.warn(`[BuildService] CONDUCTOR DB insert failed, falling back to standard:`, error);
            // Fall through to standard path
          } else {
            plan.buildId = build.id;
            conductorManagedBuilds.add(build.id);

            console.log(`[BuildService] CONDUCTOR build created: ${build.id} (type: ${analysis.type}, complexity: ${analysis.complexity})`);

            return { success: true, data: { buildId: build.id, plan, conductor: true } };
          }
        } else {
          console.log(`[BuildService] CONDUCTOR routing suggests standard orchestrator: ${routing.reason}`);
        }
      } catch (conductorError) {
        console.warn(`[BuildService] CONDUCTOR analysis failed, falling back to standard:`, conductorError);
        // Fall through to standard path
      }
    }
    ...
    const build = buildData as { id: string } | null;
    if (error || !build) {
      return { success: false, error: { code: 'CREATE_FAILED', message: error?.message || 'Failed to create build' } };
    }
  ...
  async start(buildId: string, options?: OrchestrationOptions): Promise<BuildServiceResponse<BuildProgress>> {
    const supabase = createServiceRoleClient();
    ...
    if (build.status !== 'queued' && build.status !== 'paused') {
      return { success: false, error: { code: 'INVALID_STATUS', message: `Cannot start build with status: ${build.status}` } };
    }

    // Check if this is a CONDUCTOR-managed build
    const isConductorBuild = (build as any).config?.conductor === true || conductorManagedBuilds.has(buildId);
    ...
    if (isConductorBuild) {
      ...
      try {
        ...
        const result = await conductorService.startBuild({
          ...
          tier: conductorTier,
          options: (build as any).config?.conductorOptions,
        });
        ...
        console.warn(`[BuildService] CONDUCTOR start failed, falling back to standard:`, conductorError);
    ...
    // Update status
    await (supabase.from('builds') as any).update({ status: 'running', started_at: new Date().toISOString() }).eq('id', buildId);
    ...
    orchestrator.start().then(async (result) => {
      ...
      (supabase.from('builds') as any).update({
        status: 'failed',
  ...
  async getProgress(buildId: string): Promise<BuildServiceResponse<BuildProgress>> {
    ...
    const supabase = createServiceRoleClient();
    const { data: buildData, error } = await supabase.from('builds').select('*').eq('id', buildId).single();
    const build = buildData as Build | null;
  ...
  async pause(buildId: string): Promise<BuildServiceResponse<void>> {
    const orchestrator = activeOrchestrators.get(buildId);
    if (!orchestrator) {
      return { success: false, error: { code: 'NOT_ACTIVE', message: 'Build is not actively running' } };
    }
  ...
  async cancel(buildId: string): Promise<BuildServiceResponse<void>> {
    ...
    const supabase = createServiceRoleClient();
    await (supabase.from('builds') as any).update({ status: 'canceled', completed_at: new Date().toISOString() }).eq('id', buildId);
  ...
  async resumeFromCheckpoint(buildId: string, checkpointId?: string): Promise<BuildServiceResponse<BuildProgress>> {
    if (!conductorManagedBuilds.has(buildId)) {
      return {
        success: false,
