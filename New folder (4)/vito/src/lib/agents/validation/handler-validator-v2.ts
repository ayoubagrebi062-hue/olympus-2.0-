...
const FAKE_PATTERNS: FakePattern[] = [
  ...
  {
    regex: /^\s*\([\w\s,]*\)\s*=>\s*\{?\s*console\.(log|info|warn|debug|error)\([^)]*\)\s*;?\s*\}?\s*$/,
    reason: 'Only console.log - no real operation',
    severity: 'critical',
    suggestedFix: 'Replace with toast notification: () => { toast("Action completed"); actualAction(); }',
  },
  ...
  // CRITICAL: Alert only
  {
    ...
    severity: 'critical',
    suggestedFix: 'Replace with toast/modal: () => { showModal({ title: "Confirm", onConfirm: actualAction }); }',
  },
...
const REAL_PATTERNS: RealPattern[] = [
  ...
  { regex: /useReducer/, category: 'reducer', confidence: 0.90 },

  // Custom handlers (HIGH confidence)
  { regex: /handle[A-Z]\w*\(/, category: 'custom-handler', confidence: 0.90 },
  ...
  { regex: /api\.\w+\(/, category: 'api-call', confidence: 0.90 },
  { regex: /\.post\(|\.get\(|\.put\(|\.delete\(|\.patch\(/, category: 'http-method', confidence: 0.90 },

  // Navigation (HIGH confidence)
  { regex: /router\.(push|replace|back|forward)/, category: 'next-router', confidence: 0.95 },
  { regex: /navigate\(/, category: 'navigation', confidence: 0.90 },
  { regex: /redirect\(/, category: 'redirect', confidence: 0.90 },
  ...
  { regex: /setValue\(/, category: 'form-setValue', confidence: 0.90 },
  { regex: /register\(/, category: 'form-register', confidence: 0.80 },

  // Modal/Dialog (HIGH confidence)
  { regex: /setOpen\(|setIsOpen\(|setShow\(|setVisible\(/, category: 'modal-control', confidence: 0.95 },
  { regex: /onOpenChange\(/, category: 'modal-callback', confidence: 0.90 },
  { regex: /closeModal\(|openModal\(/, category: 'modal-function', confidence: 0.90 },
  ...
  { regex: /\.remove\(|\.delete\(|\.update\(|\.create\(|\.add\(|\.insert\(/, category: 'crud-operation', confidence: 0.90 },

  // Event handling (MEDIUM confidence - might be part of larger handler)
  { regex: /e\.preventDefault\(\)|event\.preventDefault\(\)/, category: 'prevent-default', confidence: 0.70 },
  { regex: /e\.stopPropagation\(\)|event\.stopPropagation\(\)/, category: 'stop-propagation', confidence: 0.70 },

  // Notifications (HIGH confidence)
  { regex: /toast(\.\w+)?\(/, category: 'toast-notification', confidence: 0.95 },
  { regex: /notify\(/, category: 'notification', confidence: 0.90 },
  { regex: /showNotification\(|showToast\(/, category: 'notification-function', confidence: 0.90 },

  // Async indicators (HIGH confidence)
  { regex: /await\s+/, category: 'async-await', confidence: 0.90 },
  ...
  { regex: /onComplete\(|onSuccess\(|onError\(/, category: 'lifecycle-callback', confidence: 0.90 },

  // File operations (HIGH confidence)
  { regex: /FileReader|readAsDataURL|readAsText/, category: 'file-read', confidence: 0.90 },
  { regex: /URL\.createObjectURL/, category: 'blob-url', confidence: 0.90 },
  ...
  { regex: /localStorage\.\w+\(|sessionStorage\.\w+\(/, category: 'storage', confidence: 0.80 },
];
...
function analyzeHandlerV2(handler: ExtractedHandler): HandlerAnalysisV2 {
  ...
  if (trimmedBody.length > 50 || semicolonCount >= 2 || lineCount >= 3) {
    return {
      ...
      lineNumber,
      isReal: true,
      confidence: 0.60, // Lower confidence - no recognized pattern
      severity: 'low',
...
export function validateHandlersV2(code: string, options?: {
  useCache?: boolean;
  skipMetrics?: boolean;
  ...
  const result: HandlerValidationResultV2 = {
    ...
    metrics: {
      ...
      handlersPerSecond: totalHandlers > 0
        ? Math.round((totalHandlers / totalTimeMs) * 1000)
        : 0,
      cacheHit: false,
      contentHash,
    },
...
export function generateHandlerReportV2(result: HandlerValidationResultV2): string {
  ...
  if (nonCritical.length > 0) {
    ...
    for (const issue of nonCritical) {
      lines.push(`- **Line ${issue.lineNumber} - ${issue.location}** [${issue.severity.toUpperCase()}]`);
