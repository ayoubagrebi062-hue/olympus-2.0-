...
export const COMPLEXITY_THRESHOLDS: Record<string, ComplexityThreshold> = {
  ...
  dashboard: {
    ...
    description: 'Dashboard pages need stat cards, charts, tables, and multiple interactions per PIXEL rules',
  },
...
export function inferPageType(filePath: string, code: string): string {
  ...
  if (content.includes('dashboard') || content.includes('statcard') || content.includes('metrics')) return 'dashboard';
  if (content.includes('datatable') || content.includes('<table') || content.includes('.map(')) return 'list';
  if (content.includes('useform') || content.includes('onsubmit')) return 'form';
  if (content.includes('signin') || content.includes('signup') || content.includes('password')) return 'auth';
...
export function validateComplexity(
  ...
  const hasCriticalViolation = violations.some(v =>
    v.metric === 'lineCount' || v.metric === 'handlerCount' || v.metric === 'stateHookCount'
  );
