...
export function hardenedValidateComplexity(
  ...
  try {
    ...
    const result = validateComplexityV2(codeStr, filePath || config.filePath, config.pageType || undefined);
...
export async function hardenedValidateCode(
  ...
  try {
    ...
    if (error.message === 'TIMEOUT') {
      return {
        success: false,
        error: { code: 'TIMEOUT', message: `Validation exceeded ${config.maxExecutionTime}ms limit` },
        metrics: {
          inputSize,
          executionTimeMs: safeNow() - startTime,
          rateLimitRemaining: config.maxRequestsPerSecond,
...
export async function healthCheck(): Promise<{
  ...
  try {
    ...
    validators.push({ name: 'handlers', status: result.success ? 'ok' : 'error', message: result.error?.message });
  } catch (error: any) {
  ...
  try {
    ...
    validators.push({ name: 'complexity', status: result.success ? 'ok' : 'error', message: result.error?.message });
  } catch (error: any) {
  ...
  try {
    ...
    validators.push({ name: 'unified', status: result.success ? 'ok' : 'error', message: result.error?.message });
  } catch (error: any) {
