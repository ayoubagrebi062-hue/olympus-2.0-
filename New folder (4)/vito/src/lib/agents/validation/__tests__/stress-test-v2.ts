...
import { validateHandlersV2, generateHandlerReportV2, clearValidationCache } from '../handler-validator-v2';
...
async function runStressTestV2() {
  ...
  for (const testCase of testCases) {
    ...
    console.log('');
    console.log(`  OVERALL: Grade ${result.grade} | Score ${result.score}% | ${result.valid ? '✅ PASS' : '❌ FAIL'}`);
    ...
    // Handler details
    if (result.handlers) {
      console.log(`  HANDLERS: ${result.handlers.valid ? '✅' : '❌'} Score ${result.handlers.score}%`);
      console.log(`    Real: ${result.handlers.realHandlers} | Fake: ${result.handlers.fakeHandlers}`);
      if (result.handlers.criticalIssues.length > 0) {
        console.log(`    Critical: ${result.handlers.criticalIssues.map(i => i.reason).join(', ')}`);
      }
    }

    // Complexity details
    if (result.complexity) {
      console.log(`  COMPLEXITY: ${result.complexity.valid ? '✅' : '❌'} Score ${result.complexity.score}%`);
      console.log(`    Lines: ${result.complexity.metrics.lineCount} | CC: ${result.complexity.metrics.cyclomaticComplexity}`);
      if (result.complexity.criticalViolations.length > 0) {
        console.log(`    Critical: ${result.complexity.criticalViolations.map(v => v.metric).join(', ')}`);
      }
    }

    // Features
    if (result.features) {
      console.log(`  FEATURES: ${result.features.valid ? '✅' : '❌'} Score ${result.features.score}%`);
  ...
  console.log('='.repeat(80));
  console.log('');
  console.log('| Test Case   | Lines | Grade | Score | Conf | Critical | Handler | Complex | Time   | Pass |');
  console.log('|-------------|-------|-------|-------|------|----------|---------|---------|--------|------|');
  ...
  const cachedResult = await validateCodeV2(PRODUCTION_CODE, { userPrompt: USER_PROMPT, enableCache: true });
  ...
  console.log('='.repeat(80));
  console.log('');
  console.log(`  Does STUB output get REJECTED?      ${results[0].valid ? '❌ NO (bad)' : '✅ YES (correct)'}`);
  console.log(`  Does MINIMAL output get REJECTED?   ${results[1].valid ? '❌ NO (bad)' : '✅ YES (correct)'}`);
  console.log(`  Does PRODUCTION output get ACCEPTED? ${results[2].valid ? '✅ YES (correct)' : '❌ NO (bad)'}`);
  console.log('');
