...
export function generateAgentRoadmap(weaknesses: AgentWeakness[]): void {
  ...
  const totalEffort = weaknesses.reduce((sum, w) => {
    switch (w.estimatedEffort) {
      case 'hours': return sum + 0.25;
      case 'day': return sum + 1;
      case 'days': return sum + 3;
      case 'week': return sum + 5;
...
function main() {
  ...
  // Output results
  console.log(`Found ${weaknesses.length} agent weaknesses across ${new Set(weaknesses.map(w => w.agent)).size} agents`);
  ...
  const sortedAgentCounts = Array.from(agentCounts.entries())
    .sort((a, b) => b[1] - a[1]);
