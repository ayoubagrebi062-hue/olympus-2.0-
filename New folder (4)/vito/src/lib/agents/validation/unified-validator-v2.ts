...
import { validateFeatures, extractRequiredFeatures, FeatureValidationResult, FeatureRequirement } from './feature-validator';
import { validateDesignTokens, DesignTokenValidationResult } from './design-validator';
...
export interface UnifiedValidationResult {
  ...
  design: DesignTokenValidationResult | null;
...
function aggregateIssues(
  ...
  if (design && !design.valid) {
    if (design.hardcodedColors > 0) {
      issues.push({
        validator: 'design',
        severity: 'medium',
        message: `${design.hardcodedColors} hardcoded colors found`,
        suggestedFix: 'Replace with design tokens: bg-background, text-foreground, etc.',
      });
    }
    if (design.hardcodedSpacing > 0) {
      issues.push({
        validator: 'design',
        severity: 'low',
        message: `${design.hardcodedSpacing} hardcoded spacing values found`,
...
export async function validateCodeV2(
  ...
  let design: DesignTokenValidationResult | null = null;
