  ...
  dashboard: {
    ...
    requirements: [
      ...
      },
      {
        ...
        weight: 8,
        severity: 'high',
        suggestedFix: 'Use Card, Button, Table, Chart, Badge, Avatar, DropdownMenu, Input components',
      },
      {
 ...
 */
function calculateFunctionMetrics(code: string): { count: number; maxLength: number; avgLength: number } {
...
export function calculateComplexityV2(code: string): ComplexityMetricsV2 {
  ...
  return {
    ...
    uniqueComponentsUsed: new Set(
      (code.match(/<([A-Z][a-zA-Z0-9]*)/g) || []).map(m => m.slice(1))
    ).size,
    propsCount,
    ...
    loopCount: (code.match(/\.map\(|\.forEach\(|\.filter\(|\.reduce\(|for\s*\(|while\s*\(/g) || []).length,
    functionCount: functionMetrics.count,

    // Quality
    commentLineCount: commentLines.length,
    commentRatio: codeLines.length > 0 ? Math.round((commentLines.length / codeLines.length) * 100) / 100 : 0,
...
export function inferPageTypeV2(filePath: string, code: string): string {
  ...
  if (path.includes('login') || path.includes('signup') || path.includes('register') || path.includes('auth')) return 'auth';
  if (path.includes('form') || path.includes('create') || path.includes('new') || path.includes('edit')) return 'form';
  ...
  if (content.includes('dashboard') || content.includes('statcard') || content.includes('chart') || content.includes('metric')) return 'dashboard';
  if (content.includes('datatable') || content.includes('<table') || content.includes('pagination')) return 'list';
  if (content.includes('useform') || content.includes('onsubmit') || content.includes('form-control')) return 'form';
  if (content.includes('signin') || content.includes('signup') || content.includes('password') || content.includes('credentials')) return 'auth';
  if (content.includes('loading') && content.includes('notfound')) return 'detail';

  // Fallback: location-based
  if (!path.includes('/app/') && !path.includes('/pages/') && !path.includes('/routes/')) return 'component';
...
export function validateComplexityV2(
  ...
  const rawScore = threshold.requirements.length > 0
    ? Math.round(((threshold.requirements.length - violations.length) / threshold.requirements.length) * 100)
    : 100;
