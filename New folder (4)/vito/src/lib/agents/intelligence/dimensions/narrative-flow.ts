...
const GOOD_TRANSITIONS: Transition[] = [
  ...
  },
  {
    ...
    to: 'solution',
    patterns: [
      /(introducing|meet|here's|presenting|discover)/i,
      /(with \w+, you|using \w+, you)/i,
    ],
    quality: 'smooth',
  },
  {
...
export function analyzeNarrativeFlow(content: string): DimensionScore {
  ...
  const missingRequired = requiredBeats.filter(
    b => !detectedBeats.some(d => d.beat === b.name)
  );
  ...
  if (!orderAnalysis.isCorrect) {
    ...
    issues.push({
      ...
      impact: 'Readers expect: Hook → Problem → Bridge → Solution → CTA. Breaking this confuses them.',
    });
    suggestions.push({
      type: 'restructure',
  ...
  const presentOptional = optionalBeats.filter(
    b => detectedBeats.some(d => d.beat === b.name)
  );
  ...
  if (detectedBeats.length > 0 && detectedBeats[0].beat === 'hook' && detectedBeats[0].strength > 0.8) {
    score += 5;
  ...
  const confidence = Math.min(0.95, 0.5 + (sentences.length / 50) * 0.3 + (detectedBeats.length / STORY_BEATS.length) * 0.2);
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    for (const beat of STORY_BEATS) {
      ...
      if (patternMatches > 0 && antiPatternMatches === 0) {
        const strength = Math.min(1, (patternMatches / beat.patterns.length) * (positionMatch ? 1.2 : 0.7));
...
function analyzeOrder(beats: DetectedBeat[]): {
  ...
  const idealOrder = ['hook', 'problem', 'agitation', 'bridge', 'solution', 'proof', 'urgency', 'call_to_action'];
...
function analyzeTransitions(content: string, beats: DetectedBeat[]): {
  ...
  for (let i = 0; i < beats.length - 1; i++) {
    ...
    if (transition) {
      const hasGoodTransition = transition.patterns.some(p => p.test(betweenText) || p.test(to.text));
...
function getTemplateForBeat(beat: string): string {
  const templates: Record<string, string> = {
    ...
    problem: '"You\'ve tried [solution attempts]. Nothing seems to work. Every time you [action], you end up [negative outcome]..."',
