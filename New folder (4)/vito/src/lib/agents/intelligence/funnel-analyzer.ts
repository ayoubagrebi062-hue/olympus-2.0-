...
export class FunnelAnalyzer {
   ...
   */
  async analyzeFunnel(pages: { name: string; content: string; stage: FunnelStage }[]): Promise<FunnelAnalysis> {
    // Input validation
  ...
  private analyzeStages(pages: FunnelPage[]): Record<FunnelStage, StageAnalysis> {
    const stages: FunnelStage[] = ['awareness', 'interest', 'consideration', 'intent', 'purchase', 'retention', 'advocacy'];
    ...
    for (const stage of stages) {
      ...
      const avgScore = stagePages.reduce((sum, p) => sum + p.analysis.totalScore, 0) / stagePages.length;
  ...
  private isPromiseDelivered(promise: string, content: string): boolean {
    ...
    const keyWords = promise.toLowerCase().split(/\s+/).filter(w =>
      w.length > 4 && !['will', 'would', 'could', 'should', 'receive', 'provide'].includes(w)
    );
  ...
  private detectDisconnect(
    ...
    const hasTransition = /(continue|next|now that|let's|ready to)/i.test(next.content.slice(0, 200));
  ...
  private detectTone(content: string): 'formal' | 'casual' | 'neutral' {
    const formalIndicators = (content.match(/\b(therefore|furthermore|moreover|consequently|hereby)\b/gi) || []).length;
    const casualIndicators = (content.match(/\b(hey|awesome|cool|yeah|gonna|wanna|super|totally)\b/gi) || []).length;
