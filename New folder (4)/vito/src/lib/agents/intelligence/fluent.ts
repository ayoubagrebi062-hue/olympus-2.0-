...
function generateHumanReport(analysis: ContentAnalysis): HumanReport {
  ...
  const topPriority = topImprovement ? {
    ...
    impact: `+${topImprovement.predictedLift}% conversion lift`,
  } : null;
...
function generateRecommendation(score: number, verdict: string): HumanReport['recommendation'] {
  if (score >= PUBLISH_THRESHOLD) {
    ...
    return {
      action: 'rewrite',
      reason: 'Consider starting fresh with a different angle. The core message may need rethinking.',
    };
  }
}
...
function generateExplanation(analysis: ContentAnalysis): Explanation {
  ...
  const summary = `Score: ${totalScore}/100 (${percentile}). ` +
...
function detectSecrets(content: string): string | null {
  for (const pattern of SECRET_PATTERNS) {
    if (pattern.test(content)) {
      return 'Content may contain sensitive data (API key, password, or token pattern detected). ' +
...
export class AnalysisBuilder {
  ...
  async run(): Promise<ContentAnalysis> {
    ...
    if (this.useCache) {
      ...
      if (cached) {
        this.emitProgress({ phase: 'complete', percentComplete: 100, elapsedMs: Date.now() - this.startTime });
    ...
    this.emitProgress({ phase: 'validating', percentComplete: 5, elapsedMs: Date.now() - this.startTime });

    const engine = new ConversionIntelligenceEngine();

    this.emitProgress({ phase: 'analyzing', percentComplete: 10, elapsedMs: Date.now() - this.startTime });
  ...
  private emitMetrics(result: ContentAnalysis, cacheHit: boolean): void {
    ...
    try {
      ...
      if (metricsResult && typeof metricsResult.catch === 'function') {
        metricsResult.catch(() => {
...
export async function compare(
  ...
  const dims1 = analysis1.dimensions as Record<string, DimensionScore>;
  const dims2 = analysis2.dimensions as Record<string, DimensionScore>;
  ...
  // Generate insight
  const insight = generateComparisonInsight(winner, scoreDiff, content1Strengths, content2Strengths);
...
function generateToWinRecommendations(
  ...
  for (const [dim, data1] of Object.entries(dims1)) {
    ...
    if (data2.score > data1.score + 10) {
      ...
      const improvement = analysis1.improvementPlan.quickWins.find(
        (qw) => qw.dimension === dim
      );
      if (improvement) {
...
export async function check(content: string): Promise<string> {
  ...
  const verdictLabel =
    ...
    verdict === 'good' ? 'Good' :
    verdict === 'needs_work' ? 'Needs Work' : 'Poor';

  // Get the #1 actionable suggestion
  const topSuggestion = result.improvementPlan.quickWins[0]?.suggestion.suggested
    || result.improvementPlan.structuralFixes[0]?.suggestion.suggested
    || 'Content looks solid!';
 ...
 */
export async function benchmark(content: string, industry: Industry = 'general'): Promise<BenchmarkResult> {
...
export async function showcase(content: string, industry: Industry = 'general'): Promise<ShowcaseResult> {
  const result = await analyze(content).run();
  ...
  const dimensions = result.dimensions as Record<string, DimensionScore>;
  const dimensionEntries = Object.entries(dimensions);
  const topDimension = dimensionEntries.reduce((best, [name, dim]) =>
    dim.score > (best[1]?.score || 0) ? [name, dim] : best
  , ['', { score: 0 }] as [string, { score: number }]);

  // Find top weakness (lowest dimension)
  const weakestDimension = dimensionEntries.reduce((worst, [name, dim]) =>
    dim.score < (worst[1]?.score || 100) ? [name, dim] : worst
  , ['', { score: 100 }] as [string, { score: number }]);

  // Get quick win
  const quickWin = result.improvementPlan.quickWins[0]?.suggestion.suggested
    || 'Content looks solid!';
...
export async function analyzeMany(
  ...
  const successful = items
    .filter((item): item is BatchItem & { result: ContentAnalysis } =>
      item.status === 'completed' && item.result !== undefined
    )
    .map(item => item.result);

  const failures = items
    .filter((item): item is BatchItem & { error: Error } =>
...
export async function rewrite(content: string): Promise<RewriteResult> {
  ...
  for (const improvement of quickWins) {
    ...
    if (suggestion.type === 'rewrite' && suggestion.original && suggestion.suggested) {
      ...
      const insertResult = smartInsert(improvedContent, suggestion.suggested, improvement.dimension);
  ...
  const percentageLift = originalScore > 0
    ? Math.round((pointsLift / originalScore) * 100)
    : 0;
...
function smartInsert(content: string, toInsert: string, dimension: string): { content: string; changed: boolean } {
...
function applyHeuristicImprovements(
  ...
  if (analysis.dimensions.ctaStrength?.score < REWRITE_THRESHOLDS.CTA_WEAK) {
    ...
    if (sentences.length >= 2 && sentences[sentences.length - 2]) {
      ...
      if (!lastSentence.toLowerCase().includes('now') &&
...
function generateDiff(original: string, improved: string, changes: RewriteResult['changes'][]): string {
 ...
 * }
 */
export async function* stream(content: string): AsyncGenerator<StreamEvent, ContentAnalysis, unknown> {
  const startTime = Date.now();
  const engine = new ConversionIntelligenceEngine();
