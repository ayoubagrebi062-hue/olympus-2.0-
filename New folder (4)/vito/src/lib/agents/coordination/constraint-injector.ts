  ...
  if (['pixel', 'wire'].includes(agentId) && agentOutputs?.strategos?.featureChecklist) {
    ...
    if (checklist.critical && checklist.critical.length > 0) {
      ...
      for (const feature of checklist.critical) {
        ...
        if (!feature.assignedTo || feature.assignedTo === agentId || feature.assignedTo === 'pixel') {
...
export function validateAgainstConstraints(
    ...
    for (const file of files) {
      ...
      if (filePath.includes('queries') || filePath.includes('repository')) {
        const hasQuery = content.includes('findMany') || content.includes('findFirst') ||
                         content.includes('SELECT') || content.includes('query(');
        const hasTenantFilter = content.includes('tenantId') || content.includes('tenant_id') ||
  ...
  if (agentId === 'engine') {
    ...
    for (const service of services) {
      ...
      if (decisions.architecture.isMultiTenant) {
        const hasTenantContext = service.tenantAware === true ||
  ...
  if (agentId === 'gateway') {
    ...
    for (const route of routes) {
      ...
      if (!isPublic && !hasAuth && path && !path.includes('/public/') && !path.includes('/health')) {
        violations.push({
          agentId,
  ...
  if (agentId === 'cartographer' && decisions.architecture.isMultiTenant) {
    ...
    for (const page of pages) {
      ...
      if (dataSource && !dataSource.includes('tenant') &&
          !['login', 'signup', 'public', 'landing', 'pricing'].some(p => pageName.toLowerCase().includes(p))) {
        violations.push({
          agentId,
