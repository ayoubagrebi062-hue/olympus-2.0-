...
async function testImageFetch(): Promise<boolean> {
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      ...
      console.error(`❌ Image fetch failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    return false;
  }
}
...
async function runAllTests(): Promise<void> {
  ...
  console.log('╠════════════════════════════════════════════════════════════╣');
  console.log(`║ Overall: ${allPassed ? '✅ ALL TESTS PASSED' : '❌ SOME TESTS FAILED'}${' '.repeat(allPassed ? 20 : 18)}║`);
  console.log('╚════════════════════════════════════════════════════════════╝');
  console.log('');
