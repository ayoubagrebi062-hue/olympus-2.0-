...
import { LeonardoClient, getLeonardoClient, LEONARDO_MODELS, PRESET_STYLES } from '../../integrations/leonardo/client';
  ...
  if (!MOCK_MODE) {
    results.push(
      await runTest('Generate icon', async () => {
        const service = getImageService();
        const asset = await service.generateIcon(
          'A simple check mark icon, flat design',
          'modern'
        );
