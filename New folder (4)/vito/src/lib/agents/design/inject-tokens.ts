...
export function generateDesignContext(
  ...
  const detectedBrand = interpretation.matchedKeywords.length > 0
    ? interpretation.matchedKeywords[0]
    : 'olympus';
  ...
  if (sections.includes('components') && fullTokens) {
    ...
    Object.entries(tokens.components.button.sizes).forEach(([size, config]) => {
      parts.push(`  ${size}: height=${config.height}, padding=${config.padding}, fontSize=${config.fontSize}`);
    });
    parts.push('');
    parts.push('### Card');
 ...
 */
export function getTypographyForCSS(userRequest?: string): Record<string, {
  fontSize: string;
  fontWeight: number;
}> {
  const tokens = getDesignTokens(userRequest);
...
export function validateDesignCompliance(
  ...
  const allowedColors = new Set([
    ...
    '#ffffff', '#000000', // Common exceptions
  ].map(c => c.toLowerCase()));
