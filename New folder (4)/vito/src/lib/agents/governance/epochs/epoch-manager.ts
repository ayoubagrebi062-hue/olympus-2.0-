...
export interface IEpochManager {
  ...
  advancePhase(epochId: string, targetPhase: EpochPhase, operator: string): Promise<EpochTransition>;
...
export class EpochManager implements IEpochManager {
  ...
  async advancePhase(epochId: string, targetPhase: EpochPhase, operator: string): Promise<EpochTransition> {
