...
import { ILedgerStore, GovernanceLedgerEntry, LedgerHash, LedgerConsistencyCheck, BuildLevelLock } from './types';
...
export class PostgresLedgerStore implements ILedgerStore {
  ...
  async getEntries(buildId: string, limit: number = 100): Promise<GovernanceLedgerEntry[]> {
    const { data, error } = await supabase
      .from('governance_ledger')
      .select('id, build_id, agent_id, action_type, timestamp, ledger_hash, previous_hash, immutable')
      .eq('build_id', buildId)
    ...
    const entries = data.map(row => ({
      ...
      actionData: row.action_data as any,
  ...
  async verifyConsistency(): Promise<LedgerConsistencyCheck> {
    const { data, error } = await supabase
      .from('governance_ledger')
      .select('id, build_id, agent_id, action_type, timestamp, ledger_hash, previous_hash')
    ...
    for (let i = 1; i < entries.length; i++) {
      ...
      const expectedHash = this.computeHash(previousEntry, entry.previous_hash);

      if (entry.ledger_hash !== expectedHash) {
        return {
          isConsistent: false,
          totalEntries: entries.length,
          chainBrokenAt: i,
          expectedHash,
          actualHash: entry.ledger_hash
        };
      }
    }
  ...
  async unlockBuild(buildId: string, operator: string): Promise<boolean> {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        timestamp: new Date().toISOString(),
        ledger_hash: existingLock.ledgerHash || '',
        previous_hash: '',
        immutable: true
      })
      .select('id')
  ...
  async verifyAndAppend(entry: GovernanceLedgerEntry): Promise<{ success: boolean; reason?: string }> {
    const latestHash = await this.getLatestHash();
