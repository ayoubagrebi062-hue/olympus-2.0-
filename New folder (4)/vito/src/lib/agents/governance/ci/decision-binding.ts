...
export class DecisionBindingGate {
  ...
  constructor(decisionSchemaPath: string = 'contracts/decision-schema.json', phasePath: string = 'contracts/governance-phase.json') {
    this.decisionSchema = this.loadDecisionSchema(decisionSchemaPath);
    this.UUID_V4_REGEX = /["']?([0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})["']?/gi;
  ...
  private writeReport(report: EnforcementReport, domainCounts: { EXEC: number; NON_EXEC: number; GOVERNANCE: number }): void {
  ...
  private analyzeDestructionSemantics(content: string, domain: ExecutionDomain): DestructionSemantics | undefined {
    if (domain !== 'EXEC') {
      return undefined;
    }
  ...
  private analyzeFile(filePath: string): Violation[] {
    ...
    if (domain === 'NON_EXEC' && tier === 'tier3') {
      ...
      if (decisionIds.length === 0) {
        ...
        const blockReason = 'NON_EXEC domain: UI file with Tier 3-like keywords. Keywords refer to presentation logic, not destructive operations. Decision ID recommended but not required.';
    ...
    if (tier === 'tier2' || tier === 'tier3' || scope === 'critical') {
      if (decisionIds.length === 0) {
        const violationType = tier === 'tier3' ? 'tier3_without_decision_id' : 'tier2_without_decision_id';
        const blockReason = isCritical && !isLegacyCritical
   ...
   */
  private classifyTier(content: string, domain: ExecutionDomain): { tier: string; effectTrigger?: string } {
    const contentLower = content.toLowerCase();
    ...
    if (deleteWithoutWhereMatch) {
      return { tier: 'tier3', effectTrigger: `DELETE without WHERE: ${deleteWithoutWhereMatch[0]}` };
    }
    ...
    const dbDropMatch = content.match(/db\.drop\s*\(|\.drop\s*\(\s*\{[\s\S]*?type:\s*['"](table|database|collection)['"]/g);
    if (dbDropMatch) {
    ...
    if (
      ...
      (contentLower.includes('.insert(') || contentLower.includes('.update(') || contentLower.includes('.delete().eq(')) ||
    ...
    if (fs.existsSync(decisionLogPath)) {
      try {
        ...
        if (decisionLog.decisions && Array.isArray(decisionLog.decisions)) {
          return decisionLog.decisions.some((d: any) => 
  ...
  private printReport(report: EnforcementReport, domainCounts: { EXEC: number; NON_EXEC: number; GOVERNANCE: number }): void {
    ...
    console.log(`  GOVERNED:           ${this.isObservationOnlyPhase() ? 'WARNING (Observation Only)' : 'BLOCK'}`);
    console.log(`  LEGACY:             ${this.isObservationOnlyPhase() ? 'WARNING (Observation Only)' : 'BLOCK'}`);
    console.log(`  NON_EXEC (UI):       Cannot be Tier C (keywords refer to presentation, not destruction)`);
    ...
    if (report.violations.length > 0) {
      ...
      const errorViolations = report.violations.filter(v =>
        v.severity === 'error' && !v.description.includes('[PHASE A+ DESTRUCTION ANALYSIS]')
      );
      const warningViolations = report.violations.filter(v =>
        v.severity === 'warning' && !v.description.includes('[PHASE A+ DESTRUCTION ANALYSIS]')
      );
      ...
      if (phaseAPlusWarnings.length > 0) {
        ...
        console.log('Destruction analysis helps developers understand why their code is dangerous.');
        console.log('These are WARNINGS ONLY and will NOT block builds.\n');

        phaseAPlusWarnings.forEach(v => {
          const domainBadge = v.domain === 'EXEC' ? '[EXEC]' : v.domain === 'GOVERNANCE' ? '[META]' : '[UI]';
          ...
          if (v.destructionSemantics) {
            ...
            if (declared) {
              console.log(`    Declared: scope="${declared.scope}", reversibility="${declared.reversibility}"`);
            ...
            if (inferred) {
              console.log(`    Inferred: scope="${inferred.scope}" (confidence: ${inferred.confidence})`);
              console.log(`    Patterns: ${inferred.patterns.join(', ')}`);
              console.log(`    Scope Match: ${scopeMatch ? '✓ MATCH' : '✗ MISMATCH'}`);
            }
          }

          console.log(`    Warning: ${v.description.replace('[PHASE A+ DESTRUCTION ANALYSIS] ', '')}\n`);
        });
      }
      ...
      if (errorViolations.length > 0) {
        ...
        errorViolations.forEach(v => {
          const scopeIcon = v.scope === 'critical' ? '⚠ CRITICAL' : v.scope === 'governed' ? '🔒 GOVERNED' : '📋 LEGACY';
          const domainBadge = v.domain === 'EXEC' ? '[EXEC]' : v.domain === 'GOVERNANCE' ? '[META]' : '[UI]';
          const effectInfo = v.effectTrigger ? ` | Effect: ${v.effectTrigger}` : '';
