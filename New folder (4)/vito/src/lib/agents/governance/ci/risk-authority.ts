 ...
 */
export type InferredRole = 'Developer' | 'Tech Lead' | 'Architect' | 'Product' | 'Executive' | 'Unknown';
...
export class RiskAuthorityAnalyzer {
   ...
   */
  private readonly ROLE_PATTERNS: { pattern: RegExp; role: InferredRole; authority: AuthorityLevel }[] = [
    { pattern: /^(cto|ceo|vp|chief|director)/i, role: 'Executive', authority: AuthorityLevel.EXECUTIVE },
    { pattern: /(product|pm|product.?manager)/i, role: 'Product', authority: AuthorityLevel.PRODUCT },
    { pattern: /(architect|arch|principal|staff)/i, role: 'Architect', authority: AuthorityLevel.ARCH_REVIEW },
    { pattern: /(lead|senior|sr\.?|tech.?lead|team.?lead)/i, role: 'Tech Lead', authority: AuthorityLevel.TECH_LEAD },
    { pattern: /(dev|developer|engineer|swe|sde)/i, role: 'Developer', authority: AuthorityLevel.TEAM }
  ];
  ...
  constructor() {
    ...
    this.AUTHORITY_OUTPUT_PATH = path.join(process.cwd(), 'data', 'governance', 'risk-authority.json');
  }
    ...
    if (causality === Causality.ARCH_CONSTRAINT) {
      if (resistanceLevel === 'EXTREME' || riskLevel === 'critical') {
        return {
          ...
          reason: 'Architectural constraint with extreme resistance or critical risk requires executive decision'
        };
      }
      return {
  ...
  private detectMismatch(
    ...
    if (requiredAuthority === AuthorityLevel.UNKNOWN || acknowledgedAuthority === AuthorityLevel.UNKNOWN) {
  ...
  analyze(): AuthorityReport {
    ...
    for (const entry of causalities) {
      ...
      const { authority: requiredAuthority, reason: requiredAuthorityReason } = this.inferRequiredAuthority(
      ...
      const { role: inferredRole, authority: acknowledgedAuthority } = this.inferAcknowledgedAuthority(
  ...
  printTop5AuthorityGaps(report: AuthorityReport): void {
    ...
    const top5 = mismatches.length > 0 ? mismatches.slice(0, 5) : report.authorityAnalyses.slice(0, 5);
    ...
    top5.forEach((analysis) => {
      ...
      console.log(`   Acknowledged By: ${analysis.acknowledgedBy || 'None'} (${analysis.inferredRole})`);
