...
interface Violation {
  file: string;
  violationType: 'UNAUTHORIZED_WRITE' | 'UNAUTHORIZED_ENFORCEMENT' | 'READ_ONLY_VIOLATION' | 'FORBIDDEN_ACTION' | 'PARSE_ERROR';
  details: string;
  line?: number;
}
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          if (entry.name !== 'node_modules' && entry.name !== '.next' && entry.name !== 'generated-builds' && entry.name !== 'dist' && entry.name !== 'build' && entry.name !== '.git') {
