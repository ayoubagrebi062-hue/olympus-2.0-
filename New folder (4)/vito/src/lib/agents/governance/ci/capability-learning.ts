...
export class CapabilityLearningExtractor {
  ...
  extract(): CapabilityLearningArtifact {
    ...
    const crossCapabilityPatterns = this.detectCrossCapabilityPatterns(capabilityDecisions, signals);
  ...
  private decisionMatchesCapability(decision: DecisionSchema['decisions'][number], capabilityId: string): boolean {
    const decisionContent = JSON.stringify(decision);
    return decisionContent.includes(capabilityId);
  }

  private generateLearningSignals(capabilityDecisions: Map<string, Array<DecisionSchema['decisions'][number]>>): CapabilityLearningSignal[] {
    const signals: CapabilityLearningSignal[] = [];

    for (const [capabilityId, decisions] of capabilityDecisions.entries()) {
      const capability = this.capabilitySchema.capabilities?.current?.find(c => c.id === capabilityId);
    ...
    for (const decision of decisions) {
      if (decision.outcome?.result) {
        ...
        if (decision.outcome.result === 'failure') {
          repeatedFailures++;
          if (decision.outcome.unintendedConsequences && decision.outcome.unintendedConsequences.length > 0) {
  ...
  private detectPerformanceDegradation(decisions: Array<DecisionSchema['decisions'][number]>): boolean {
  ...
  private generateRecommendations(
    ...
    if (patterns.repeatedFailures >= 3) {
      recommendations.push({
        ...
        rationale: 'Pattern of repeated failures indicates potential design or implementation issues',
        suggestedActions: [
    ...
    if (patterns.frequentEscalations >= 5) {
      recommendations.push({
        ...
        rationale: 'High escalation rate suggests capability may be operating beyond designed authority',
        suggestedActions: [
    ...
    if (alertThresholds?.ethicalVetos && patterns.frequentEthicalVetos >= alertThresholds.ethicalVetos) {
      recommendations.push({
        type: 'investigation',
        priority: 'high',
    ...
    if (patterns.unexpectedBehaviors.length > 0) {
      recommendations.push({
        ...
        rationale: 'Unexpected behaviors may indicate unhandled edge cases or incomplete requirements',
        suggestedActions: [
  ...
  private generateRiskIndicators(
    ...
    if (patterns.frequentEthicalVetos > 0) {
      ...
      riskIndicators.push({
        type: 'ethical',
        severity: patterns.frequentEthicalVetos > threshold * 2 ? 'high' : patterns.frequentEthicalVetos > threshold ? 'medium' : 'low',
    ...
    if (patterns.frequentEscalations > 0) {
      ...
      riskIndicators.push({
        type: 'governance',
        severity: patterns.frequentEscalations > threshold * 2 ? 'high' : patterns.frequentEscalations > threshold ? 'medium' : 'low',
  ...
  private calculateConfidence(totalDecisions: number, outcomes: CapabilityLearningSignal['outcomes']): {
    confidence: 'high' | 'medium' | 'low';
    evidenceLevel: 'L1' | 'L2' | 'L3' | 'L4';
  } {
  ...
  private detectCrossCapabilityPatterns(
    ...
    const failureCapabilities = signals.filter(s => s.patterns.repeatedFailures >= 3).map(s => s.capabilityId);
    ...
    const escalationCapabilities = signals.filter(s => s.patterns.frequentEscalations >= 5).map(s => s.capabilityId);
    if (escalationCapabilities.length >= 2) {
      patterns.push({
        ...
        firstObserved: new Date().toISOString(),
        lastObserved: new Date().toISOString(),
        recommendation: 'Review if capabilities are operating beyond designed authority or if agent training is needed'
      });
    }

    const ethicalVetoCapabilities = signals.filter(s => s.patterns.frequentEthicalVetos >= 2).map(s => s.capabilityId);
  ...
  private generateSystemLevelInsights(
    ...
    const totalRecommendations = signals.reduce((sum, s) => sum + s.learningRecommendations.length, 0);
    ...
    const averageSuccessRate = signals.length > 0
      ? signals.reduce((sum, s) => sum + s.successRate, 0) / signals.length
      : 0;
  ...
  private calculateSummary(signals: CapabilityLearningSignal[]): CapabilityLearningArtifact['summary'] {
    ...
    if (artifact.signals.length > 0) {
      ...
      if (highPrioritySignals.length > 0) {
        ...
        for (const signal of highPrioritySignals.slice(0, 5)) {
          ...
          console.log(`  ${signal.capabilityName}: ${highRecs.length} high-priority recommendations`);
        }
      }
    }
