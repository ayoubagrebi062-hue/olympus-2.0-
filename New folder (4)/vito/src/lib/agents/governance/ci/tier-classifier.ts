...
export class TierClassifier {
  ...
  private detectBehaviors(content: string): DetectedBehaviors {
    ...
    const hasIrreversibility = this.matchesAnyPattern(content, this.config.irreversibility_patterns);
    const hasEthicalOversight = this.matchesAnyPattern(content, this.config.ethical_markers);
    const hasHumanAccountability = this.matchesAnyPattern(content, this.config.human_accountability_markers);
    const hasHumanOverrideRequired = this.matchesAnyPattern(content, this.config.human_override_markers);
