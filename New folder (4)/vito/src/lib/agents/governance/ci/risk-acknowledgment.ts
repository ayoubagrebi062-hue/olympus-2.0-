...
export class RiskAcknowledgmentObserver {
  ...
  private detectTrigger(pattern: DestructionPattern): AcknowledgmentTrigger | null {
    ...
    const triggerRules = [
      ...
      },
      {
        ...
        description: 'Global operations marked as reversible but patterns suggest difficult restoration'
      },
      {
    ...
    for (const trigger of triggers) {
      ...
      if (existingEntry) {
        merged.set(trigger.patternId, {
          ...
          unresolved: existingEntry.acknowledged === false || existingEntry.acknowledged === undefined
        });
      } else {
        merged.set(trigger.patternId, {
  ...
  private calculateSummary(acknowledgments: AcknowledgmentEntry[]): RiskAcknowledgmentReport['summary'] {
    ...
    const highestUnresolvedRisk = acknowledgments
      .filter(a => a.acknowledged !== true)
      .sort((a, b) => {
    ...
    const byRiskLevel = acknowledgments.reduce((acc, a) => {
      acc[a.riskLevel] = (acc[a.riskLevel] || 0) + 1;
      return acc;
    }, {} as Record<string, number>);
  ...
  printAcknowledgmentStatus(report: RiskAcknowledgmentReport): void {
    ...
    console.log(`  Highest Unresolved Risk:     ${report.summary.highestUnresolvedRisk.toUpperCase()}\n`);
    ...
    if (report.summary.totalRequiredAcknowledgments > 0) {
      ...
      unacknowledged.forEach((ack, index) => {
        const riskIcon = ack.riskLevel === 'critical' ? '🔴' : ack.riskLevel === 'high' ? '🟠' : '🟡';
