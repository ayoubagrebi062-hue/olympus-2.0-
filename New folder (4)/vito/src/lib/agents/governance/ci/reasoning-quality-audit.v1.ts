...
interface DecisionSchema {
  decisions: Array<{
    ...
    reasoningArtifacts?: {
      ...
      ethicalAssessment?: {
        present?: boolean;
        principlesEvaluated?: Record<string, {
          rating?: number;
          explanation?: string;
        }>;
...
export class ReasoningQualityAuditor {
  ...
  private scoreLogicalCoherence(decision: DecisionSchema['decisions'][number]): DimensionScore {
    ...
    const hasAssumptions = reasoningChain?.keyAssumptions && reasoningChain.keyAssumptions.length > 0;
    if (!hasAssumptions || !reasoningChain?.keyAssumptions || reasoningChain.keyAssumptions.length === 0) {
  ...
  private scoreUncertaintyHonesty(decision: DecisionSchema['decisions'][number]): DimensionScore {
    ...
    const hasUncertaintyInReasoning = reasoningChain?.steps?.some((s: any) =>
  ...
  private scoreEthicalDepth(decision: DecisionSchema['decisions'][number]): DimensionScore {
    ...
    if (ethicalAssessment?.stakeholderAnalysis && ethicalAssessment.stakeholderAnalysis.length > 0) {
      score += 5;
  ...
  private scoreActionJustification(decision: DecisionSchema['decisions'][number]): DimensionScore {
    ...
    if (actionRec?.unintendedConsequences && actionRec.unintendedConsequences.length > 0) {
      score += 5;
      factors.push(`${actionRec.unintendedConsequences.length} unintended consequence(s) considered`);
    }
  ...
  private detectFlags(
    ...
    if (decision.decisionClass === 'C' && !decision.reasoningArtifacts?.ethicalAssessment?.present) {
    ...
    if (!decision.reasoningArtifacts?.reasoningChain?.keyAssumptions || decision.reasoningArtifacts?.reasoningChain?.keyAssumptions?.length === 0) {
  ...
  private generateNotes(
    ...
    if (scores.actionJustificationQuality.score < 12 && decision.decisionClass !== 'A') {
      notes.push('Action justification could be strengthened with risk assessment and unintended consequence analysis');
    }

    return notes;
  }

  private calculateSummary(assessments: DecisionQualityAssessment[]): ReasoningQualityArtifact['summary'] {
  ...
  private detectHighRiskPatterns(assessments: DecisionQualityAssessment[]): QualityPattern[] {
    ...
    const unstatedAssumptions = assessments.filter(a =>
      a.flags.includes('UNSTATED_ASSUMPTIONS')
    );
  ...
  private analyzePatterns(assessments: DecisionQualityAssessment[]): ReasoningQualityArtifact['patterns'] {
  ...
  private extractEthicalShallowness(assessments: DecisionQualityAssessment[]): QualityPattern[] {
    const shallowCount = assessments.filter(a => {
      const hasShallowNote = a.notes.some(n =>
        n.toLowerCase().includes('ethical') && n.toLowerCase().includes('comprehensive')
      );
  ...
  private extractReasoningGaps(assessments: DecisionQualityAssessment[]): QualityPattern[] {
    const gapCount = assessments.filter(a =>
      a.flags.includes('LOGICAL_INCOHERENCE')
    ).length;
  ...
  private printSummary(artifact: ReasoningQualityArtifact): void {
    ...
    console.log('\nBy Decision Class:');
    console.log(`  Class A:      ${artifact.summary.byDecisionClass.A.count} decisions, avg: ${artifact.summary.byDecisionClass.A.averageScore.toFixed(1)}`);
    console.log(`  Class B:      ${artifact.summary.byDecisionClass.B.count} decisions, avg: ${artifact.summary.byDecisionClass.B.averageScore.toFixed(1)}`);
    console.log(`  Class C:      ${artifact.summary.byDecisionClass.C.count} decisions, avg: ${artifact.summary.byDecisionClass.C.averageScore.toFixed(1)}`);
    console.log(`  Unknown:      ${artifact.summary.byDecisionClass.UNKNOWN.count} decisions, avg: ${artifact.summary.byDecisionClass.UNKNOWN.averageScore.toFixed(1)}`);
    ...
    if (artifact.summary.highRiskPatterns.length > 0) {
      ...
      for (const pattern of artifact.summary.highRiskPatterns) {
        console.log(`  [${pattern.severity.toUpperCase()}] ${pattern.pattern}: ${pattern.description}`);
      }
    }
