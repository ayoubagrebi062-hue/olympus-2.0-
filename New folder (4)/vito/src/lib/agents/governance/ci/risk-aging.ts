...
export class RiskAgingAnalyzer {
  ...
  analyze(): RiskAgingReport {
    ...
    let highestRiskWithStaleness: { riskLevel: 'critical' | 'high' | 'medium' | 'low'; staleness: 'fresh' | 'aging' | 'stale' | 'fossil' } = {
      riskLevel: 'low',
      staleness: 'fresh'
    };
    ...
    for (const acknowledgment of acknowledgments) {
      ...
      if (agingInfo) {
        ...
        if (acknowledgment.acknowledged && this.isHigherRisk(acknowledgment.riskLevel, highestRiskLevel)) {
  ...
  writeAcknowledgments(report: RiskAgingReport): void {
    try {
      ...
      const updatedAcknowledgments = report.acknowledgments.map(ack => {
        const existing = existingData.acknowledgments.find((e: AcknowledgmentEntry) => e.patternId === ack.patternId);
        if (existing) {
          const preserved = {
            ...existing,
  ...
  printAgingStatus(report: RiskAgingReport): void {
    ...
    console.log(`  Oldest Acknowledgment:        ${report.summary.oldestAcknowledgmentAgeDays} days\n`);
    ...
    console.log(`Highest Risk + Staleness: ${report.summary.highestRiskWithStaleness.riskLevel.toUpperCase()} @ ${report.summary.highestRiskWithStaleness.staleness.toUpperCase()}\n`);
