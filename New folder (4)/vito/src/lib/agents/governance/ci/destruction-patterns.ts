...
export class DestructionPatternObserver {
    ...
    try {
      ...
      if (report.violations && Array.isArray(report.violations)) {
        for (const violation of report.violations) {
          if (violation.destructionSemantics && violation.destructionSemantics.warnings.length > 0) {
  ...
  private aggregatePatterns(warnings: DestructionWarning[], existingReport: DestructionPatternsReport | null): DestructionPatternsReport {
    ...
    const summary = {
      ...
      highRiskPatterns: patterns.filter(p =>
  ...
  printReport(report: DestructionPatternsReport): void {
    ...
    if (report.patterns.length > 0) {
      ...
      report.patterns.sort((a, b) => b.occurrenceCount - a.occurrenceCount).forEach((pattern, index) => {
        const riskIndicator = pattern.inferredScope === 'global' && pattern.scopeConsistency === 'always_mismatch' ? '⚠ HIGH RISK' : 'ℹ OBSERVED';
        const domainBadge = pattern.domain === 'EXEC' ? '[EXEC]' : pattern.domain === 'GOVERNANCE' ? '[META]' : '[UI]';
        
        console.log(`${index + 1}. ${riskIndicator} ${domainBadge} ${pattern.file}`);
        console.log(`   Occurrences: ${pattern.occurrenceCount} (First: ${pattern.firstSeen}, Last: ${pattern.lastSeen})`);
        console.log(`   Inferred Scope: ${pattern.inferredScope.toUpperCase()} | Declared Scope: ${pattern.declaredScope.toUpperCase()}`);
