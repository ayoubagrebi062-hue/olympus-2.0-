...
export class DeliberationAuditor {
  ...
  private analyzeFile(filePath: string): DeliberationFinding[] {
    ...
    findings.push(...this.checkReasoningArtifacts(filePath, decisionClass, reasoningArtifacts, lines));
    findings.push(...this.checkEthicalReasoning(filePath, decisionClass, reasoningArtifacts, lines));
    findings.push(...this.checkUncertaintyHandling(filePath, decisionClass, hasUncertainty, hasPrecaution, hasEscalation, lines));
  ...
  private detectUncertainty(content: string): boolean {
    return this.config.uncertaintyPatterns.some(pattern =>
      new RegExp(pattern, 'i').test(content)
    );
  }

  private detectPrecaution(content: string): boolean {
    return this.config.precautionPatterns.some(pattern =>
      new RegExp(pattern, 'i').test(content)
    );
  }

  private detectEscalation(content: string): boolean {
    return this.config.escalationPatterns.some(pattern =>
      new RegExp(pattern, 'i').test(content)
    );
  }
  ...
  private checkReasoningArtifacts(
    ...
    if (decisionClass === 'B' || decisionClass === 'C') {
      ...
      const missingArtifacts = requiredArtifacts.filter(artifact => !reasoningArtifacts.has(artifact));
      ...
      if (missingArtifacts.length > 0) {
        findings.push({
          ...
          recommendation: decisionClass === 'C'
  ...
  private checkEthicalReasoning(
    ...
    if (decisionClass === 'C' && !reasoningArtifacts.has('ethicalAssessment')) {
      findings.push({
        ...
        recommendation: 'Add Ethical Assessment artifact with stakeholder analysis, principle evaluation, and conflict resolution',
        context: {
  ...
  private createArtifact(findings: DeliberationFinding[], generatedAt: string): DeliberationAuditArtifact {
