...
export class TierEnforcer {
  ...
  private checkAmbiguity(analysis: FileAnalysis): Violation[] {
    ...
    const hasTier2Behavior = (behaviors.hasDbWrites || behaviors.hasEnforcement) && !behaviors.hasIrreversibility;
    const hasTier3Behavior = behaviors.hasIrreversibility;

    const behaviorCount = [hasTier1Behavior, hasTier2Behavior, hasTier3Behavior].filter(Boolean).length;
    ...
    if (behaviors.hasHumanOverrideRequired && !behaviors.hasIrreversibility) {
      violations.push({
        filePath,
        tier: analysis.detectedTier,
        violation: 'HUMAN_OVERRIDE_REQUIRED marker found without irreversible operations (Tier3 only)',
        severity: 'error'
      });
    }
