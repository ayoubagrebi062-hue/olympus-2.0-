...
import { TransactionContext, TransactionResult, TransactionOperation, VerificationEvent } from './types';
...
export class InMemoryTransactionStore implements ITransactionStore {
  ...
  async commitTransaction(context: TransactionContext): Promise<TransactionResult<void>> {
    ...
    console.log(`[TransactionStore] Transaction ${context.transactionId} committed (${duration}ms)`);
  ...
  async rollbackTransaction(context: TransactionContext): Promise<TransactionResult<void>> {
    ...
    console.log(`[TransactionStore] Transaction ${context.transactionId} rolled back (${duration}ms)`);
  ...
  async transaction<T>(fn: (context: TransactionContext) => Promise<T>): Promise<TransactionResult<T>> {
    ...
    try {
      ...
      console.error(`[TransactionStore] Transaction ${context.transactionId} failed: ${error.message}`);
