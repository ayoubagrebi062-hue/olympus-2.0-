...
import type { AgentRole, AgentIdentity } from '../types';
...
export class GovernanceControlPlane implements IControlPlane {
  ...
  private initializeAuthMatrix(): void {
    ...
    this.AUTH_MATRIX.set(AgentRole.MONITOR, [
      ControlAction.ESCALATE
    ]);
  }

  getCurrentState(): ControlState {
    return { ...this.state, pausedBuilds: new Set(this.state.pausedBuilds), lockedTenants: new Set(this.state.lockedTenants) };
  }

  async executeAction(action: ControlAction, target: string, operator: string): Promise<ControlDecision> {
    ...
    const authorized = await this.evaluateControl({ agentId: operator, version: '1.0.0', fingerprint: '', role: AgentRole.GOVERNANCE, tenantId: '', buildId: target } as AgentIdentity, action);
  ...
  async getControlHistory(target?: string, limit: number = 100): Promise<ControlEvent[]> {
    ...
    return allEvents.sort((a, b) => b.triggeredAt.getTime() - a.triggeredAt.getTime()).slice(0, limit);
  }
  ...
  private async applyAction(action: ControlAction, target: string, operator: string): Promise<boolean> {
    switch (action) {
      case ControlAction.HALT:
        this.state.halted = true;
        this.state.haltedAt = new Date();
