...
const SECURITY_PATTERNS: SecurityPattern[] = [
  ...
  // ===================
  {
    ...
    description: 'dangerouslySetInnerHTML can execute arbitrary HTML/JS. Ensure content is sanitized.',
...
export class SecurityScanner {
  ...
  constructor(options: ScanOptions = {}) {
    ...
    if (this.options.ignoredRules?.length) {
      this.patterns = this.patterns.filter(
        (p) => !this.options.ignoredRules!.includes(p.id)
      );
    }
  }
  ...
  getReport(result: ScanResult): string {
    ...
    if (result.vulnerabilities.length > 0) {
      ...
      for (const vuln of result.vulnerabilities) {
        ...
        lines.push(`   ID: ${vuln.id.split('-')[0]} | Line: ${vuln.line} | CWE: ${vuln.cwe || 'N/A'}`);
