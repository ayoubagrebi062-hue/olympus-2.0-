...
export function buildContextSummary(
  ...
  if (targetAgentDef?.phase === 'backend' || targetAgentDef?.phase === 'architecture') {
    ...
    if (knowledge.apiEndpoints?.length) {
      sections.push(`## API Endpoints\n${knowledge.apiEndpoints.slice(0, 20).map((e) => `- ${e}`).join('\n')}`);
    }
  }
...
function summarizePreviousOutputs(outputs: Record<AgentId, AgentOutput>, targetAgent: AgentId): string {
...
function summarizeAgentOutput(output: AgentOutput): string {
  ...
  if (output.decisions.length) {
    const decisionSummary = output.decisions.slice(0, 5).map((d) => `- ${d.type}: ${d.choice}`).join('\n');
  ...
  if (files.length) {
    parts.push(`Files: ${files.slice(0, 10).map((f) => f.path).join(', ')}`);
  }
...
function truncateToTokenLimit(text: string, maxTokens: number): string {
  ...
  const prioritySections = sections.filter(s =>
    PRIORITY_SECTIONS.some(p => s.includes(p))
  );
...
export function extractArtifactsForPurpose(artifacts: Artifact[], purpose: 'code' | 'design' | 'docs'): Artifact[] {
  switch (purpose) {
    case 'code':
