...
import type { BuildContextData, BuildKnowledge, BuildState, ContextSnapshot, ContextUpdateEvent, TechStackDecision } from './types';
...
export class BuildContextManager {
  ...
  /** Get build ID */
  get buildId(): string { return this.data.buildId; }

  /** Get current state */
  get state(): BuildState { return this.data.state; }

  /** Get current phase */
  get currentPhase(): BuildPhase | null { return this.data.currentPhase; }

  /** Get iteration number */
  get iteration(): number { return this.data.iteration; }

  /** Get accumulated knowledge */
  get knowledge(): BuildKnowledge { return this.data.knowledge; }

  /** 50X COORDINATION: Get critical decisions for downstream agents */
  get criticalDecisions(): CriticalDecisions | null { return this._criticalDecisions; }
  ...
  setState(state: BuildState): void {
    ...
    this.emit({ type: 'state_changed', buildId: this.buildId, timestamp: new Date(), data: { state } });
  }
  ...
  recordOutput(output: AgentOutput): void {
    ...
    this.emit({ type: 'agent_completed', buildId: this.buildId, agentId: output.agentId, timestamp: new Date() });
  }
  ...
  private updateKnowledge(output: AgentOutput): void {
    ...
    switch (output.agentId) {
      case 'oracle':
        this.data.knowledge.marketAnalysis = parsed.market_analysis ? JSON.stringify(parsed.market_analysis).substring(0, 500) : undefined;
        break;
      case 'empathy':
        this.data.knowledge.targetPersonas = parsed.personas?.map((p: any) => p.name || p.title) || [];
        break;
      case 'strategos':
        this.data.knowledge.coreFeatures = parsed.mvp_features?.map((f: any) => f.name || f.title) || [];
      ...
      case 'blocks':
        ...
        break;
      case 'cartographer':
        this.data.knowledge.pageStructure = parsed.pages?.reduce((acc: any, p: any) => ({ ...acc, [p.name]: p.sections || [] }), {});
        break;
      case 'archon':
      ...
      case 'datum':
        ...
        break;
      case 'nexus':
        this.data.knowledge.apiEndpoints = parsed.endpoints?.map((e: any) => `${e.method} ${e.path}`) || [];
        break;
      case 'sentinel':
    ...
    if (parsed.files?.length) {
      this.data.knowledge.generatedFiles = [...(this.data.knowledge.generatedFiles || []), ...parsed.files.map((f: any) => f.path)];
    }
  }
  ...
  private parseAgentArtifacts(output: AgentOutput): Record<string, any> {
    ...
    if (docArtifact) {
      try { return JSON.parse(docArtifact.content); } catch { return {}; }
    }
    return {};
  }
  ...
  private summarizeSchema(tables: any[]): string {
    ...
    return tables.map((t: any) => `${t.name}(${t.columns?.map((c: any) => c.name).join(', ') || ''})`).join('; ');
  }
  ...
  addFeedback(feedback: string, focusAreas?: string[]): void {
    ...
    this.emit({ type: 'feedback_added', buildId: this.buildId, timestamp: new Date(), data: { feedback } });
  }
  ...
  createSnapshot(): ContextSnapshot {
    ...
    snapshot.checksum = createHash('sha256').update(JSON.stringify(snapshot)).digest('hex').substring(0, 16);
    return snapshot;
  }
  ...
  /** Get all artifacts */
  getArtifacts(): Artifact[] { return Array.from(this.data.artifacts.values()); }

  /** Get all decisions */
  getDecisions(): Decision[] { return [...this.data.decisions]; }

  /** Get all agent outputs (for file writing) */
  getAgentOutputs(): Map<AgentId, AgentOutput> { return this.data.agentOutputs; }
