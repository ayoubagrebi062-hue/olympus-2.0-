...
It offers everything you need. It comes with a warranty.`,
      ...
      expect(result.scores.wiifm.suggestions.some(s =>
        s.toLowerCase().includes('benefit') || s.toLowerCase().includes('you get')
      )).toBe(true);
    });
...
You'll get amazing benefits from this product.`,
      ...
      const hasOpeningFeedback = result.scores.wiifm.issues.some(i =>
        i.toLowerCase().includes('first') || i.toLowerCase().includes('opening') || i.toLowerCase().includes('hook')
      ) || result.scores.wiifm.suggestions.some(s =>
        s.toLowerCase().includes('opening') || s.toLowerCase().includes('what\'s in it')
      );
