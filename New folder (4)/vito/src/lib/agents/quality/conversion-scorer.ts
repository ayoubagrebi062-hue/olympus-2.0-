...
const DEFAULT_WEIGHTS = {
  ...
  emotional: 0.20,       // 20%
  ctaStrength: 0.20,     // 20%
  objectionCoverage: 0.10, // 10%
  antiPlaceholder: 0.10,   // 10%
};
...
const POWER_WORDS = [
  // Urgency
  'now', 'today', 'immediately', 'instant', 'instantly', 'hurry', 'quick', 'fast', 'limited', 'deadline', 'expires', 'last chance',
  // Value
  'free', 'bonus', 'save', 'discount', 'exclusive', 'premium', 'luxury', 'valuable',
  // Trust
  'proven', 'guaranteed', 'certified', 'trusted', 'secure', 'safe', 'authentic', 'official',
  // Curiosity
  'secret', 'hidden', 'revealed', 'discover', 'unlock', 'inside', 'behind-the-scenes',
  // Results
  'results', 'success', 'breakthrough', 'transform', 'achieve', 'accomplish', 'win', 'boost',
  // Exclusivity
  'private', 'members-only', 'invitation', 'vip', 'elite', 'handpicked', 'select',
  // Fear/Loss
  'mistake', 'avoid', 'never', 'stop', 'warning', 'danger', 'risk', 'miss out', 'losing',
  // New
  'new', 'introducing', 'announcing', 'finally', 'revolutionary', 'breakthrough',
  // Easy
  'easy', 'simple', 'effortless', 'straightforward', 'step-by-step', 'foolproof',
];
...
const EMOTIONAL_TRIGGERS = [
  'fear', 'greed', 'guilt', 'exclusivity', 'salvation', 'curiosity', 'urgency', 'trust', 'belonging', 'pride',
];
...
const WEAK_CTA_WORDS = [
  'maybe', 'try', 'submit', 'click here', 'learn more', 'read more', 'see more', 'continue',
  'next', 'enter', 'send', 'go', 'okay', 'ok', 'proceed',
];
...
const STRONG_ACTION_VERBS = [
  'get', 'grab', 'claim', 'unlock', 'access', 'start', 'join', 'discover', 'download', 'reserve',
  'secure', 'activate', 'launch', 'begin', 'create', 'build', 'transform', 'boost', 'save', 'earn',
];
 ...
 */
const JARGON_WORDS = [
  'leverage', 'synergy', 'paradigm', 'ecosystem', 'scalable', 'robust', 'holistic', 'optimize',
  'streamline', 'innovative', 'cutting-edge', 'game-changing', 'revolutionary', 'disruptive',
  'best-in-class', 'world-class', 'state-of-the-art', 'next-generation', 'enterprise-grade',
  'mission-critical', 'actionable', 'bandwidth', 'circle back', 'deep dive', 'drill down',
];
...
const CORE_OBJECTIONS = {
  price: ['expensive', 'cost', 'afford', 'budget', 'money', 'investment', 'value', 'worth', 'price', 'pay'],
  time: ['time', 'busy', 'schedule', 'hours', 'minutes', 'quickly', 'fast', 'slow'],
  trust: ['trust', 'scam', 'legit', 'guarantee', 'refund', 'risk-free', 'proven', 'testimonial', 'review'],
  willItWork: ['work for me', 'my situation', 'different', 'unique', 'specific', 'case', 'circumstances'],
  triedBefore: ['tried', 'failed', 'before', 'again', 'different', 'this time', 'unlike', 'not like'],
};
...
function findComplexWords(text: string): string[] {
  ...
  const commonWords = new Set(['beautiful', 'important', 'different', 'possible', 'everything', 'something', 'another', 'because']);
...
function scoreWIIFM(content: ConversionContent): WIIFMScoreDetail {
  ...
  // Benefit keywords
  const benefitKeywords = ['you', 'your', 'get', 'receive', 'gain', 'achieve', 'enjoy', 'discover', 'learn', 'save', 'earn', 'transform', 'become'];
  const featureKeywords = ['it has', 'it includes', 'it features', 'it comes with', 'it offers', 'it provides', 'we have', 'we include', 'we offer'];
...
function scoreClarity(content: ConversionContent): ClarityScoreDetail {
  ...
  if (avgParagraphSentences > 4) {
    ...
    score -= penalty;
    issues.push(`Paragraphs too long: ${avgParagraphSentences.toFixed(1)} sentences avg (target: <4)`);
...
function scoreEmotional(content: ConversionContent): EmotionalScoreDetail {
  ...
  const emotionalTriggersUsed = EMOTIONAL_TRIGGERS.filter(trigger => {
    ...
    switch (trigger) {
      case 'fear': return text.includes('miss out') || text.includes('lose') || text.includes('risk') || text.includes('avoid');
      case 'greed': return text.includes('earn') || text.includes('profit') || text.includes('wealth') || text.includes('gain');
      case 'guilt': return text.includes('deserve') || text.includes('owe it') || text.includes('finally');
      case 'exclusivity': return text.includes('exclusive') || text.includes('limited') || text.includes('only') || text.includes('select');
      case 'salvation': return text.includes('solution') || text.includes('answer') || text.includes('finally') || text.includes('breakthrough');
      case 'curiosity': return text.includes('secret') || text.includes('discover') || text.includes('reveal') || text.includes('hidden');
      case 'urgency': return text.includes('now') || text.includes('today') || text.includes('limited') || text.includes('deadline');
      case 'trust': return text.includes('guarantee') || text.includes('proven') || text.includes('trusted') || text.includes('risk-free');
      case 'belonging': return text.includes('join') || text.includes('community') || text.includes('together') || text.includes('members');
      case 'pride': return text.includes('proud') || text.includes('achievement') || text.includes('success') || text.includes('accomplish');
  ...
  if (emotionalTriggersUsed.length >= 4) {
    ...
    suggestions.push('Incorporate more emotional triggers: fear, greed, guilt, exclusivity, salvation');
  }

  // Check for dream state
  const hasDreamState = text.includes('imagine') || text.includes('picture yourself') || text.includes('dream') ||
    text.includes('finally') || text.includes('freedom') || text.includes('achieve') || text.includes('transform');
  ...
  const hasFearState = text.includes('if you don\'t') || text.includes('without this') || text.includes('stuck') ||
    text.includes('struggle') || text.includes('frustrat') || text.includes('painful') || text.includes('miss out');
  ...
  const hasAnalogy = text.includes('like') || text.includes('just as') || text.includes('similar to') ||
    text.includes('think of it as') || text.includes('it\'s like');
...
function scoreCTAStrength(content: ConversionContent): CTAScoreDetail {
  ...
  if (allCTATexts.length === 0) {
    ...
    const ctaPatterns = /(?:click|get|grab|claim|start|join|download|sign up|subscribe|buy|order|try)/gi;
  ...
  const allCTAs = allCTATexts.map(ctaText => {
    ...
    // Check for benefit
    const benefitWords = ['free', 'instant', 'your', 'save', 'get', 'gain', 'earn', 'learn', 'discover', 'access'];
  ...
  const primaryCTAAnalysis = {
    ...
    hasActionVerb: STRONG_ACTION_VERBS.some(v => primaryLower.startsWith(v) || primaryLower.includes(' ' + v)),
    hasBenefit: ['free', 'instant', 'your', 'save', 'get', 'access'].some(b => primaryLower.includes(b)),
...
function scoreObjectionCoverage(content: ConversionContent): ObjectionScoreDetail {
  ...
  const objectionAnalysis: Record<string, { addressed: boolean; strength: 'strong' | 'weak' | 'none' }> = {};
...
function scoreAntiPlaceholder(content: ConversionContent): AntiPlaceholderScoreDetail {
  ...
  if (placeholdersFound.length > 0) {
    issues.push(`Found ${placeholdersFound.length} placeholder(s): ${placeholdersFound.slice(0, 3).join(', ')}${placeholdersFound.length > 3 ? '...' : ''}`);
...
export class ConversionScorer {
  ...
  score(content: ConversionContent): ConversionScoreResult {
    ...
    const totalScore = Math.round(
      scores.wiifm.score * weights.wiifm +
      scores.clarity.score * weights.clarity +
      scores.emotional.score * weights.emotional +
      scores.ctaStrength.score * weights.ctaStrength +
      scores.objectionCoverage.score * weights.objectionCoverage +
      scores.antiPlaceholder.score * weights.antiPlaceholder
    );

    // Determine verdict
    let verdict: 'PASS' | 'ENHANCE' | 'REJECT';
    if (totalScore >= this.config.thresholds.pass) {
      verdict = 'PASS';
    } else if (totalScore >= this.config.thresholds.enhance) {
      verdict = 'ENHANCE';
    } else {
      verdict = 'REJECT';
    }
  ...
  private estimateImprovement(scores: ConversionScoreResult['scores'], fixes: string[]): string {
    ...
    if (scores.wiifm.score < 90) potentialGain += (90 - scores.wiifm.score) * weights.wiifm;
    if (scores.clarity.score < 90) potentialGain += (90 - scores.clarity.score) * weights.clarity;
    if (scores.emotional.score < 90) potentialGain += (90 - scores.emotional.score) * weights.emotional;
    if (scores.ctaStrength.score < 90) potentialGain += (90 - scores.ctaStrength.score) * weights.ctaStrength;
    if (scores.objectionCoverage.score < 90) potentialGain += (90 - scores.objectionCoverage.score) * weights.objectionCoverage;
    if (scores.antiPlaceholder.score < 100) potentialGain += (100 - scores.antiPlaceholder.score) * weights.antiPlaceholder;
  ...
  quickCheck(content: ConversionContent): { wouldPass: boolean; criticalIssues: string[] } {
    ...
    if (ctas.length === 0 && !text.toLowerCase().includes('get') && !text.toLowerCase().includes('start')) {
 ...
 */
export function quickConversionCheck(content: ConversionContent): { wouldPass: boolean; criticalIssues: string[] } {
  const scorer = createConversionScorer();
