...
export class CheckpointManager {
  ...
  private async enforceMaxCheckpoints(buildId: string): Promise<void> {
    ...
    if (checkpoints.length > this.config.maxCheckpointsPerBuild) {
      const toDelete = checkpoints.slice(0, checkpoints.length - this.config.maxCheckpointsPerBuild);
      for (const cp of toDelete) {
        await this.store.delete(cp.id);
      }
    }
  ...
  private reconstructContext(
    ...
    const context: ReconstructedContext = {
      ...
      criticalDecisions: buildContext.criticalDecisions ? { ...buildContext.criticalDecisions } : undefined,
  ...
  private getAllAgentsFromContext(context: ReconstructedContext): string[] {
    ...
    return [
      ...
      // Design
      'palette', 'grid', 'blocks', 'cartographer', 'flow', 'polish',
      // Architecture
