...
export class JudgeModule {
  ...
  async judge(
    ...
    if (SPEC_COMPLIANCE_AGENTS.includes(agentId.toLowerCase()) && this.specComplianceValidator.isInitialized()) {
      ...
      const artifacts = (outputData?.artifacts as Array<{ type: string; path?: string; content?: string }>) || [];
      ...
      if (generatedFiles.size > 0) {
        ...
        if (specComplianceResult.overallCompliance < 90 && context.currentRetry < 2) {
          const missingPages = specComplianceResult.missing.criticalPages.map(p => p.path).slice(0, 5);
          const missingComponents = specComplianceResult.missing.criticalComponents.map(c => c.name).slice(0, 5);

          const complianceFeedback = `CRITICAL: Your output is missing required spec items. Generate these FIRST:\n` +
  ...
  private updateMetrics(agentId: string, buildId: string, score: QualityScore): void {
    ...
    if (existing) {
      ...
      if (scores.length >= 3) {
        ...
        const older = scores.slice(0, -3).reduce((sum, s) => sum + s.overall, 0) / Math.max(1, scores.length - 3);
