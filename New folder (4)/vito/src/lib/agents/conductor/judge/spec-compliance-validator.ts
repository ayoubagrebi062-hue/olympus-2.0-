...
export class SpecComplianceValidator {
  ...
  validateSingleOutput(
    ...
    if (!this.spec) {
      return { valid: false, issues: [{ type: 'incomplete', severity: 'critical', message: 'Validator not initialized' }] };
    }
  ...
  private validatePages(generatedFiles: Map<string, string>): CompletionReport {
    ...
    for (const page of this.spec.pages) {
      const generatedPath = this.findGeneratedFile(generatedFiles, page.expectedFilePath, page.path);
      ...
      if (generatedPath) {
        ...
        if (issues.length === 0) {
          completed++;
          details.push({ requirement: page, status: 'completed', generatedPath, confidence: 1, issues: [] });
        } else {
          partial++;
          details.push({ requirement: page, status: 'partial', generatedPath, confidence: 0.7, issues: issues.map(i => i.message) });
        }
      } else {
        missing++;
        details.push({ requirement: page, status: 'missing', confidence: 0, issues: ['Page not generated'] });
      }
    }
  ...
  private validateComponents(generatedFiles: Map<string, string>): CompletionReport {
    ...
    for (const component of this.spec.components) {
      ...
      for (const [path, fileContent] of fileEntries) {
        if (this.pathMatches(path, component.path) || this.componentNameInFile(component.name, fileContent)) {
          found = true;
          generatedPath = path;
          content = fileContent;
          break;
      ...
      if (found) {
        ...
        if (issues.length === 0) {
          completed++;
          details.push({ requirement: component, status: 'completed', generatedPath, confidence: 1, issues: [] });
        } else {
          partial++;
          details.push({ requirement: component, status: 'partial', generatedPath, confidence: 0.7, issues: issues.map(i => i.message) });
        }
      } else {
        missing++;
        details.push({ requirement: component, status: 'missing', confidence: 0, issues: ['Component not generated'] });
      }
    }
   ...
   */
  private validateComponentContent(component: ComponentRequirement, content: string): ComplianceIssue[] {
    const issues: ComplianceIssue[] = [];

    // Check for component export
    const exportPattern = new RegExp(`export\\s+(?:default\\s+)?(?:function|const)\\s+${component.name}\\b`);
  ...
  private checkAllCriticalPresent(generatedFiles: Map<string, string>): boolean {
    ...
    for (const component of criticalComponents) {
      ...
      for (const [path, content] of fileEntries) {
        if (this.pathMatches(path, component.path) || this.componentNameInFile(component.name, content)) {
          found = true;
          break;
        }
      }
  ...
  private componentNameInFile(componentName: string, content: string): boolean {
    const pattern = new RegExp(`export\\s+(?:default\\s+)?(?:function|const)\\s+${componentName}\\b`, 'i');
