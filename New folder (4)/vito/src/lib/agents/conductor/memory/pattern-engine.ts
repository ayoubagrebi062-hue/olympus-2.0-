...
export class PatternEngine implements IPatternStore {
  ...
  async getConfidentPatterns(minConfidence: number = this.config.patternConfidenceThreshold): Promise<LearnedPattern[]> {
  ...
  private extractQualityPatterns(builds: BuildRecord[]): LearnedPattern[] {
    ...
    for (const [projectType, typeBuilds] of Object.entries(byType)) {
      ...
      const successfulBuilds = typeBuilds.filter(b => b.status === 'completed' && b.overallQuality >= 7);
      if (successfulBuilds.length < 3) continue;

      // Calculate optimal quality threshold
      const avgQuality = successfulBuilds.reduce((sum, b) => sum + b.overallQuality, 0) / successfulBuilds.length;
  ...
  private extractAgentPatterns(builds: BuildRecord[]): LearnedPattern[] {
    ...
    for (const [key, groupBuilds] of Object.entries(grouped)) {
      ...
      const successfulBuilds = groupBuilds.filter(b => b.status === 'completed' && b.overallQuality >= 7);
      if (successfulBuilds.length < 3) continue;
  ...
  private extractRetryPatterns(builds: BuildRecord[]): LearnedPattern[] {
    ...
    for (const [projectType, typeBuilds] of Object.entries(byType)) {
      ...
      const successfulWithRetries = typeBuilds.filter(b => b.status === 'completed' && b.overallQuality >= 7);
      if (successfulWithRetries.length < 2) continue;

      const avgRetries = successfulWithRetries.reduce((sum, b) => {
  ...
  private extractErrorPatterns(builds: BuildRecord[]): LearnedPattern[] {
    ...
    const recoveredBuilds = builds.filter(b =>
      b.status === 'completed' &&
      b.errors.some(e => e.recoverable)
    );
    ...
    for (const [errorType, stats] of Object.entries(errorGroups)) {
      ...
      patterns.push(this.createPattern({
        ...
        trigger: {
          conditions: [
            { field: 'errorType', operator: 'matches', value: errorType }
          ],
          operator: 'AND'
        },
        action: {
  ...
  private extractCostPatterns(builds: BuildRecord[]): LearnedPattern[] {
    ...
    const avgQuality = completedBuilds.reduce((sum, b) => sum + b.overallQuality, 0) / completedBuilds.length;

    // Find builds that are both high quality and cost efficient
    const efficientBuilds = completedBuilds.filter(b =>
      b.overallQuality >= avgQuality &&
      b.costUSD < avgCost * 0.8
    );
    ...
    for (const [tier, tierBuilds] of Object.entries(byTier)) {
      ...
      patterns.push(this.createPattern({
        ...
        trigger: {
          conditions: [
            { field: 'tier', operator: 'equals', value: tier }
          ],
          operator: 'AND'
        },
        action: {
  ...
  switch (action.type) {
    case 'modify_threshold':
      ...
      if (action.parameters.dimension) {
        (target.thresholds as Record<string, number>)[String(action.parameters.dimension)] = Number(action.parameters.threshold);
      }
      return {
        success: true,
        applied: true,
    ...
    case 'adjust_retry':
      target.retryPolicy = Number(action.parameters.retryPolicy ?? action.parameters.maxRetries ?? action.parameters.suggestedRetries);
      return {
        success: true,
        applied: true,
        changes: target,
