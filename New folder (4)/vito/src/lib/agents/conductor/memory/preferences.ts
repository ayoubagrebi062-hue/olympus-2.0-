...
export class PreferencesManager implements IPreferencesStore {
  ...
  async updateFromBuild(tenantId: string, buildRecord: BuildRecord): Promise<void> {
    ...
    const totalRetries = Object.values(buildRecord.outputs)
      .reduce((sum, o) => sum + o.retryCount, 0);
  ...
  async setStrategyPreference(tenantId: string, strategy: UserPreferences['preferredStrategy']): Promise<void> {
  ...
  async setCodeStylePreferences(tenantId: string, codeStyle: Partial<CodeStylePreferences>): Promise<void> {
    const prefs = await this.getOrCreate(tenantId);
  ...
  async setDesignStylePreferences(tenantId: string, designStyle: Partial<DesignStylePreferences>): Promise<void> {
    const prefs = await this.getOrCreate(tenantId);
  ...
  async getStatistics(): Promise<{
    ...
    for (const prefs of allPrefs) {
      ...
      if (prefs.preferredStrategy) {
        strategyDistribution[prefs.preferredStrategy] = (strategyDistribution[prefs.preferredStrategy] || 0) + 1;
      }
