...
import { VectorStore, createOllamaEmbeddingFn, type VectorStoreConfig, type OllamaEmbeddingConfig } from './vector-store';
...
export * from './types';
export { BuildStore, createBuildRecord, addOutputToRecord, addErrorToRecord, finalizeBuildRecord } from './build-store';
...
export class MemoryModule {
  ...
  async getAnalytics(): Promise<MemoryAnalytics> {
    ...
    if (allBuilds.length >= 10) {
      ...
      const recentAvg = recent.filter(b => b.status === 'completed')
        .reduce((sum, b) => sum + b.overallQuality, 0) / 5;
      const olderAvg = older.filter(b => b.status === 'completed')
        .reduce((sum, b) => sum + b.overallQuality, 0) / 5;
  ...
  async generateRecommendations(
    ...
    for (const pattern of patterns.slice(0, 3)) {
      recommendations.push({
        ...
        impact: pattern.action.priority >= 7 ? 'high' : pattern.action.priority >= 4 ? 'medium' : 'low',
