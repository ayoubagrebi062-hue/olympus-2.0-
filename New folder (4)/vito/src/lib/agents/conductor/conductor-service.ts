...
export class ConductorService {
  ...
  constructor(config: Partial<ConductorConfig> = {}, aiRouter?: AIRouter) {
    ...
    this.judgeModule = new JudgeModule(
      this.aiRouter || undefined,
      this.config.judgeConfig
    );
    ...
    this.checkpointManager.onEvent((event) => {
      ...
      if ('checkpoint' in event.data && event.data.checkpoint) {
        ...
        this.emitConductorEvent(`conductor:${event.type.replace('checkpoint:', '')}` as any, cp.buildId, event.data);
      }
    });
    ...
    const defaultAgents: AgentCapabilities[] = [
      {
        ...
        name: 'Frontend Agent',
        description: 'Handles UI/UX tasks including React components, styling, and responsive design',
        capabilities: ['react', 'vue', 'css', 'tailwind', 'ui', 'ux', 'component', 'styling'],
        triggerKeywords: ['button', 'form', 'modal', 'dropdown', 'animation', 'responsive', 'layout'],
        priority: 1,
        available: true,
        maxConcurrent: 5,
        currentLoad: 0,
      ...
      },
      {
        ...
        capabilities: ['security', 'vulnerability', 'xss', 'csrf', 'injection', 'auth', 'encryption'],
  ...
  subscribe(
    buildId: string,
    listener: (event: ExtendedOrchestrationEvent) => void
  ): () => void {
  ...
  async getMemoryRecommendations(
    ...
    return this.memoryModule.generateRecommendations(tenantId, description, projectType, complexity);
  }
  ...
  private async executeWithPhaseControl(buildState: ConductorBuildState): Promise<void> {
    ...
    const contextTier = this.mapConductorTierToBuildTier(request.tier || buildState.analysis.suggestedTier);
   ...
   */
  private async checkAgentDependencies(
    planId: string,
    agent: AgentPlan
  ): Promise<boolean> {
  ...
  private async executeAgent(
    ...
    try {
      ...
      const tokenTracker = buildState.tokenTracker || new TokenTracker(
        buildState.buildId,
  ...
  async validatePhaseTransition(
    ...
    const context: TransitionContext = {
      ...
      errors: new Map(
        plan.agents.filter((a) => a.error).map((a) => [a.agentId, a.error as string])
      ),
    };
  ...
  private mapToPhases(analysis: ProjectAnalysis): BuildPlan['phases'] {
    ...
    if (['saas-app', 'e-commerce', 'full-stack', 'api-service', 'dashboard'].includes(analysis.type)) {
      phases.push({
        phase: 'backend',
        name: 'backend',
  ...
  private async executeBuild(buildState: ConductorBuildState): Promise<void> {
    ...
    const tier = this.mapConductorTierToBuildTier(request.tier || buildState.analysis.suggestedTier);
  ...
  private async judgeAgentOutput(
    ...
    try {
      ...
      const decision = isCritical
        ? await this.judgeModule.judge(
            agentId,
            payload,
            agentDef,
            judgeContext
          )
        : this.judgeModule.quickJudge(
            agentId,
            payload,
            agentDef,
            judgeContext
    ...
    switch (decision.action) {
      ...
      case 'fail':
        ...
        if (this.config.judgeConfig?.strictMode) {
          throw new Error(`Agent ${agentId} failed quality check with score ${decision.score.overall}`);
        }
        break;
    }
  }
   ...
   */
  private getAgentDefinition(agentId: string): { id: string; description?: string; outputSchema?: Record<string, unknown> } {
  ...
  private async createCheckpoint(
    ...
    const { buildId, request, contextManager, tokenTracker, orchestrator, analysis, plan } = buildState;
    ...
    if (this.config.checkpointConfig?.enabled) {
      ...
      const serializedBuildContext: SerializedBuildContext = {
        ...
        buildPlan: {
          phases: plan.phases.map(p => ({
            ...
            status: (progress?.completedPhases || []).includes(p.name ?? p.phase) ? 'completed' as const : 'pending' as const,
          })),
      ...
      if (contextManager?.getCriticalDecisions()) {
        ...
        decisions.push(
          { key: 'projectType', value: cd.projectType, reason: 'From analysis', madeBy: 'conductor', madeAt: new Date() },
          { key: 'primaryFramework', value: cd.primaryFramework, reason: 'From analysis', madeBy: 'conductor', madeAt: new Date() },
          { key: 'stylingApproach', value: cd.stylingApproach, reason: 'From analysis', madeBy: 'conductor', madeAt: new Date() }
        );
      }
