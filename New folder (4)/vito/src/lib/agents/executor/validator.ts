...
export function validateOutput(
  ...
  if (output.artifacts.length === 0) {
    warnings.push({ field: 'artifacts', message: 'No artifacts generated', suggestion: 'Agent should produce at least one artifact' });
  }
  ...
  // COMPLEXITY GATE
  // ═══════════════════════════════════════════════════════════════
  for (const artifact of output.artifacts.filter(a => a.type === 'code' && a.path?.endsWith('.tsx'))) {
  ...
  for (const artifact of output.artifacts.filter(a => a.type === 'code' && a.path?.endsWith('.tsx'))) {
    ...
    if (!designResult.valid) {
      errors.push({
        ...
        details: designResult.violations.slice(0, 5).map(v => v.value).join(', '),
  ...
  if (!importValidation.valid) {
    errors.push({
      ...
      suggestion: 'Agent must generate ALL files that are imported. Missing: ' + importValidation.unresolvedImports.join(', '),
    });
  }
...
function criteriaToPatterns(criteria: string, featureId: string): RegExp[] {
  ...
  if (patterns.length === 0) {
    ...
    for (const keyword of keywords.slice(0, 3)) {
      // Skip common words
      if (!['must', 'have', 'with', 'that', 'this', 'from', 'should', 'using'].includes(keyword.toLowerCase())) {
...
function validateAgainstSchema(content: string, schema: AgentDefinition['outputSchema']): ValidationError[] {
  const errors: ValidationError[] = [];
  ...
  try {
    ...
    if (schema.type === 'object' && typeof parsed !== 'object') {
      errors.push({ field: 'root', message: 'Expected object', expected: 'object', received: typeof parsed });
      return errors;
    }

    if (schema.type === 'array' && !Array.isArray(parsed)) {
      errors.push({ field: 'root', message: 'Expected array', expected: 'array', received: typeof parsed });
      return errors;
    }
...
function validatePropertyType(key: string, value: unknown, schema: { type: string; items?: any }): ValidationError | null {
...
function validateCodeArtifact(artifact: Artifact): { errors: ValidationError[]; warnings: ValidationWarning[] } {
  const errors: ValidationError[] = [];
  const warnings: ValidationWarning[] = [];
  ...
  if (content.includes('TODO') || content.includes('FIXME')) {
    warnings.push({ field: 'content', message: 'Code contains TODO/FIXME comments', suggestion: 'Consider completing implementation' });
  }

  if (content.includes('console.log') && artifact.path?.includes('.ts')) {
    warnings.push({ field: 'content', message: 'Code contains console.log statements', suggestion: 'Use proper logging' });
  }
  ...
  if (artifact.path?.endsWith('.ts') || artifact.path?.endsWith('.tsx')) {
    if (content.includes(': any') && (content.match(/: any/g)?.length || 0) > 5) {
      warnings.push({ field: 'content', message: 'Excessive use of any type', suggestion: 'Add proper TypeScript types' });
    }
  }
  ...
  if (artifact.path?.match(/\.(ts|tsx)$/)) {
    const compileResult = validateTypeScriptCompilation(
      content,
      artifact.path
    );
    ...
    if (!compileResult.success) {
      ...
      for (const compileError of compileResult.errors.slice(0, 5)) { // Limit to first 5 errors
        errors.push({
          field: 'compilation',
  ...
  if (artifact.path?.includes('/components/') && artifact.path?.endsWith('.tsx')) {
    ...
    if (!stateResult.passed) {
      ...
      const isInteractive = /Button|Input|Select|Checkbox|Toggle|Switch|Slider/i.test(componentName);
  ...
  if (artifact.path?.includes('page.tsx') || artifact.path?.includes('Page.tsx')) {
    ...
    if (lineCount < 80) {
      ...
      errors.push({
        ...
        details: 'Pages must have loading state, empty state, real handlers, and proper UI structure',
      });
    }
    ...
    if (missingRequirements.length > 2) {
      warnings.push({
        ...
        suggestion: 'Complete pages should handle loading, error, and empty states with real handlers',
      });
    }
  }
  ...
  const buttonsWithoutHandler = buttonMatches.filter(btn =>
  ...
  const inputsWithoutHandler = inputMatches.filter(input =>
  ...
  if (inputsWithoutHandler.length > 2) { // Allow some uncontrolled inputs
    warnings.push({
      field: 'handlers',
...
function validateImportResolution(artifacts: Artifact[]): ImportValidationResult {
  ...
  for (const artifact of artifacts) {
    ...
    for (const importPath of allImports) {
      ...
      const resolved = generatedPaths.has(normalizedImport) ||
                       generatedPaths.has(normalizedImport + '/index');
...
export function getValidationSummary(result: ValidationResult): string {
  ...
  if (result.warnings.length > 0) {
    parts.push(`${result.warnings.length} warning(s): ${result.warnings.map((w) => w.message).join('; ')}`);
  }
