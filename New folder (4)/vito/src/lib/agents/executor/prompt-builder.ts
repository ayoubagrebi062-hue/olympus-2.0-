...
async function fetchRelevantExamples(
  ...
  try {
    ...
    const filtered = results.filter((r) =>
      config.fileTypes.includes(r.fileType as any)
    );
...
export function buildAgentPrompt(input: AgentInput, definition: AgentDefinition): { systemPrompt: string; messages: ChatMessage[] } {
...
function buildSystemPrompt(
  ...
  if (definition.phase === 'frontend' || definition.phase === 'backend' ||
      definition.phase === 'integration' || definition.id === 'pixel' ||
      definition.id === 'wire' || definition.id === 'polish' ||
      definition.id === 'gateway' || definition.id === 'keeper' ||
      definition.id === 'engine' || definition.id === 'forge') {
    parts.push(`\n\n${CODE_QUALITY_RULES}`);
  }
...
function summarizeOutputForDependency(output: AgentOutput): string {
  ...
  if (output.decisions.length) {
    parts.push(`decisions: ${output.decisions.slice(0, 3).map((d) => d.choice).join(', ')}`);
  }
