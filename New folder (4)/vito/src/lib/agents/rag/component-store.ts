...
export class ComponentStore {
  ...
  async initialize(): Promise<void> {
    try {
      const exists = await this.client.getCollections().then(
        collections => ({ exists: collections.collections.some(c => c.name === this.collectionName) })
      );
  ...
  async addComponent(component: ComponentExample, embedding: number[]): Promise<void> {
    ...
    if (component.quality_score < 85) {
      console.warn(`[ComponentStore] Skipping low-quality component: ${component.name} (score: ${component.quality_score})`);
      return;
    }
   ...
   */
  async addComponents(components: Array<{ component: ComponentExample; embedding: number[] }>): Promise<number> {
  ...
  async getStats(): Promise<{
    ...
    const categories: ComponentCategory[] = [
      'button', 'card', 'form', 'input', 'navbar', 'hero', 'footer',
      'modal', 'table', 'sidebar', 'dropdown', 'badge', 'other'
    ];
