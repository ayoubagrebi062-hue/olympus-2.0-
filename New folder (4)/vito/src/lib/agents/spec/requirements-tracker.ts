...
export class RequirementsTracker {
  ...
  private matchToComponent(
    ...
    for (const component of this.spec.components) {
      ...
      if (fileNameNoExt === componentKebab || fileNameNoExt.toLowerCase() === component.name.toLowerCase()) {
        return { requirement: component, confidence: 0.9 };
      }

      // Check if content exports the component name
      const exportMatch = content.match(
        new RegExp(`export\\s+(?:default\\s+)?(?:function|const|class)\\s+${component.name}\\b`, 'i')
      );
      if (exportMatch) {
  ...
  getSummary(): {
    ...
    const overall = Math.round(pages.percentage * 0.4 + components.percentage * 0.4 + designSystem * 0.1 + critical * 0.1);
