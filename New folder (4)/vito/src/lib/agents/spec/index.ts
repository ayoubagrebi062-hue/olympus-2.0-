...
import type { SpecParserOptions, CompletenessGateConfig, CompletenessGateResult, SpecParserResult } from './types';
...
export function quickComplianceCheck(
  ...
  return {
    passed: tracker.getPageCompletion().percentage >= 90 &&
...
export function getSpecStats(specContent: string): {
  ...
  return {
    ...
    hasDesignSystem: Object.keys(requirements.designSystem.colors).length > 0 ||
