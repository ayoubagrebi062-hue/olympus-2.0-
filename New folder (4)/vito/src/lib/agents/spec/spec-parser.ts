...
export class SpecParser {
  ...
  private splitIntoSections(): Map<string, string> {
    ...
    while ((match = headerRegex.exec(this.rawSpec)) !== null) {
      ...
      if (!sections.has(headerName)) {
        ...
        const nextHeaderMatch = /^#{1,2}\s+/m.exec(this.rawSpec.slice(match.index + match[0].length));
  ...
  private parsePagesFromTables(): PageRequirement[] {
    ...
    const tableRegex = /\|[^|]*#[^|]*\|[^|]*Path[^|]*\|[^|]*Name[^|]*\|[^|]*Priority[^|]*\|[\s\S]*?(?=\n\n|\n#|$)/gi;
  ...
  private extractPageSections(pagePath: string): string[] {
    ...
    const sectionRegex = new RegExp(`${pageName}[\\s\\S]*?(?:sections?|contains?):\\s*([\\s\\S]*?)(?=\\n\\n|$)`, 'i');
  ...
  private parseComponentsFromTree(): ComponentRequirement[] {
    ...
    while ((match = treeRegex.exec(this.rawSpec)) !== null) {
      ...
      if (seenNames.has(name) || name.length < 2 || /^(src|app|components|ui|lib|utils)$/i.test(name)) {
        continue;
      }
  ...
  private parseColors(): Record<string, string> {
    ...
    const colorRegex = /(\w+(?:[-_]\w+)?)\s*[:=]\s*['"]?(#[0-9a-fA-F]{3,8}|rgba?\([^)]+\)|hsla?\([^)]+\))['"]?/gi;
    let match: RegExpExecArray | null;
  ...
  private parseTypography(): Record<string, TypographySpec> {
    ...
    const typoRegex = /\.text-(\w+)\s*\{[\s\S]*?font-size:\s*([^;]+);[\s\S]*?font-weight:\s*([^;]+);[\s\S]*?line-height:\s*([^;]+);/gi;
    let match: RegExpExecArray | null;
  ...
  private parseAnimations(): string[] {
    ...
    const animationPatterns = ['fadeIn', 'fadeInUp', 'fadeInScale', 'slideIn', 'stagger', 'hover', 'pulse', 'glow'];
  ...
  private parseFeatures(): FeatureRequirement[] {
    ...
    const featureRegex = /(?:feature|capability):\s*([^\n]+)[\s\S]*?(?:description|details?):\s*([^\n]+)/gi;
    let match: RegExpExecArray | null;
  ...
  private inferAuthRequired(path: string): boolean {
    return path.startsWith('/dashboard') || path.startsWith('/settings') || path.startsWith('/account');
  }
  ...
  private inferPageCategory(path: string): PageCategory {
    ...
    if (path === '/' || ['/features', '/pricing', '/about', '/contact'].includes(path)) return 'public';
    return 'other';
  }
  ...
  private inferPageData(pages: PageRequirement[]): void {
    for (const page of pages) {
      ...
      if (page.sections.length === 0) {
        if (page.path === '/') {
          page.sections = ['hero', 'social-proof', 'how-it-works', 'features', 'pricing', 'testimonials', 'cta', 'footer'];
