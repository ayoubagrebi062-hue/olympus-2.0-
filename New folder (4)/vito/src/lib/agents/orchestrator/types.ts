...
export type OrchestrationStatus = 'idle' | 'running' | 'paused' | 'completed' | 'failed' | 'canceled';
...
export type OrchestrationEvent =
  ...
  | { type: 'phase_completed'; phase: BuildPhase; status: PhaseStatus; data?: Record<string, unknown> }
  | { type: 'agent_started'; agentId: AgentId; phase: BuildPhase; data?: Record<string, unknown> }
  | { type: 'agent_completed'; agentId: AgentId; output: AgentOutput; data?: Record<string, unknown> }
  | { type: 'agent_failed'; agentId: AgentId; error: OrchestrationError; data?: Record<string, unknown> }
  | { type: 'validation_started'; buildId: string; data?: Record<string, unknown> }
  | { type: 'build_completed'; buildId: string; success: boolean; outputPath?: string; filesWritten?: number; buildTime?: number; validationErrors?: string[]; data?: Record<string, unknown> }
  | { type: 'build_failed'; buildId: string; error: OrchestrationError; data?: Record<string, unknown> }
