...
export interface LogEntry {
  ...
  level: LogLevel;
  buildId: string;
  component: 'circuit_breaker' | 'self_healing' | 'fingerprint' | 'degradation' | 'tracing' | 'anomaly' | 'health';
  message: string;
...
export type ResilienceEvent =
  | { type: 'circuit:state_change'; agentId: AgentId; from: CircuitState; to: CircuitState; reason: string; timestamp: Date }
  | { type: 'circuit:open'; agentId: AgentId; failureCount: number; lastError: string; timestamp: Date }
  ...
  | { type: 'healing:attempt'; agentId: AgentId; action: SelfHealingAction; attemptNumber: number; timestamp: Date }
  | { type: 'healing:success'; agentId: AgentId; action: SelfHealingAction; timestamp: Date }
  | { type: 'healing:failure'; agentId: AgentId; action: SelfHealingAction; error: string; timestamp: Date }
  | { type: 'healing:exhausted'; agentId: AgentId; totalAttempts: number; timestamp: Date }
  | { type: 'degradation:tier_change'; from: DegradationTier; to: DegradationTier; reason: string; agentsDropped: number; timestamp: Date }
  ...
  | { type: 'fingerprint:stored'; hash: string; result: 'SUCCESS' | 'FAILURE' | 'PARTIAL'; timestamp: Date }
  | { type: 'health:score_calculated'; score: number; prediction: string; factors: Record<string, number>; timestamp: Date }
  | { type: 'health:build_blocked'; score: number; reason: string; timestamp: Date }
  | { type: 'anomaly:detected'; category: 'latency' | 'memory' | 'error_rate'; agentId?: AgentId; value: number; threshold: number; timestamp: Date }
  | { type: 'security:injection_detected'; agentId: AgentId; snippet: string; timestamp: Date }
  | { type: 'trace:span_start'; traceId: string; spanId: string; operation: string; timestamp: Date }
  | { type: 'trace:span_end'; traceId: string; spanId: string; durationMs: number; status: 'SUCCESS' | 'FAILURE'; timestamp: Date }
  // Bulkhead events (COMPETITIVE FEATURE)
  | { type: 'bulkhead:acquired'; agentId: AgentId; currentPerAgent: number; currentTotal: number; waitedMs: number; timestamp: Date }
  | { type: 'bulkhead:released'; agentId: AgentId; currentPerAgent: number; currentTotal: number; timestamp: Date }
  | { type: 'bulkhead:rejected'; agentId: AgentId; reason: 'per_agent_limit' | 'total_limit' | 'timeout'; queueLength: number; timestamp: Date }
  | { type: 'bulkhead:queued'; agentId: AgentId; position: number; estimatedWaitMs: number; timestamp: Date };
...
export class ResilienceEventBus {
  ...
  onAll(handler: ResilienceEventHandler): () => void {
    if (this.handlerCount >= MAX_EVENT_HANDLERS) {
      throw new Error(`Event handler limit (${MAX_EVENT_HANDLERS}) exceeded. Possible memory leak or attack.`);
    }
...
export class ResilienceLogger {
  ...
  private log(level: LogLevel, component: LogEntry['component'], message: string, metadata?: Record<string, unknown>): void {
  ...
  debug(component: LogEntry['component'], message: string, metadata?: Record<string, unknown>): void {
    this.log('debug', component, message, metadata);
  }

  info(component: LogEntry['component'], message: string, metadata?: Record<string, unknown>): void {
    this.log('info', component, message, metadata);
  }

  warn(component: LogEntry['component'], message: string, metadata?: Record<string, unknown>): void {
    this.log('warn', component, message, metadata);
  }

  error(component: LogEntry['component'], message: string, metadata?: Record<string, unknown>): void {
    this.log('error', component, message, metadata);
  }

  fatal(component: LogEntry['component'], message: string, metadata?: Record<string, unknown>): void {
...
export const DEGRADATION_TIERS: Record<DegradationTier, AgentId[]> = {
  ...
  PLATINUM: [
    ...
    // Conversion (3)
    'psyche', 'scribe', 'architect_conversion',
    // Design (6)
    'palette', 'grid', 'blocks', 'cartographer', 'flow', 'artist',
    // Architecture (6)
  ...
  GOLD: [
    // Discovery (4)
    'oracle', 'empathy', 'strategos', 'scope',
    // Conversion (2)
    'scribe', 'architect_conversion',
    // Design (4)
    'palette', 'blocks', 'cartographer', 'artist',
    // Architecture (5)
  ...
  BRONZE: [
    'oracle', 'strategos',           // Discovery
    'blocks',                         // Design
    'archon', 'datum',               // Architecture
    'pixel', 'wire',                 // Frontend
    'docker'                          // Deployment
  ],
};
...
export class ResilienceEngine {
  ...
  private bulkheadQueue: Array<{ agentId: AgentId; resolve: () => void; reject: (err: Error) => void; timestamp: number }> = [];
  ...
  private setupMetricsFromEvents(): void {
    this.events.onAll((event) => {
      ...
      switch (event.type) {
        case 'circuit:open':
          this.metricsCollector.increment('resilience.circuit.opened', 1, { ...tags, agentId: event.agentId });
          break;
        case 'circuit:close':
          this.metricsCollector.increment('resilience.circuit.closed', 1, { ...tags, agentId: event.agentId });
          break;
        case 'healing:success':
          this.metricsCollector.increment('resilience.healing.success', 1, { ...tags, action: event.action.type });
          break;
        case 'healing:failure':
          this.metricsCollector.increment('resilience.healing.failure', 1, { ...tags, action: event.action.type });
          break;
        ...
        case 'anomaly:detected':
          this.metricsCollector.increment('resilience.anomaly', 1, { ...tags, category: event.category });
          break;
        case 'security:injection_detected':
          this.metricsCollector.increment('resilience.security.injection', 1, { ...tags, agentId: event.agentId });
          break;
      }
    });
  }
  ...
  private validateAndMergeConfig(config: Partial<ResilienceConfig>): ResilienceConfig {
    ...
    if (config.circuitBreaker) {
      merged.circuitBreaker = {
        ...merged.circuitBreaker,
        enabled: typeof config.circuitBreaker.enabled === 'boolean' ? config.circuitBreaker.enabled : merged.circuitBreaker.enabled,
        failureThreshold: this.clampNumber(config.circuitBreaker.failureThreshold, 1, 100, merged.circuitBreaker.failureThreshold),
        successThreshold: this.clampNumber(config.circuitBreaker.successThreshold, 1, 50, merged.circuitBreaker.successThreshold),
        timeout: this.clampNumber(config.circuitBreaker.timeout, CIRCUIT_TIMEOUT_MIN_MS, CIRCUIT_TIMEOUT_MAX_MS, merged.circuitBreaker.timeout),
        volumeThreshold: this.clampNumber(config.circuitBreaker.volumeThreshold, 1, 100, merged.circuitBreaker.volumeThreshold),
      };
    }
    ...
    if (config.selfHealing) {
      merged.selfHealing = {
        ...merged.selfHealing,
        enabled: typeof config.selfHealing.enabled === 'boolean' ? config.selfHealing.enabled : merged.selfHealing.enabled,
        maxAttempts: this.clampNumber(config.selfHealing.maxAttempts, 1, 10, merged.selfHealing.maxAttempts),
        analyzeFailures: typeof config.selfHealing.analyzeFailures === 'boolean' ? config.selfHealing.analyzeFailures : merged.selfHealing.analyzeFailures,
        autoSimplify: typeof config.selfHealing.autoSimplify === 'boolean' ? config.selfHealing.autoSimplify : merged.selfHealing.autoSimplify,
      };
    }
    ...
    if (config.fingerprinting) {
      merged.fingerprinting = {
        ...merged.fingerprinting,
        enabled: typeof config.fingerprinting.enabled === 'boolean' ? config.fingerprinting.enabled : merged.fingerprinting.enabled,
        cacheDir: this.sanitizePath(config.fingerprinting.cacheDir) || merged.fingerprinting.cacheDir,
        maxCacheAge: this.clampNumber(config.fingerprinting.maxCacheAge, FINGERPRINT_MAX_AGE_MIN_MS, FINGERPRINT_MAX_AGE_MAX_MS, merged.fingerprinting.maxCacheAge),
        reusePartial: typeof config.fingerprinting.reusePartial === 'boolean' ? config.fingerprinting.reusePartial : merged.fingerprinting.reusePartial,
      };
    }
    ...
    if (config.prediction) {
      merged.prediction = {
        ...merged.prediction,
        enabled: typeof config.prediction.enabled === 'boolean' ? config.prediction.enabled : merged.prediction.enabled,
        blockOnLowScore: typeof config.prediction.blockOnLowScore === 'boolean' ? config.prediction.blockOnLowScore : merged.prediction.blockOnLowScore,
        minScoreToStart: this.clampNumber(config.prediction.minScoreToStart, 0, 100, merged.prediction.minScoreToStart),
        warnThreshold: this.clampNumber(config.prediction.warnThreshold, 0, 100, merged.prediction.warnThreshold),
      };
    }
    ...
    if (config.degradation) {
      merged.degradation = {
        ...merged.degradation,
        enabled: typeof config.degradation.enabled === 'boolean' ? config.degradation.enabled : merged.degradation.enabled,
        autoDegrade: typeof config.degradation.autoDegrade === 'boolean' ? config.degradation.autoDegrade : merged.degradation.autoDegrade,
        startTier: this.validateTier(config.degradation.startTier) || merged.degradation.startTier,
        degradeOnTimeout: typeof config.degradation.degradeOnTimeout === 'boolean' ? config.degradation.degradeOnTimeout : merged.degradation.degradeOnTimeout,
        degradeOnAgentFailure: this.clampNumber(config.degradation.degradeOnAgentFailure, 1, 50, merged.degradation.degradeOnAgentFailure),
      };
    }
    ...
    if (config.tracing) {
      merged.tracing = {
        ...merged.tracing,
        enabled: typeof config.tracing.enabled === 'boolean' ? config.tracing.enabled : merged.tracing.enabled,
    ...
    if (config.bulkhead) {
      merged.bulkhead = {
        ...merged.bulkhead,
        enabled: typeof config.bulkhead.enabled === 'boolean' ? config.bulkhead.enabled : merged.bulkhead.enabled,
        maxConcurrentPerAgent: this.clampNumber(config.bulkhead.maxConcurrentPerAgent, 1, 100, merged.bulkhead.maxConcurrentPerAgent),
        maxConcurrentTotal: this.clampNumber(config.bulkhead.maxConcurrentTotal, 1, 500, merged.bulkhead.maxConcurrentTotal),
        maxWaitMs: this.clampNumber(config.bulkhead.maxWaitMs, 0, 300000, merged.bulkhead.maxWaitMs), // 0 to 5min
        fairQueuing: typeof config.bulkhead.fairQueuing === 'boolean' ? config.bulkhead.fairQueuing : merged.bulkhead.fairQueuing,
      };
    }
    ...
    if (config.anomaly) {
      merged.anomaly = {
        ...merged.anomaly,
        enabled: typeof config.anomaly.enabled === 'boolean' ? config.anomaly.enabled : merged.anomaly.enabled,
        latencyThreshold: this.clampNumber(config.anomaly.latencyThreshold, 100, 1000, merged.anomaly.latencyThreshold),
        errorRateThreshold: this.clampNumber(config.anomaly.errorRateThreshold, 1, 100, merged.anomaly.errorRateThreshold),
        memoryThreshold: this.clampNumber(config.anomaly.memoryThreshold, 256, 16384, merged.anomaly.memoryThreshold), // 256MB - 16GB
      };
    }
  ...
  private sanitizePath(inputPath: string | undefined): string | undefined {
    ...
    if (inputPath.includes('..') || inputPath.includes('~') || path.isAbsolute(inputPath)) {
      this.logger.warn('self_healing', 'Blocked potentially unsafe path', { path: inputPath.substring(0, 50) });
      return undefined;
    }
    return inputPath;
  }
  ...
  recordFailure(agentId: AgentId, error: string, category: FailureCategory): void {
    ...
    if (circuit.state === 'CLOSED' || circuit.state === 'HALF_OPEN') {
      ...
      if (totalCalls >= this.config.circuitBreaker.volumeThreshold &&
    ...
    if (this.config.degradation.autoDegrade &&
  ...
  async acquireBulkhead(agentId: AgentId): Promise<void> {
    ...
    if (currentPerAgent < this.config.bulkhead.maxConcurrentPerAgent &&
    ...
    if (this.config.bulkhead.maxWaitMs === 0) {
      // Fail fast mode
      const reason = currentPerAgent >= this.config.bulkhead.maxConcurrentPerAgent
      ...
      throw new Error(`Bulkhead rejected: ${reason} (agent: ${agentId}, perAgent: ${currentPerAgent}/${this.config.bulkhead.maxConcurrentPerAgent}, total: ${this.bulkheadTotal}/${this.config.bulkhead.maxConcurrentTotal})`);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeoutId = setTimeout(() => {
        ...
        if (index !== -1) {
          ...
          reject(new Error(`Bulkhead timeout after ${this.config.bulkhead.maxWaitMs}ms waiting for ${agentId}`));
        }
  ...
  categorizeFailure(error: string): FailureCategory {
    ...
    if (lower.includes('invalid') || lower.includes('parse') || lower.includes('schema') || lower.includes('json')) {
    ...
    if (lower.includes('token') || lower.includes('context length') || lower.includes('max_tokens')) {
  ...
  applyHealingAction(input: unknown, action: SelfHealingAction): unknown {
    ...
    this.logger.info('self_healing', `Applied action: ${action.type}`, { confidence: action.confidence });
    return modified;
  }
  ...
  getCachedBuild(fingerprint: string): BuildFingerprint | null {
    ...
    if (cached.result === 'SUCCESS' ||
      ...
      this.logger.debug('fingerprint', `Cache HIT: ${fingerprint.substring(0, 12)}...`, { result: cached.result });
      return cached;
    }
  ...
  storeBuildFingerprint(
    ...
    if (prompt && prompt.length > MAX_PROMPT_LENGTH) {
      this.logger.warn('fingerprint', `Prompt too large (${prompt.length} chars), truncating to ${MAX_PROMPT_LENGTH}`);
  ...
  private loadFingerprintCache(): void {
    ...
    try {
      ...
      this.logger.warn('fingerprint', 'Failed to load cache, starting fresh', { error: String(error) });
  ...
  private pruneFingerprints(): number {
    ...
    if (pruned > 0) {
      this.logger.debug('fingerprint', `Pruned ${pruned} entries`, { remaining: this.fingerprints.size });
    }
  ...
  calculateHealthScore(prompt: string, tier: DegradationTier): HealthScore {
    if (!this.config.prediction.enabled) {
      return {
        overall: 100,
        factors: { promptComplexity: 100, historicalSuccessRate: 100, currentSystemLoad: 100, agentAvailability: 100, resourceBudget: 100 },
    ...
    // Weighted average
    const weights = { promptComplexity: 0.25, historicalSuccessRate: 0.30, currentSystemLoad: 0.15, agentAvailability: 0.20, resourceBudget: 0.10 };
    let overall = 0;
    ...
    this.internalMetrics.avgHealthScore = this.healthHistory.reduce((a, b) => a + b, 0) / this.healthHistory.length;
    ...
    if (factors.promptComplexity < 50) {
      recommendations.push('Consider simplifying the prompt - complex prompts have higher failure rates');
    }
    ...
    if (factors.agentAvailability < 70) {
      recommendations.push(`Some agents have open circuits - consider using ${this.suggestLowerTier(tier)} tier`);
    }
    ...
    this.logger.info('health', `Score: ${overall}/100 (${prediction})`, { factors, recommendations });
  ...
  private scorePromptComplexity(prompt: string): number {
    ...
    if (prompt.includes('complex') || prompt.includes('everything') || prompt.includes('all features')) {
      score -= 20;
    }
  ...
  setTier(tier: DegradationTier): void {
    if (tier !== this.currentTier) {
      ...
      this.logger.info('degradation', `Manual tier change: ${fromTier} → ${tier}`, { agentsDropped });
    }
  }
   ...
   * Start a child span
   */
  startChildSpan(parent: TraceSpan, operationName: string, agentId?: AgentId, phase?: BuildPhase): TraceSpan {
    if (!this.config.tracing.enabled) {
      return this.createDummySpan(operationName);
    }
  ...
  detectAnomalies(agentId: AgentId, latencyMs: number): string[] {
    ...
    if (baseline) {
      ...
      if (ratio > this.config.anomaly.latencyThreshold) {
        anomalies.push(`Latency anomaly: ${agentId} took ${latencyMs}ms (${ratio.toFixed(0)}% of baseline ${baseline}ms)`);
    ...
    if (memoryMB > this.config.anomaly.memoryThreshold) {
      anomalies.push(`Memory anomaly: ${memoryMB.toFixed(0)}MB exceeds threshold ${this.config.anomaly.memoryThreshold}MB`);
...
function validateBuildId(buildId: string): void {
  ...
  if (!BUILDID_PATTERN.test(buildId)) {
    throw new Error(`Invalid buildId "${buildId.substring(0, 20)}...". Must match pattern: alphanumeric, underscore, hyphen, max 128 chars`);
  }
}

export function getResilienceEngine(buildId: string, options?: ResilienceEngineOptions): ResilienceEngine {
  ...
  if (!engines.has(buildId) && engines.size >= MAX_CONCURRENT_ENGINES) {
    throw new Error(`Engine limit (${MAX_CONCURRENT_ENGINES}) reached. Cannot create new engine. Possible resource exhaustion attack.`);
  }
