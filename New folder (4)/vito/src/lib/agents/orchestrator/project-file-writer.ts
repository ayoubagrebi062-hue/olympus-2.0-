...
export async function writeProjectFiles(config: FileWriterConfig): Promise<FileWriterResult> {
  const { buildId, outputPath, cleanBeforeWrite = false, dryRun = false, agentOutputs: providedOutputs } = config;
  ...
  try {
    ...
    if (providedOutputs && providedOutputs.size > 0) {
      ...
      console.log(`[FileWriter] Using ${agentOutputs.size} provided agent outputs (no DB required)`);
    } else {
    ...
    for (const [agentId, output] of agentOutputs) {
      ...
      for (const artifact of output.artifacts) {
        ...
        if (fileMap.has(filePath)) {
          console.log(`[FileWriter] File ${filePath} updated by ${agentId} (was from ${fileMap.get(filePath)!.source})`);
        }
    ...
    for (const [relativePath, { content, source }] of fileMap) {
      ...
      try {
        ...
        console.log(`[FileWriter] ${dryRun ? 'Would write' : 'Wrote'}: ${relativePath} (${source})`);
      } catch (writeError) {
    ...
    console.log(`[FileWriter] Complete. Files written: ${result.filesWritten}, Errors: ${result.errors.length}`);
