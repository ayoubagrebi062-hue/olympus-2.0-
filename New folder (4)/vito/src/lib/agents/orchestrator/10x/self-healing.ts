...
export class FallbackRegistry {
  ...
  getAvailableAgent(primaryAgentId: string, errorCode?: string): string | null {
    ...
    const sortedFallbacks = [...config.fallbacks].sort(
...
export interface HealthAlert {
  ...
  type: 'high_latency' | 'high_error_rate' | 'circuit_open' | 'degrading_trend' | 'predicted_failure';
  message: string;
  timestamp: Date;
    ...
    try {
      ...
      const result = await circuitBreaker.execute(async () => {
        return await retryStrategy!.execute(execute, (ctx) => {
          this.eventStore.append(buildId, 'AGENT_RETRIED', {
            agentId,
            attempt: ctx.attempt,
      ...
      if (this.config.autoFallback) {
        ...
        if (fallbackAgentId && fallbackAgentId !== agentId) {
          ...
          await this.eventStore.append(buildId, 'AGENT_CIRCUIT_OPENED', {
            agentId,
