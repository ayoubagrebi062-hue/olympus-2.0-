...
export interface RiskFactor {
  type: 'high_failure_agent' | 'complex_dependencies' | 'resource_intensive' | 'novel_pattern' | 'time_sensitive';
...
export class PredictionEngine {
  ...
  async refinePrediction(
    ...
    const remainingEstimate =
      currentPrediction.estimatedDuration.expected * 0.7 * performanceRatio;

    // Recalculate failure probability based on actual failures
    const actualFailures = Array.from(completedAgents.values()).filter(
      (a) => !a.success
    ).length;
    ...
    return {
      ...
      estimatedDuration: {
        ...
        confidence: Math.min(
          0.95,
          currentPrediction.estimatedDuration.confidence + 0.1
        ),
      },
      failureProbability: {
  ...
  async detectAnomalies(buildId: string): Promise<AnomalyDetection> {
    ...
    const projectType = (createEvent?.payload as { projectType?: string })?.projectType || 'unknown';
  ...
  private generateRecommendations(
    ...
    if (resourceEstimate.estimatedCost > stats.averageCost * 1.5) {
      recommendations.push({
        ...
        description: 'Estimated cost is significantly above average. Consider caching or optimizing prompts.',
  ...
  private calculateStats(builds: any[], projectType: string): HistoricalStats {
    ...
    for (const build of builds) {
      ...
      for (const [phaseId, duration] of Object.entries(phaseDurations)) {
        ...
        existing.averageDuration =
          (existing.averageDuration + (duration as number)) / 2;
      ...
      for (const [agentId, perf] of Object.entries(agentPerf)) {
        ...
        existing.averageDuration =
          (existing.averageDuration + p.duration) / 2;
        existing.averageQuality =
          (existing.averageQuality + (p.quality || 0)) / 2;
    ...
    const recentAvgDuration = recentBuilds.length > 0
      ? recentBuilds.reduce((s, b) => s + b.total_duration, 0) / recentBuilds.length
      : avgDuration;
    const olderAvgDuration = olderBuilds.length > 0
    ...
    return {
      ...
      averageCost:
        builds.reduce((s, b) => s + (b.total_cost || 0), 0) / Math.max(builds.length, 1),
      phaseStats,
      agentStats,
      trends: {
        durationTrend,
  ...
  private calculateBuildMetrics(events: BuildEvent[]): {
    ...
    const agentPerformance = new Map<string, { duration: number; success: boolean; quality: number }>();
...
export class PatternRecognizer {
  ...
  analyze(events: BuildEvent[]): RecognizedPattern[] {
    ...
    if (agentStarts.length > 0 && agentCompletes.length > 0) {
      ...
      const avgLaterDuration = laterCompletes.length > 0
        ? laterCompletes.reduce((sum, e, i) => {
            const start = agentStarts.find(
              (s) => (s.payload as { agentId: string }).agentId === (e.payload as { agentId: string }).agentId
            );
