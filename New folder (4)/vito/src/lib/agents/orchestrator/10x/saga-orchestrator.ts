...
export class SagaOrchestrator extends EventEmitter {
  ...
  private executionHandlers: Map<string, (step: SagaStep, context: SagaContext) => Promise<unknown>> = new Map();
  ...
  private async executeAsync(instance: SagaInstance, definition: SagaDefinition): Promise<void> {
    try {
      ...
      await Promise.race([
        this.executeSteps(instance, definition),
        timeoutPromise,
      ]);
  ...
  private async executeSteps(instance: SagaInstance, definition: SagaDefinition): Promise<void> {
    ...
    for (const stepId of executionOrder) {
      ...
      const depsComplete = step.dependencies.every(
        (dep) => instance.completedSteps.includes(dep)
      );
  ...
  private async executeBarrierStep(
    ...
    while (Date.now() - startTime < timeout) {
      const allComplete = step.barrierSteps.every(
        (s) => instance.completedSteps.includes(s)
      );
