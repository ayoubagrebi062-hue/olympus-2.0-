...
async function testSelfHealing(): Promise<void> {
  ...
  const timeoutActions = engine.analyzeFailure('oracle', 'Request timed out after 30s', { prompt: 'test' });
  ...
  const rateLimitActions = engine.analyzeFailure('oracle', 'Rate limit exceeded (429)', { prompt: 'test' });
  assert(rateLimitActions.length > 0, 'Rate limit produces healing actions');
  assert(
    rateLimitActions.some(a =>
      ['SIMPLIFY_PROMPT', 'REDUCE_SCOPE', 'SWITCH_MODEL', 'ADD_CONTEXT', 'RETRY_WITH_CACHE'].includes(a.type)
    ),
    'Suggests valid action for rate limit'
  );

  // Test quality failure - could suggest ADD_CONTEXT or SIMPLIFY_PROMPT
  const qualityActions = engine.analyzeFailure('oracle', 'Quality score below threshold', { prompt: 'test' });
  assert(
    qualityActions.some(a =>
      a.type === 'ADD_CONTEXT' || a.type === 'SIMPLIFY_PROMPT' || a.type === 'REDUCE_SCOPE'
    ),
...
export async function runResilienceDemo(): Promise<{ passed: number; failed: number }> {
  ...
  console.log('\n' + '═'.repeat(70));
  console.log(`  RESULTS: \x1b[32m${testsPassed} passed\x1b[0m, \x1b[31m${testsFailed} failed\x1b[0m`);
  console.log('═'.repeat(70) + '\n');
