  ...
  try {
    ...
    if (quality.score < minQuality && !isContinuation) {
      ...
      if (retryQuality.score > quality.score) {
        ...
        emit({ type: "success", latency: Date.now() - startTime, quality: retryQuality.score, tokens: tokens + retryTokens });
...
function assessQuality(code: string, task: string): Quality {
  ...
  const checks = {
    hasTypes: /:\s*(string|number|boolean|void|Promise|Array|Record|React\.|HTMLElement|\[\]|<.*>)/i.test(code),
    ...
    hasErrorHandling: /try\s*\{|catch\s*\(|\.catch\(|throw\s+new/.test(code) || !needsErrorHandling(task),
...
async function processPendingQueue(): Promise<void> {
  ...
  for (const [key, item] of Array.from(pending.entries())) {
    ...
    try {
      const { content } = await callAPIStreaming(item.task, undefined, undefined, () => {}, undefined);
