...
export class RealConductor extends EventEmitter {
  ...
  async *buildStream(request: BuildRequest): AsyncGenerator<BuildStreamEvent> {
    ...
    try {
      ...
      for (const phase of phases) {
        ...
        const phaseResult = await this.executePhaseWithStreaming(state, phase, async function* (event) {
          yield event;
        });
  ...
  private buildAgentInput(
    ...
    variables.existingFiles = state.request.projectContext.existingFiles
      .map(f => `${f.path}:\n${f.content}`)
      .join('\n\n---\n\n');
    variables.styleGuide = state.request.projectContext.styleGuide || 'Follow standard TypeScript/React best practices';
