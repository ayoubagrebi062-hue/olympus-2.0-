  ...
  async enqueue(
    ...
    if (this.queue.length >= this.maxQueueSize) {
      throw new QueueFullError(
        `Queue full (${this.maxQueueSize} pending). Try again in a moment.`
      );
    }
    ...
    if (this.requestTimestamps.length >= this.requestsPerMinute) {
      ...
      throw new RateLimitError(
        `Rate limit reached. Try again in ${waitTime}s.`
      );
    }
  ...
  async build(task: string, context?: string): Promise<HardenedResult> {
    ...
    try {
      const result = await this.queue.enqueue(
        sanitizedTask.clean,
        sanitizedContext.clean,
        (t, c) => this.executeWithProtection(t, c, threats)
      );
...
export function getHardenedVito(config?: ConstructorParameters<typeof HardenedVito>[0]): HardenedVito {
  if (!instance || config) {
