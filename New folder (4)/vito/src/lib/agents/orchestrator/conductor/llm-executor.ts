...
export class OutputParser {
  ...
  parseCode(content: string, language?: string): ParsedOutput {
...
export class AgentExecutor extends EventEmitter {
  ...
  async execute(agentId: string, input: AgentInput): Promise<AgentOutput> {
    ...
    for (let attempt = 0; attempt <= agent.retryConfig.maxRetries; attempt++) {
      try {
        ...
        const isRetryable = agent.retryConfig.retryableErrors.some(
  ...
  async *executeStream(agentId: string, input: AgentInput): AsyncGenerator<{
