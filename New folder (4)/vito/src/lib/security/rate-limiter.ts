...
export const ENDPOINT_LIMITS: EndpointLimit[] = [
  ...
  { path: '/api/auth/forgot-password', method: 'POST', requests: 3, windowMs: 60 * 60 * 1000, keyBy: 'ip' },
  // Builds - Expensive
  { path: /^\/api\/ai\/builds/, method: 'POST', requests: 5, windowMs: 60 * 1000, keyBy: 'user' },
  { path: /^\/api\/builds\/[^/]+\/iterate/, method: 'POST', requests: 20, windowMs: 60 * 1000, keyBy: 'user' },
  // Deployments
  { path: /^\/api\/deployments/, method: 'POST', requests: 3, windowMs: 60 * 1000, keyBy: 'user' },
  // Storage
  { path: /^\/api\/storage\/upload/, method: 'POST', requests: 30, windowMs: 60 * 1000, keyBy: 'user' },
];
...
export const RATE_LIMIT_KEYS = {
  ...
  endpoint: (identifier: string, endpoint: string, window: number) => `ratelimit:endpoint:${endpoint}:${identifier}:${window}`,
...
export function getEffectiveLimit(endpointLimit: EndpointLimit | undefined, tier: RateLimitTier): RateLimitConfig {
...
export function createRateLimitResult(
  ...
  return {
    allowed, limit, remaining, resetAt, retryAfter,
    headers: {
...
function getRedis(): RedisClient | null {
  ...
  try {
    ...
    console.log('[rate-limiter] Redis URL detected. Using memory fallback until Redis client is implemented.');
  ...
  cleanupIntervalId = setInterval(async () => {
    ...
    try {
      ...
      if (deletedCount > 0 || memoryStore.size > MAX_MEMORY_STORE_SIZE * 0.8) {
        console.log(`[rate-limiter] Cleanup: removed ${deletedCount} expired, store size: ${memoryStore.size}`);
      }
    } catch (error) {
    ...
    async check(identifier: string): Promise<RateLimitResult> {
      ...
      try {
        ...
        if (redis) {
          ...
          const result = createRateLimitResult(legacyResult.allowed, maxRequests, legacyResult.remaining, legacyResult.resetAt.getTime());
...
function checkWithMemory(
  ...
  console.warn('[rate-limiter] Lock contention in sync checkWithMemory - consider using async version');
 ...
 */
export function createRateLimitHeaders(result: LegacyRateLimitResult, maxRequests: number = 100): Record<string, string> {
  return {
