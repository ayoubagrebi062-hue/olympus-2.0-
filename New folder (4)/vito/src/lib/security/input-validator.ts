...
const PATTERNS = {
  ...
  sqlInjection: /('|--|;|\/\*|\*\/|xp_|0x|char\s*\(|concat\s*\(|UNION[\s/\*]+SELECT|SELECT[\s/\*]+.+[\s/\*]+FROM|INSERT[\s/\*]+INTO|UPDATE[\s/\*]+.+[\s/\*]+SET|DELETE[\s/\*]+FROM|DROP[\s/\*]+TABLE|DROP[\s/\*]+DATABASE|EXEC[\s(]|EXECUTE[\s(]|WAITFOR[\s]+DELAY|BENCHMARK\s*\(|SLEEP\s*\()/i,

  // L8 fix - more comprehensive XSS patterns
  xssPatterns: /(javascript\s*:|data\s*:|vbscript\s*:|on\w+\s*=|<\s*script|<\s*img[^>]+onerror|<\s*svg[^>]+onload|<\s*body[^>]+onload|<\s*iframe|<\s*object|<\s*embed|<\s*link[^>]+href\s*=|expression\s*\(|url\s*\(\s*javascript|@import)/i,
};
...
export const uuidSchema = z
  .string()
  .refine(isValidUuid, 'Invalid UUID format');
