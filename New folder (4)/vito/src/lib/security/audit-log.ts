...
export function createAuditLogger(context: AuditContext) {
  return {
    ...
    userLogin: (method: string) =>
      auditLog('user.login', { method }, context),

    userLogout: () =>
      auditLog('user.logout', {}, context),

    buildAction: (action: 'create' | 'update' | 'delete' | 'export' | 'share', buildId: string, details: Record<string, unknown> = {}) =>
      auditLog(`build.${action}` as AuditAction, { targetId: buildId, targetType: 'build', ...details }, context),

    subscriptionAction: (action: 'create' | 'upgrade' | 'downgrade' | 'cancel', details: Record<string, unknown>) =>
