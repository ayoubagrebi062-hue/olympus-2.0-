...
export interface TimelineEvent {
  timestamp: number;
  type: 'phase_start' | 'phase_end' | 'agent_start' | 'agent_end' | 'error' | 'recovery' | 'milestone';
  title: string;
  description?: string;
  agentId?: string;
  phaseId?: string;
...
export class BuildVisualizer {
  ...
  constructor(buildId: string, config: {
    phases: Array<{ id: string; name: string; description: string; agents: Array<{ id: string; name: string; role: string }> }>;
    tokensLimit?: number;
    costLimit?: number;
  }) {
  ...
  private notifyListeners(): void {
    ...
    for (const listener of this.listeners) {
      try {
        ...
        logger.warn('Visualization listener error', { error: error instanceof Error ? error : new Error(String(error)) });
      }
    }
  }
  ...
  updateAgent(
    agentId: string,
    updates: Partial<AgentVisualization>
  ): void {
    for (const phase of this.state.phases) {
      ...
      if (agent) {
        ...
        this.state.metrics.agentsActive = this.state.phases
          .flatMap(p => p.agents)
          .filter(a => ['thinking', 'generating', 'validating'].includes(a.state))
          .length;
  ...
  getSummary(): string {
    ...
    return `Build ${this.state.buildId}: ${status} | ` +
