    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        if (data.type === 'visualization') {
          ...
          setState(prev => prev ? { ...prev, status: data.success ? 'complete' : 'failed' } : null);
          eventSource.close();
...
export function useBuildMetrics(buildId: string) {
  const { metrics, elapsedTime, estimatedRemaining, overallProgress } = useBuildVisualization({ buildId });
