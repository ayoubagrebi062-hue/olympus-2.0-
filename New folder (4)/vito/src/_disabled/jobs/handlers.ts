  ...
  async execute(payload: BuildPayload, context: JobContext): Promise<BuildResult> {
    ...
    const startResult = await buildService.start(payload.buildId, {
      onProgress: async (progress) => {
        ...
        await context.updateProgress(progress.progress, `Phase: ${progress.currentPhase || 'initializing'}`);
      ...
      onAgentComplete: async (agentId, output) => {
        await buildRealtime.publishAgentUpdate(
          payload.buildId,
          agentId,
          agentId,
          'completed',
          { progress: 100, duration: output.duration }
        );
      ...
      onError: async (error) => {
        await buildRealtime.publishLog(payload.buildId, 'error', error.message, { agent: 'orchestrator' });
      },
    });
    ...
    while (!isComplete && !context.signal.aborted) {
      ...
      if (progressResult.success && progressResult.data) {
        ...
        await context.updateProgress(progress.progress, `Phase: ${progress.currentPhase || 'processing'}`);
    ...
    context.log('info', `[BuildHandler] Build completed: ${filesGenerated} files from ${phasesCompleted} phases`);
...
export class DeployHandler extends BaseJobHandler<DeployPayload, DeployResult> {
  ...
  async execute(payload: DeployPayload, context: JobContext): Promise<DeployResult> {
    ...
    const domain = payload.environment === 'production'
...
export class CleanupOrphansHandler extends BaseJobHandler<{}, { filesDeleted: number; bytesFreed: number }> {
  ...
  async execute(_payload: {}, context: JobContext): Promise<{ filesDeleted: number; bytesFreed: number }> {
...
export class WebhookHandler extends BaseJobHandler<WebhookPayload, { status: number; responseTime: number }> {
  ...
  async execute(payload: WebhookPayload, context: JobContext): Promise<{ status: number; responseTime: number }> {
