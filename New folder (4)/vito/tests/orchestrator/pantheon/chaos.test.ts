...
const CHAOS_SCENARIOS: ChaosScenario[] = [
  {
    ...
    chaos: { enabled: true, agentFailureRate: 0.05, timeoutRate: 0.02, randomDelayMax: 50, networkFailureRate: 0.01, memoryPressure: false },
  ...
  },
  {
    ...
    chaos: { enabled: true, agentFailureRate: 0.15, timeoutRate: 0.08, randomDelayMax: 100, networkFailureRate: 0.05, memoryPressure: false },
    buildTier: 'professional',
    iterations: 50,
  ...
  },
  {
    ...
    chaos: { enabled: true, agentFailureRate: 0.30, timeoutRate: 0.15, randomDelayMax: 200, networkFailureRate: 0.10, memoryPressure: true },
    buildTier: 'professional',
    iterations: 50,
  ...
  },
  {
    ...
    chaos: { enabled: true, agentFailureRate: 0.40, timeoutRate: 0.10, randomDelayMax: 100, networkFailureRate: 0.05, memoryPressure: false },
    buildTier: 'starter',
    iterations: 50,
  ...
  },
  {
    ...
    chaos: { enabled: true, agentFailureRate: 0.05, timeoutRate: 0.05, randomDelayMax: 50, networkFailureRate: 0.40, memoryPressure: false },
    buildTier: 'professional',
    iterations: 50,
  ...
  },
  {
    ...
    chaos: { enabled: true, agentFailureRate: 0.10, timeoutRate: 0.40, randomDelayMax: 300, networkFailureRate: 0.05, memoryPressure: false },
    buildTier: 'professional',
    iterations: 50,
    expectedMinSuccessRate: 0.2,
    expectedMaxInvariantViolations: 0
  },
  {
    ...
    chaos: { enabled: true, agentFailureRate: 0.20, timeoutRate: 0.10, randomDelayMax: 100, networkFailureRate: 0.10, memoryPressure: true },
    buildTier: 'ultimate',
    iterations: 30,
    expectedMinSuccessRate: 0.3,
    expectedMaxInvariantViolations: 0
...
async function runChaosScenario(scenario: ChaosScenario): Promise<ChaosReport> {
  ...
  return {
    ...
    worstRun,
    passed: successRate >= scenario.expectedMinSuccessRate && invariantViolations <= scenario.expectedMaxInvariantViolations
  };
}
...
└──────────────────────────────────────────────────────────────────────────────┘`);
        ...
        expect(report.invariantViolations).toBeLessThanOrEqual(scenario.expectedMaxInvariantViolations);
  ...
  describe('Resilience Patterns', () => {
    ...
    it('RESILIENCE: Retry mechanism works under pressure', async () => {
      const config: BuildConfig = {
        ...
        tier: 'starter',
        agents: [
          { id: 'flaky', phase: 'discovery', dependencies: [], optional: false, estimatedTokens: 1000, failureRate: 0.6, minDuration: 10, maxDuration: 20 }
        ],
        maxConcurrency: 1,
  ...
  describe('Memory & Performance Under Chaos', () => {
    it('MEMORY: No memory leaks after many chaos runs', async () => {
      ...
      // Warm up
      for (let i = 0; i < 5; i++) {
        const sim = new BuildSimulator(createStandardBuildConfig('starter', i), createChaosConfig('high'));
        await sim.run();
      }
      ...
      for (let i = 0; i < runs; i++) {
        const sim = new BuildSimulator(createStandardBuildConfig('professional', i), createChaosConfig('high'));
        await sim.run();
      }
