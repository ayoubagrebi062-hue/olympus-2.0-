...
function buildTimeline(events: SimEvent[]): TimelineEntry[] {
  ...
  const agentStarts = new Map<AgentId, { sequence: number; time: number; phase: BuildPhase; retries: number }>();
...
function generateMermaidDiagram(events: SimEvent[], finalState: BuildSnapshot): string {
  ...
  // Phase transitions
  const phases: BuildPhase[] = ['discovery', 'architecture', 'implementation', 'quality', 'deployment'];
...
function generateGanttChart(timeline: TimelineEntry[], baseTime: number): string {
  ...
  for (const [phase, entries] of phaseGroups) {
    ...
    for (const entry of entries) {
      ...
      const status = entry.status === 'completed' ? '' : entry.status === 'failed' ? 'crit, ' : 'active, ';
      const label = entry.retries > 0 ? `${entry.agentId} (${entry.retries} retries)` : entry.agentId;
...
function generateHTML(report: DebugReport): string {
  ...
  const eventRows = report.events.map((event) => {
    ...
    const dataStr = event.data ? escapeHtml(JSON.stringify(event.data, null, 0).slice(0, 80)) : '';
  ...
  const timelineItems = report.timeline.map(entry => {
    ...
    const color = entry.status === 'completed' ? phaseColors[entry.phase] :
...
export class VisualDebugger {
  ...
  generateReport(): DebugReport {
    ...
    const partialReport = {
      ...
      mermaidDiagram: finalState ? generateMermaidDiagram(this.events, finalState) : 'stateDiagram-v2\n  [*] --> Empty',
    };
