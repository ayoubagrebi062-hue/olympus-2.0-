...
function updateModel(model: StatisticalModel, value: number): StatisticalModel {
  ...
  const newStdDev = Math.sqrt(
    ((model.samples - 1) * model.stdDev * model.stdDev +
      (value - model.mean) * (value - newMean)) /
...
export const TEMPORAL_PATTERNS: TemporalPattern[] = [
  {
    ...
    violation: (events) => {
      ...
      for (const event of events) {
        ...
        } else if ((event.type === 'AGENT_COMPLETED' || event.type === 'AGENT_FAILED') && event.agentId) {
...
export class TestOracle {
   ...
   */
  getBaselines(): { duration: Map<string, StatisticalModel>; throughput: Map<string, StatisticalModel> } {
    return {
  ...
  private buildProfile(snapshots: BuildSnapshot[], events: SimulatorEvent[]): ExecutionProfile {
    ...
    if (snapshots.length > 0 && currentPhase) {
      ...
      phaseBreakdown.set(
        currentPhase,
        (phaseBreakdown.get(currentPhase) || 0) +
          (lastSnapshot.updatedAt - lastPhaseStart)
      );
    }
    ...
    const totalDuration =
      snapshots.length > 0
        ? snapshots[snapshots.length - 1].updatedAt - snapshots[0].startedAt
        : 0;
    ...
    for (const [phase, duration] of phaseBreakdown) {
      ...
      if (ratio > 0.4) {
        bottlenecks.push({
          ...
          suggestedFix: 'Consider parallelizing agents within this phase or optimizing individual agents',
        });
      }
    }
  ...
  private generateSuggestions(
    ...
    if (profile.agentUtilization < THRESHOLDS.MIN_AGENT_UTILIZATION) {
      suggestions.push(`Low agent utilization (${(profile.agentUtilization * 100).toFixed(0)}% < ${THRESHOLDS.MIN_AGENT_UTILIZATION * 100}%) - increase parallelism`);
    }

    if (profile.concurrencyEfficiency < THRESHOLDS.MIN_CONCURRENCY_EFFICIENCY) {
      suggestions.push(`Concurrency underutilized (${(profile.concurrencyEfficiency * 100).toFixed(0)}% < ${THRESHOLDS.MIN_CONCURRENCY_EFFICIENCY * 100}%) - review dependencies`);
    }

    if (profile.chaosResilience < THRESHOLDS.MIN_CHAOS_RESILIENCE) {
      suggestions.push(`Low chaos resilience (${(profile.chaosResilience * 100).toFixed(0)}% < ${THRESHOLDS.MIN_CHAOS_RESILIENCE * 100}%) - add retries and circuit breakers`);
    }
  ...
  private calculateConfidence(
    ...
    confidence -= violations.filter(v => v.severity === 'critical').length * THRESHOLDS.CRITICAL_VIOLATION_PENALTY;
