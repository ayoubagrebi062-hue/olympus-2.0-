...
interface Violation {
  ...
  violationType: 'UNAUTHORIZED_WRITE' | 'UNAUTHORIZED_ENFORCEMENT' | 'DUPLICATE_AUTHORITY' | 'MULTIPLE_WRITERS';
  details: string;
  line?: number;
}
...
function parseAuthorityRegistry(registryPath: string): { authorities: ParsedAuthority[]; readOnly: ParsedReadOnly[] } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^- /) && (line.includes('WRITE:') || line.includes('VERIFY:') || line.includes('APPEND:') || line.includes('LOCK:') || line.includes('REGISTER:') || line.includes('EXECUTE:') || line.includes('CREATE:') || line.includes('TRANSITION:') || line.includes('HALT:') || line.includes('KILL_SWITCH:') || line.includes('BUILD_CONTROL:') || line.includes('TENANT_CONTROL:') || line.includes('ESCALATE:') || line.includes('ASSESS:') || line.includes('CONTAIN:') || line.includes('POLICY:') || line.includes('QUARANTINE:') || line.includes('METRICS:') || line.includes('RECORD:') || line.includes('ROLLBACK:'))) {
    ...
    if (line.match(/^- /) && (line.includes('WRITE to') || line.includes('UPDATE:') || line.includes('DELETE:') || line.includes('MODIFY:') || line.includes('BYPASS:') || line.includes('IGNORE:') || line.includes('EXCEED:') || line.includes('TRANSITION:') || line.includes('SKIP:') || line.includes('ESCALATE:'))) {
      const action = line.replace(/^- /, '').trim();
    ...
    if (currentReadOnly !== null && line.match(/^\*\*Location:\*/)) {
      ...
      currentReadOnly.status = lines.slice(Math.max(0, i - 5), i).join('\n').includes('READ-ONLY') ? 'READ-ONLY' : 'DEAD';
    }
  }
...
function analyzeFile(filePath: string, authorities: ParsedAuthority[], readOnly: ParsedReadOnly[]): Violation[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (tableMatch && (line.includes('.insert(') || line.includes('.update(') || line.includes('.delete('))) {
...
function scanDirectory(directory: string, authorities: ParsedAuthority[], readOnly: ParsedReadOnly[]): Violation[] {
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          if (entry.name !== 'node_modules' && entry.name !== '.next' && entry.name !== 'generated-builds' && entry.name !== 'dist' && entry.name !== 'build') {
            scanRecursive(fullPath);
          }
        } else if (entry.isFile() && entry.name.endsWith('.ts')) {
          // Scan all governance, auth, security, audit related files
          if (fullPath.includes('governance') || fullPath.includes('auth') || fullPath.includes('security') || fullPath.includes('audit')) {
...
export function enforceAuthority(registryPath: string, codebasePath: string): EnforcementResult {
  ...
  // Remove duplicates
  const uniqueViolations = violations.filter((v, i, self) =>
    i === self.findIndex((v2) =>
      v.file === v2.file &&
  ...
  const hasBlockingViolations = uniqueViolations.some(v =>
  ...
  if (uniqueViolations.length > 0) {
    ...
    const blockingViolations = uniqueViolations.filter(v =>
      v.violationType === 'UNAUTHORIZED_WRITE' ||
      v.violationType === 'UNAUTHORIZED_ENFORCEMENT' ||
      v.violationType === 'DUPLICATE_AUTHORITY' ||
      v.violationType === 'MULTIPLE_WRITERS'
  ...
  console.log('\u001b[36m========================================\u001b[0m');
  console.log(`Status: ${hasBlockingViolations ? '\u001b[31mFAIL\u001b[0m' : '\u001b[32mPASS\u001b[0m'}`);
  console.log('\u001b[36m========================================\u001b[0m\n');
