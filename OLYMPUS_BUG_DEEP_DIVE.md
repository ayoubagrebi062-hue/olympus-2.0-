...
for (const result of results) {
  if (!result.success) {
    ...
    if (agent && !agent.optional) {
      ...
      logToFile(
        `[PHASE_CONTINUE] Required agent ${result.agentId} failed, but continueOnError=true`
      );
    }
  }
}
...
for (const result of results) {
  if (!result.success) {
    ...
    if (CRITICAL_AGENTS.includes(result.agentId)) {
      console.error(
        `[CRITICAL_FAIL] Agent ${result.agentId} is CRITICAL and failed - STOPPING BUILD`
      );
  ...
  if (components.length === 0) {
    ...
    return {
      ...
      error: {
        ...
        message:
          'PIXEL requires component specifications from BLOCKS agent. BLOCKS output was empty or failed.',
        agentId,
        phase,
        recoverable: false,
      },
...
if (this.tokenTracker) {
  ...
  if (remaining < budget * 0.2) {
    ...
    logToFile(
      `[TOKEN_WARNING] ${this.definition.id}: ${used}/${budget} tokens (${remaining} remaining)`
    );
  }
...
_All code locations verified against actual codebase_
_Fixes are production-ready with error handling_
