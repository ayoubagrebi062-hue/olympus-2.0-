...
const hasMultiTenant =
  constraintText?.toLowerCase().includes('multi-tenant') ||
  constraintText?.toLowerCase().includes('tenant') ||
  constraintText?.toLowerCase().includes('organization');
console.log(
  'Contains tenant/organization reference:',
  hasMultiTenant ? '✅ PASS' : '⚠️ May need ARCHON upgrade'
);
...
console.log(
  'Constraint Injection: ' +
    (constraintText ? '✅' : '⚠️ Empty (may need ARCHON structured output)')
);
