...
const kanbanRequirements: FeatureRequirement[] = [
  {
    name: 'columns',
    keywords: ['column', 'board', 'lane'],
    patterns: [/flex.*column/i],
    minOccurrences: 3,
  },
  {
    name: 'cards',
    keywords: ['card', 'task', 'item'],
    patterns: [/draggable/i],
    minOccurrences: 1,
  },
  {
    name: 'drag-drop',
    keywords: ['drag', 'drop', 'dnd'],
    patterns: [/onDrag|useDrag/],
    minOccurrences: 1,
  },
];
...
const buttonsWithoutOnClick = (artifact.content.match(/<button[^>]*>/g) || []).filter(
  btn => !btn.includes('onClick')
);
