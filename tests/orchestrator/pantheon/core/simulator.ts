  ...
  },
  {
    ...
    check: (current, previous) => {
      ...
      if (current.progress < previous.progress) {
        return {
          passed: false,
          violation: `Progress decreased from ${previous.progress} to ${current.progress}`,
        };
      }
      return { passed: true };
    },
  },
  {
    ...
    check: (current, previous) => {
      ...
      if (currIndex < prevIndex) {
        return {
          passed: false,
          violation: `Phase went backwards from ${previous.phase} to ${current.phase}`,
        };
      }
      return { passed: true };
    },
      ...
      for (const [id, agent] of current.agents) {
        ...
        for (const depId of config.dependencies) {
          ...
          if (!dep || dep.state !== 'completed') {
            return {
              passed: false,
              violation: `Agent ${id} running before dependency ${depId} completed`,
            };
          }
        }
      }
  ...
  },
  {
    ...
    check: current => {
      ...
      if (running > current.config.maxConcurrency) {
        return {
          passed: false,
          violation: `${running} agents running, max is ${current.config.maxConcurrency}`,
        };
      }
      return { passed: true };
    },
  ...
  },
  {
    ...
    check: (current, previous) => {
      ...
      if (terminalStates.includes(previous.state) && current.state !== previous.state) {
        return {
          passed: false,
          violation: `Build state changed from terminal ${previous.state} to ${current.state}`,
        };
      }
      return { passed: true };
    },
...
export function validateConfig(config: BuildConfig): void {
  ...
  if (processed !== config.agents.length) {
    ...
    const inCycle = config.agents.filter(a => inDegree.get(a.id)! > 0).map(a => a.id);
    throw new ConfigValidationError(
...
export class BuildSimulator extends EventEmitter {
  ...
  constructor(configOrSeed?: BuildConfig | number, chaosConfig?: ChaosConfig) {
    super();
    const seed =
      typeof configOrSeed === 'number' ? configOrSeed : (configOrSeed?.seed ?? Date.now());
    this.seed = seed;
  ...
  private normalizeConfig(config: BuildConfig, chaosConfig?: ChaosConfig): BuildConfig {
    ...
    const normalizedAgents = baseAgents.map(agent => {
      ...
      const derivedDuration =
        minDuration && maxDuration ? Math.round((minDuration + maxDuration) / 2) : 1000;
  ...
  private determineCurrentPhase(): BuildPhase | null {
    for (const phase of BUILD_PHASES) {
      ...
      const allComplete = phaseAgents.every(a => {
        ...
        return (
          agent &&
          (agent.state === 'completed' || agent.state === 'skipped' || agent.state === 'failed')
        );
      });
  ...
  private updateProgress(): void {
    ...
    const computed = required.length > 0 ? Math.round((completed / required.length) * 100) : 100;
...
export function createChaosConfig(intensity: 'low' | 'medium' | 'high' | 'extreme'): ChaosConfig {
  const configs: Record<string, ChaosConfig> = {
    ...
    high: { enabled: true, failureRate: 0.3, delayRate: 0.1, maxDelay: 200 },
    extreme: { enabled: true, failureRate: 0.5, delayRate: 0.2, maxDelay: 500 },
  };
