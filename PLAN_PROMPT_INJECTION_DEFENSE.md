...
export const JAILBREAK_SIGNATURES = [
  ...
  // Filter bypass
  {
    pattern: /bypass\s+(?:your|the|any)\s+(?:filters?|restrictions?|limitations?)/i,
    severity: 'critical',
  },
...
const AI_OUTPUT_SECURITY_PATTERNS = [
  // Secret detection
  {
    ...
    pattern:
      /(?:api[_-]?key|apikey|api_secret|access_token)\s*[:=]\s*['"][a-zA-Z0-9_\-]{20,}['"]/gi,
    severity: 'error',
  ...
  },
  {
    name: 'keylogger',
    pattern:
      /(?:keylogger|keystroke\s*(?:logger|capture)|document\.onkey(?:down|up|press).*(?:fetch|XMLHttpRequest|sendBeacon))/i,
    severity: 'critical',
  ...
  },
  {
    ...
    pattern:
      /(?:navigator\.sendBeacon|fetch|XMLHttpRequest).*(?:localStorage|sessionStorage|document\.cookie|password|credit)/i,
    severity: 'high',
  ...
  // Dangerous external connections
  {
    ...
    pattern:
      /(?:fetch|axios|XMLHttpRequest|http\.get)\s*\(\s*['"]https?:\/\/(?!localhost|127\.0\.0\.1|api\.|cdn\.)[^'"]*['"]/,
    severity: 'warning',
...
export class CostGuardian {
  ...
  async checkAndRecord(
    ...
    if (tokens.input > config.maxInputTokens) {
      return {
        allowed: false,
        reason: `Input tokens ${tokens.input} exceeds limit ${config.maxInputTokens}`,
      };
    }
    if (tokens.output > config.maxOutputTokens) {
      return {
        allowed: false,
        reason: `Output tokens ${tokens.output} exceeds limit ${config.maxOutputTokens}`,
      };
    }
    ...
    if (config.hourlyHardStop > 0 && hourlySpend > config.hourlyHardStop) {
      return {
        allowed: false,
        reason: `Hourly spend limit exceeded ($${(hourlySpend / 100).toFixed(2)})`,
      };
    }
...
export class SpendMonitor {
  ...
  async checkForSpikes(): Promise<SpendAlert[]> {
    ...
    for (const tenant of tenants) {
      ...
      // Alert if 3x average
      if (currentHourSpend > avgHourSpend * 3 && currentHourSpend > 100) {
        // Min $1
        alerts.push({
          tenantId: tenant.id,
          type: 'spike',
...
const MALWARE_PATTERNS: SecurityPattern[] = [
  // Crypto miners
  {
    ...
    pattern:
      /(?:coinhive|cryptoloot|minero|webminer|coin-hive|crypto-loot|jsecoin|coinimp|minr\.bz)/gi,
    severity: 'critical',
  },
  {
    name: 'mining-script',
    pattern: /new\s+(?:CoinHive|Miner|WebMiner)\s*\(/gi,
    severity: 'critical',
  },
  ...
  // Keyloggers
  {
    ...
    pattern: /document\.(?:onkeydown|onkeyup|onkeypress|addEventListener\s*\(\s*['"]key)/gi,
    severity: 'warning',
  },
  {
    name: 'keylogger-exfil',
    pattern: /(?:fetch|XMLHttpRequest|sendBeacon)[\s\S]{0,200}(?:keyCode|which|key)/gi,
    severity: 'critical',
  },
  ...
  {
    name: 'cookie-steal',
    pattern: /document\.cookie[\s\S]{0,50}(?:fetch|XMLHttpRequest|sendBeacon|src\s*=)/gi,
    severity: 'critical',
  },
  {
    name: 'storage-exfil',
    pattern: /(?:localStorage|sessionStorage)\.getItem[\s\S]{0,100}(?:fetch|XMLHttpRequest)/gi,
    severity: 'high',
  },
  ...
  {
    name: 'reverse-shell',
    pattern: /(?:spawn|exec)\s*\(\s*['"](?:bash|sh|cmd|powershell)/gi,
    severity: 'critical',
  },
...
_Ready for implementation. Awaiting approval._
