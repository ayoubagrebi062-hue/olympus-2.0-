...
export class BuildQueue {
  ...
  async addToWaitlist(tenantId: string, userId: string, buildData: any): Promise<number> {
    ...
    const position = await this.redis.rpush(
      key,
      JSON.stringify({ userId, buildData, timestamp: Date.now() })
    );
 ...
 */
async function handleAuthenticatedBuild(
  request: NextRequest,
  authSession: any,
  ...
  if (!createResult.success || !createResult.data) {
    return NextResponse.json(
      {
        success: false,
        error: createResult.error || { code: 'CREATE_FAILED', message: 'Failed to create build' },
      },
      { status: 400 }
    );
  }
