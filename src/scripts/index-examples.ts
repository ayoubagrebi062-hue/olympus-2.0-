...
async function indexExamples(
  ...
  for (let i = 0; i < examples.length; i += BATCH_SIZE) {
    ...
    try {
      ...
      console.log(
        `  Generated ${embeddingResult.embeddings.length} embeddings via ${embeddingResult.provider}`
      );
...
async function main() {
  ...
  console.log(
    `  Ollama: ${providers.ollama ? '✓' : '✗'} ${providers.ollamaHasEmbeddingModels ? '(with embedding models)' : ''}`
  );
  ...
  const model =
    providers.defaultProvider === 'ollama' ? 'nomic-embed-text' : 'text-embedding-3-small';
