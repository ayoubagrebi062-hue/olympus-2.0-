...
export async function requestAuthorization(
  ...
  if (!rule) {
    return createDenial(requestId, request, 'Unknown action type', [
      `Action type '${request.actionType}' is not recognized`,
    ]);
  }
  ...
  const expiresAt = rule.timeLimitMs ? new Date(Date.now() + rule.timeLimitMs).toISOString() : null;
...
export async function restoreTrust(buildId: string, verifierId: string): Promise<TrustLevel> {
  ...
  if (!verification.valid) {
    throw new Error(
      `Cannot restore trust: Chain verification failed at sequence ${verification.brokenAt}`
    );
  }
