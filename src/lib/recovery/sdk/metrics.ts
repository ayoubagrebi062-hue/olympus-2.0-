...
export class ConsoleExporter implements MetricsExporter {
  ...
  onTimeout(data: { operation: string; timeout: number; timestamp: number }): void {
    console.warn(`${this.prefix} TIMEOUT ${data.operation} after ${data.timeout}ms`);
  }
}
...
export class PrometheusExporter implements MetricsExporter {
  private operationTotal: PromCounter<'operation' | 'status' | 'error_code' | 'fallback'> | null =
    null;
  ...
  constructor(register: PromRegistry) {
    ...
    const registryConstructor = (register as unknown as { constructor: Record<string, unknown> })
      .constructor;
    const Counter = registryConstructor?.Counter as
      | (new (config: unknown) => PromCounter)
      | undefined;
    const Histogram = registryConstructor?.Histogram as
      | (new (config: unknown) => PromHistogram)
      | undefined;
 ...
 * ```
 */
export function named<T>(operationName: string, fn: () => T | Promise<T>): () => T | Promise<T> {
