 ...
 * ```
 */
export async function timeout<T>(fn: () => T | Promise<T>, ms: number): Promise<T> {
...
export async function withRetry<T>(
  ...
  const backoffMultiplier = validatePositiveNumber(
    rawMultiplier,
    'backoffMultiplier',
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    if (signal?.aborted) {
      ...
      metrics.emitFailure({
        ...
        labels: {
          operation: operationName,
          error_code: RecoveryErrorCode.CANCELLED,
          trace_id: traceId,
        },
        timestamp: Date.now(),
        traceId,
...
export async function withTimeout<T>(
  ...
  try {
    ...
    return Err(
      createRecoveryError(RecoveryErrorCode.UNKNOWN, `Operation failed: ${errorMessage}`, {
...
export function withCircuitBreaker(
  ...
  return async function <T>(operation: () => T | Promise<T>): Promise<Result<T, RecoveryError>> {
    const startTime = Date.now();
