...
export async function hedge<T>(
  ...
  const { hedgeDelay, maxHedges, cancelOnSuccess, adaptiveHedging, percentileThreshold } =
    validated;
        ...
        new Promise<void>(resolve => {
          const timer = setTimeout(() => {
            ...
            if (!abortControllers[0].signal.aborted) {
              entries.push({
                index: hedgeIndex,
                promise: executeWithAbort(
                  operation,
                  abortControllers[hedgeIndex].signal,
                  hedgeIndex
                ),
...
async function raceWithIndex<T>(entries: RaceEntry<T>[]): Promise<{ value: T; index: number }> {
  return new Promise((resolve, reject) => {
    ...
    const processEntry = (entry: RaceEntry<T>) => {
      entry.promise.then(value => tryResolve(value, entry.index)).catch(tryReject);
    };
