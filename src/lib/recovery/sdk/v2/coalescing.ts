...
export class RequestCoalescer<T> {
   ...
   */
  async execute(key: string, operation: () => T | Promise<T>): Promise<Result<T, Error>> {
  ...
  getStats(): CoalescerStats {
    ...
    return {
      ...
      dedupRatio: totalRequests > 0 ? this.stats.dedupedRequests / totalRequests : 0,
      cacheHitRatio: totalCacheAccess > 0 ? this.stats.cacheHits / totalCacheAccess : 0,
    };
  }
