 ...
 */
export function validateTimeout(field: string, value: unknown, defaultValue: number): number {
 ...
 */
export function validateProbability(field: string, value: unknown, defaultValue: number): number {
  if (value === undefined || value === null) {
    return defaultValue;
  }
...
export function validateOperation<T>(
  operation: unknown
): asserts operation is () => T | Promise<T> {
...
export function validateBulkheadConfig(
  ...
  return {
    ...
    maxConcurrent: validatePositiveInt(
      'maxConcurrent',
      config.maxConcurrent,
      LIMITS.MIN_CONCURRENT,
...
export function validateCoalescerConfig(
  ...
  return {
    ...
    errorCacheTtlMs: validatePositiveInt(
      'errorCacheTtlMs',
    ...
    maxCacheSize: validatePositiveInt(
      'maxCacheSize',
      config.maxCacheSize,
      1,
...
export function validateHedgeConfig(config: Record<string, unknown> = {}): ValidatedHedgeConfig {
  const hedgeDelay = validatePositiveInt(
    'hedgeDelay',
    config.hedgeDelay,
    1,
    LIMITS.MAX_TIMEOUT,
  ...
  return {
    ...
    percentileThreshold: validateProbability(
      'percentileThreshold',
