...
export class FallbackChain<T> {
   ...
   */
  add(name: string, operation: () => T | Promise<T>, options: FallbackOptions = {}): this {
    this.entries.push({
      name,
      operation,
      options: {
  ...
  async execute(): Promise<Result<ChainResult<T>, Error>> {
    ...
    return Err(
      new Error(`All fallbacks failed [trace: ${traceId}]. Attempted: ${attempted.join(' → ')}`)
    );
  }
  ...
  private getOverallDegradationLevel(
    healthyCount: number
  ): 'none' | 'partial' | 'severe' | 'emergency' {
