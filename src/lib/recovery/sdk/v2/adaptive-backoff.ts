...
export class AdaptiveBackoff {
  ...
  recordOutcome(success: boolean, responseTimeMs: number, delayUsed: number): void {
    ...
    if (success) {
      ...
      this.optimalDelay =
        this.optimalDelay * (1 - this.config.learningRate) + newOptimal * this.config.learningRate;
    } else {
