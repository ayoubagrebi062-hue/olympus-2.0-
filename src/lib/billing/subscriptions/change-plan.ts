...
export async function changePlan(
  ...
  const resolvedParams: ChangePlanParams =
  ...
  const changeType = isUpgrade(currentTier, newTier)
    ? 'upgrade'
  ...
  const upcomingInvoice = await (stripe.invoices as any)
    .retrieveUpcoming({
