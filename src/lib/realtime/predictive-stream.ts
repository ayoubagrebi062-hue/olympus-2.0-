...
export class PredictiveStreamManager {
  ...
  async playRewind(speed: number = 1): Promise<void> {
    ...
    while (this.rewindState.isPlaying && this.rewindState.currentFrame < this.frames.length - 1) {
  ...
  private updateGhostText(file: CodeFile, cursor: Cursor): void {
    ...
    const ghost = this.predictor.predict(file.content, cursor.position, file.language, context);
