...
import type { User, Cursor, ChatMessage, CollaborationEvent } from '../collaboration';
...
export function useCollaboration(options: UseCollaborationOptions): CollaborationHookResult {
  const {
    buildId,
    user,
      ...
      try {
        ...
        switch (eventType) {
          ...
          case 'state': {
            ...
            const statePerms = state.permissions as
              | { canEdit?: string[]; canControl?: string[] }
              | undefined;
            setPermissions({
  ...
  const grantPermission = useCallback(
    (userId: string, permission: 'edit' | 'control') => {
