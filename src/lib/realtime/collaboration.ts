...
export class CollaborationSession {
  ...
  constructor(sessionId: string, buildId: string, owner: User) {
    this.state = {
      sessionId,
      buildId,
   ...
   */
  grantPermission(granterId: string, userId: string, permission: 'edit' | 'control'): void {
   ...
   */
  revokePermission(revokerId: string, userId: string, permission: 'edit' | 'control'): void {
  ...
  private emit(event: CollaborationEvent): void {
    for (const listener of this.listeners) {
      try {
        ...
        logger.warn('Collaboration event listener error', {
  ...
  getState(): Omit<
    CollaborationState,
    'participants' | 'cursors' | 'selections' | 'permissions'
  > & {
    participants: User[];
    cursors: Cursor[];
