...
function handleStartBuild(
  ...
  const tenantId =
    (parameters as unknown as { tenantId?: string }).tenantId ||
...
function handleResolveGate(
  ...
  if (decision === 'approve') {
    ...
    const currentPhaseIndex = PHASES.findIndex(
      p => room.phases[p].status === 'running' || room.phases[p].status === 'completed'
    );
...
function completePhaseAndOpenGate(room: Room, phase: PhaseName): void {
  Authority.phaseComplete(
    room.buildId,
    phase,
    room.phases[phase].tokensUsed,
    Date.now() - new Date(room.phases[phase].startedAt!).getTime()
  );
    ...
    options: isLastPhase
      ? [
          ...
          },
          {
            id: 'iterate',
            label: 'Iterate',
            description: 'Run another build pass',
            destructive: false,
          ...
          },
          {
            id: 'discard',
            label: 'Discard',
            description: 'Delete build permanently',
            destructive: true,
            requiresConfirmation: true,
          },
        ]
  ...
  appendOutput(
    ...
    'system',
    null,
    `Gate opened: ${isLastPhase ? 'Ship gate' : `Phase ${phase} gate`}`
  );
}
