...
export class TimeTravelEngine {
  ...
  recordEvent(
    ...
    const fullEvent: TimelineEvent = {
      ...
      ...event,
      parentEventId:
        timeline.events.length > 0 ? timeline.events[timeline.events.length - 1].id : undefined,
      metadata: {},
    };
  ...
  mergeBranch(
    buildId: string,
    branchId: string,
    strategy: 'keep_all' | 'keep_best' | 'manual'
  ): DiffResult {
    const timeline = this.timelines.get(buildId);
    if (!timeline) {
      throw new Error(`No timeline found for build: ${buildId}`);
  ...
  diffBranches(buildId: string, branchId1: string, branchId2: string): DiffResult {
    ...
    for (const agentId of allAgents) {
      ...
      if (inBranch1 && inBranch2) {
        const output1 = events1.find(e => e.agentId === agentId && e.type === 'agent_end')?.data
          .output;
        const output2 = events2.find(e => e.agentId === agentId && e.type === 'agent_end')?.data
          .output;
    ...
    // Calculate time diff
    const time1 =
      events1.length > 0 ? events1[events1.length - 1].timestamp - events1[0].timestamp : 0;
    const time2 =
      events2.length > 0 ? events2[events2.length - 1].timestamp - events2[0].timestamp : 0;
  ...
  async runWhatIfScenario(
    ...
    try {
      ...
      // Calculate diff
      const originalBranch = timeline.branches.find(b => b.id === branch.parentBranchId)!;

      const diff = this.diffBranches(scenario.buildId, originalBranch.id, branch.id);
  ...
  importTimeline(data: string): BuildTimeline {
    ...
    try {
      ...
      throw new Error(
        `Invalid timeline data: JSON parse failed - ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  ...
  private generateRecommendation(diff: DiffResult): string {
    ...
    return recommendations.length > 0 ? recommendations.join('. ') : 'No significant differences';
  }
}
