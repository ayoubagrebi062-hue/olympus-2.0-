...
export class DependencyGraphBuilder {
  ...
  buildGraph(agents: AgentNode[]): ExecutionGraph {
    ...
    // Calculate estimates
    const estimatedDuration = parallelGroups.reduce((sum, g) => sum + g.estimatedDuration, 0);
   ...
   */
  private findCriticalPath(nodes: Map<string, AgentNode>, executionOrder: string[]): string[] {
  ...
  private buildParallelGroups(
    ...
    while (assigned.size < nodes.size) {
      ...
      for (const id of executionOrder) {
        ...
        const node = nodes.get(id)!;
        const depsCompleted = node.dependencies.every(d => completed.has(d) || !nodes.has(d));
...
export class ParallelExecutor {
  ...
  constructor(executor: AgentExecutor, config: Partial<ParallelExecutorConfig> = {}) {
    ...
    try {
      ...
      for (const group of graph.parallelGroups) {
        ...
        for (const [agentId, result] of groupResults) {
          ...
          this.state.pendingAgents = this.state.pendingAgents.filter(id => id !== agentId);
        }
  ...
  private async executeGroup(
    ...
    const executeWithSemaphore = async (agentId: string): Promise<void> => {
      ...
      try {
        ...
        results.set(agentId, result);
        this.state.runningAgents = this.state.runningAgents.filter(id => id !== agentId);
   ...
   */
  private async executeAgent(agentId: string, context: ExecutionContext): Promise<ExecutionResult> {
  ...
  getStats(): ExecutionStats {
    ...
    const totalAgents = this.state.completedAgents.length + this.state.failedAgents.length;

    return {
      sequentialDuration,
      parallelDuration,
      speedup: sequentialDuration / Math.max(1, parallelDuration),
      efficiency: sequentialDuration / (parallelDuration * this.config.maxConcurrency),
      tokensUsed: this.state.totalTokens,
      successRate: totalAgents > 0 ? this.state.completedAgents.length / totalAgents : 0,
    };
  }
