...
export class LeonardoClient {
  ...
  constructor(config: Partial<LeonardoConfig> = {}) {
    ...
    if (!this.config.apiKey) {
      throw new Error(
        'LEONARDO_API_KEY is required. Set it in .env.local or pass it to the constructor.'
      );
    }
  }
  ...
  async getGeneration(generationId: string): Promise<{ generations_by_pk: GenerationStatus }> {
    return this.request<{ generations_by_pk: GenerationStatus }>(`/generations/${generationId}`);
  }
