...
class CostAnomalyDetector {
  ...
  private generateDescription(type: CostAnomaly['type'], event: CostEvent, zScore: number): string {
  ...
  private generateRecommendation(type: CostAnomaly['type'], event: CostEvent): string {
    switch (type) {
      case 'spike':
...
class BudgetManager extends EventEmitter {
  ...
  createBudget(
    config: Omit<Budget, 'id' | 'spent' | 'remaining' | 'alertsSent' | 'createdAt' | 'updatedAt'>
  ): Budget {
    const budget: Budget = {
      ...config,
  ...
  private handleBudgetExhaustion(budget: Budget): {
    budget: Budget;
    action: string;
    details: string;
  } {
...
class CostOptimizer {
  ...
  findCheaperAlternative(
    ...
    for (const [, model] of MODEL_PRICING) {
      ...
      // Check capabilities
      const hasCapabilities = [...requiredCapabilities].every(cap => model.capabilities.has(cap));
      ...
      if (alternative) {
        ...
        if (savingsPercent > 20) {
          recommendations.push({
            ...
            risk: 'low',
            qualityImpact:
              pricing.qualityScore - alternative.qualityScore > 0.05 ? 'moderate' : 'minimal',
            autoApplicable: true,
          });
        }
      }
...
class ROICalculator {
  ...
  calculateROI(featureId: string, featureName: string): ROIMetrics {
    ...
    const estimatedValuePerUse = 0.1; // $0.10 per successful use
...
export class CostIntelligenceEngine extends EventEmitter {
   ...
   * Create a budget
   */
  createBudget(name: string, limit: number, options: Partial<Budget> = {}): Budget {
  ...
  generateDashboard(periodMs: number = 24 * 60 * 60 * 1000): CostDashboard {
    ...
    return {
      ...
      budgetStatus,
      projections: [this.projectCosts('hour'), this.projectCosts('day'), this.projectCosts('week')],
