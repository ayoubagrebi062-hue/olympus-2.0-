...
const ERROR_PATTERNS: Record<FailureType, RegExp[]> = {
  rate_limit: [/rate limit/i, /too many requests/i, /429/, /quota exceeded/i, /throttl/i],
  timeout: [/timeout/i, /timed out/i, /deadline exceeded/i, /ETIMEDOUT/],
  invalid_output: [/invalid json/i, /parse error/i, /unexpected token/i, /malformed/i],
  context_overflow: [/context length/i, /token limit/i, /maximum context/i, /too long/i],
  model_error: [/model.*error/i, /inference failed/i, /generation failed/i, /content filter/i],
  validation_error: [/validation failed/i, /schema.*invalid/i, /type.*mismatch/i],
  network_error: [/network/i, /ECONNRESET/, /ECONNREFUSED/, /fetch failed/i, /socket/i],
  unknown: [],
};
...
export class SelfHealingEngine {
   ...
   */
  async handleFailure(
    context: Omit<FailureContext, 'type' | 'timestamp'>
  ...
  getStats() {
    ...
    return {
      ...
      recoveryRate:
        this.recoveryStats.totalFailures > 0
          ? Math.round(
              (this.recoveryStats.successfulRecoveries / this.recoveryStats.totalFailures) * 100
            )
          : 100,
