  ...
  selectStrategies(
    ...
    if (autoApplyOnly) {
      candidates = candidates.filter(
        s => s.autoApply && prediction.failureProbability >= s.autoApplyThreshold
      );
    }
...
export class RuntimeFailurePredictionEngine extends EventEmitter {
    ...
    this.mitigationEngine.on('mitigation_completed', data => {
      ...
      if (prediction) {
        for (const signal of prediction.signals) {
          const pattern = this.patternDetector
            .getPatterns()
  ...
  private evaluatePredictions(): void {
    ...
    for (const match of matches) {
      ...
      const existingPrediction = Array.from(this.activePredictions.values()).find(p =>
        p.signals.some(s => match.matchedSignals.some(ms => ms.metric === s.metric))
      );
  ...
  private createPrediction(match: {
    ...
    const targetId =
      match.matchedSignals[0]?.agentId || match.matchedSignals[0]?.componentId || 'system';
  ...
  private updatePrediction(
    ...
    prediction.failureProbability = 0.7 * newProbability + 0.3 * prediction.failureProbability;
  ...
  recordFailure(event: Omit<RuntimeFailureEvent, 'id' | 'wasPredicted' | 'predictionId'>): void {
    ...
    const matchingPrediction = Array.from(this.activePredictions.values()).find(
      p =>
  ...
  recordRecovery(componentId: string): void {
    ...
    this.emit('recovery_recorded', {
      componentId,
      newHealth: this.componentHealth.get(componentId),
    });
  }
  ...
  getHealthSnapshot(): HealthSnapshot {
    ...
    for (const [componentId, health] of this.componentHealth) {
      const predictions = Array.from(this.activePredictions.values()).filter(
        p => p.targetId === componentId
      );

      const recentFailures = this.failureHistory.filter(
        f =>
          (f.componentId === componentId || f.agentId === componentId) &&
          f.timestamp > Date.now() - 3600000 // Last hour
      ).length;

      const riskScore =
        predictions.length > 0 ? Math.max(...predictions.map(p => p.failureProbability)) : 0;
    ...
    const overallHealth =
      healths.length > 0 ? healths.reduce((a, b) => a + b, 0) / healths.length : 1;

    // Calculate system-wide metrics
    const recentFailures = this.failureHistory.filter(f => f.timestamp > Date.now() - 3600000);
    ...
    return {
      ...
      components,
      activeAlerts: Array.from(this.activePredictions.values()).filter(
        p => p.failureProbability > 0.7
      ),
      mitigationsInProgress: this.mitigationEngine
  ...
  private calculateSystemLoad(): number {
    const recentSignals = this.recentSignals.filter(s => s.timestamp > Date.now() - 60000);
  ...
  getAccuracyStats(): {
    ...
    for (const prediction of historicalPredictions) {
      const matchingFailure = this.failureHistory.find(f => f.predictionId === prediction.id);
...
export function isOperationSafe(operationId: string): {
  ...
  const predictions = runtimeFailurePrediction
