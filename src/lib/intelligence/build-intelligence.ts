...
export function analyzePrompt(prompt: string): PromptAnalysis {
  ...
  if (
    ...
    !prompt.includes('oauth') &&
    !prompt.includes('email')
  ) {
    clarifyingQuestions.push('What authentication method? (Email/password, OAuth, magic link)');
  }
  if (
    features.includes('data-crud') &&
    !techStack.some(t => ['prisma', 'supabase', 'firebase', 'mongodb'].includes(t))
  ) {
...
const HISTORICAL_AVERAGES: Record<string, BuildHistory> = {
  ...
  complex: { complexity: 7, scope: 7, tokens: 100000, cost: 1.0, duration: 300, success: true },
  enterprise: { complexity: 9, scope: 9, tokens: 250000, cost: 2.5, duration: 600, success: true },
};
...
export function predictBuild(
  ...
  if (analysis.ambiguity >= 6) {
    optimizations.push({
      ...
      estimatedSavings: {
        tokens: Math.round(baseTokens * 0.2),
  ...
  if (tier === 'simple') {
    optimizations.push({
      ...
      estimatedSavings: {
        cost: Math.round(estimatedCost.expected * 0.7 * 100) / 100,
 ...
 */
export function optimizeCost(prompt: string, prediction: BuildPrediction): OptimizationPlan {
  const analysis = analyzePrompt(prompt);
