...
export class KnowledgeGraphEngine {
  ...
  learnFromBuild(
    ...
    for (let i = 0; i < agents.length; i++) {
      ...
      if (i > 0) {
        const prevAgentNode = this.nodes.get(this.findNodeByName('agent', agents[i - 1])!)!;
  ...
  predictBuild(
    ...
    for (const { pattern, similarity } of similarPatterns) {
      if (pattern.type === 'failure' && similarity > 0.7) {
        risks.push(
          `High similarity (${(similarity * 100).toFixed(0)}%) to failed pattern: ${pattern.name}`
        );
      }
    }
  ...
  private createPatternSignature(features: string[], agents: string[], errors: string[]): string {
  ...
  private getNodeContext(node: KnowledgeNode): string {
    ...
    return neighborNames ? `Related to: ${neighborNames}` : `Standalone ${node.type}`;
  }
}
