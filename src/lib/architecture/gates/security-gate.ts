...
const DANGEROUS_PATTERNS = [
  ...
  },
  {
    ...
    message: 'innerHTML is dangerous - use textContent or React JSX',
  ...
  },
  {
    ...
    message: 'dangerouslySetInnerHTML requires DOMPurify sanitization',
    severity: 'warning' as const,
  },
];
...
export async function securityGate(files: FileToCheck[]): Promise<GateResult> {
  ...
  for (const file of filesToCheck) {
    ...
    if (dangerousIssues.some(i => i.rule === 'no-eval' || i.rule === 'no-new-function'))
      hasEval = true;
