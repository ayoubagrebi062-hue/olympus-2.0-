...
export const FORBIDDEN_TECHNOLOGIES = {
  ...
  databases: ['MySQL', 'SQLite', 'MongoDB', 'Firebase Firestore', 'PlanetScale', 'CockroachDB'],
  orms: [
    'TypeORM',
    'Sequelize',
    'Knex',
  ...
  styling: ['Styled Components', 'Emotion', 'CSS Modules', 'Sass/SCSS files', 'Vanilla CSS files'],
  languages: ['PHP', 'Ruby', 'Python backend', 'Java', 'Go'],
} as const;
...
export function isForbiddenTechnology(name: string): boolean {
  ...
  return allForbidden.some(f => name.toLowerCase().includes(f.toLowerCase()));
}
