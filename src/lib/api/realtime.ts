...
export function subscribeToBuild(
  buildId: string,
  callback: (e: { buildId: string; status: string; progress: number; updatedAt: string }) => void
): RealtimeChannel {
  return subscribeToTable<{ id: string; status: string; progress: number; updated_at: string }>(
    'builds',
    { column: 'id', value: buildId },
    payload => {
      if (payload.new)
        callback({
          buildId,
...
export function subscribeToDeployment(
  deployId: string,
  callback: (e: { deployId: string; status: string; url: string | null; updatedAt: string }) => void
): RealtimeChannel {
  return subscribeToTable<{ id: string; status: string; url: string | null; updated_at: string }>(
    'deployments',
    { column: 'id', value: deployId },
    payload => {
      if (payload.new)
        callback({
          deployId,
...
export async function broadcastBuildProgress(
  buildId: string,
  progress: { status: string; progress: number; message?: string; step?: string }
): Promise<void> {
...
export async function broadcastBuildUpdate(
  buildId: string,
  update: {
    status: string;
    progress?: number;
...
export async function broadcastDeployStatus(
  deployId: string,
  status: { status: string; url?: string; error?: string }
): Promise<void> {
...
export async function broadcastDeployUpdate(
  deployId: string,
  update: { status: string; progress?: number; url?: string; error?: string }
): Promise<void> {
