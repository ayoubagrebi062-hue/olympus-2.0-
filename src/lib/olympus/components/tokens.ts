...
export function phaseStateColor(state: string): string {
  return (
    TOKENS.colors.state[state as keyof typeof TOKENS.colors.state] ?? TOKENS.colors.state.pending
  );
}
...
export function severityColor(severity: string): string {
  return (
    TOKENS.colors.severity[severity as keyof typeof TOKENS.colors.severity] ??
    TOKENS.colors.severity.INFO
  );
}
