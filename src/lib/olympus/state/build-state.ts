...
export function olympusReducer(state: OlympusState, action: OlympusAction): OlympusState {
  ...
  switch (action.type) {
    ...
    case 'OUTPUT_RECEIVED': {
      ...
      const trimmedStream =
        newStream.length > maxEntries ? newStream.slice(-maxEntries) : newStream;
      return {
        ...state,
...
export const selectAgents = (state: OlympusState): readonly Agent[] =>
  Array.from(state.agents.values());
export const selectArtifacts = (state: OlympusState): readonly Artifact[] =>
...
export const selectOutputBySeverity = (
  state: OlympusState,
  severity: Severity
): readonly OutputEntry[] => state.outputStream.filter(e => e.severity === severity);
