 ...
 */
function handleStreamEvent(event: BuildStreamEvent, dispatch: React.Dispatch<OlympusAction>) {
    ...
    case 'build_started':
      ...
      dispatch({
        ...
        entry: createOutputEntry(
          'info',
          `Build started with ${event.totalAgents} agents`,
          event.buildId
        ),
      });
      break;
...
function createOutputEntry(
  ...
  return {
    ...
    timestamp: new Date().toISOString(),
    severity:
      severity || (level === 'error' ? 'BLOCKING' : level === 'warning' ? 'WARNING' : 'INFO'),
    message,
    phaseId: phaseId || '',
    agentId: agentId || '',
    agentName: agentId || '',
