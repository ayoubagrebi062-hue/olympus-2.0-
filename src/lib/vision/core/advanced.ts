...
// Applicative
export const ap =
  <T, U, E>(rf: Result<(t: T) => U, E>) =>
  (rt: Result<T, E>): Result<U, E> =>
...
export class TypeSafeCircuit<S extends CircuitState> {
  ...
  recordFailure(
    this: TypeSafeCircuit<Closed>,
    threshold: number
  ): TypeSafeCircuit<Closed> | TypeSafeCircuit<Open> {
  ...
  recordSuccess(
    this: TypeSafeCircuit<HalfOpen>,
    threshold: number
  ): TypeSafeCircuit<HalfOpen> | TypeSafeCircuit<Closed> {
  ...
  tryReset(
    this: TypeSafeCircuit<Open>,
    timeoutMs: number
  ): TypeSafeCircuit<Open> | TypeSafeCircuit<HalfOpen> {
...
export class RequestBuilder<Config = {}> {
  ...
  build(this: RequestBuilder<RequiredConfig>): {
    apiKey: ApiKey;
    prompt: Prompt;
    timeout?: number;
  } {
...
type ErrorCode = 'RATE_LIMITED' | 'INVALID_KEY' | 'TIMEOUT' | 'CIRCUIT_OPEN' | 'CONTENT_VIOLATION';
...
export const handleError = (code: ErrorCode): string => {
  switch (code) {
    ...
    case 'CONTENT_VIOLATION':
      return 'Content policy violated';
    default:
      return assertNever(code); // Compile error if any case is missing
  }
};
...
export const createPipeline = <In, Out, E>(step: Step<In, Out, E>): Pipeline<In, Out, E> => ({
  steps: [step],
  then: <Next, E2>(nextStep: Step<Out, Next, E2>) =>
    createPipeline({
        ...
        chain(
          ...
          result =>
            (result._tag === 'Ok'
              ...
              : pure(result as Result<never, Error>)) as Effect<E2, Result<Next, Error>>
        ),
