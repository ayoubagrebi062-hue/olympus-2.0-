...
async function main() {
  ...
  if (isOk(success)) log(true, `Ok: ${JSON.stringify(success.value)}`);
  if (isErr(failure)) log(true, `Err: ${failure.error.code}, retryable=${failure.error.retryable}`);
  ...
  log(
    (remainingTime(ctx) ?? 0) > 0,
    `trace=${ctx.traceId?.slice(0, 12) ?? 'none'}... remaining=${remainingTime(ctx) ?? 0}ms`
  );
  await sleep(120);
