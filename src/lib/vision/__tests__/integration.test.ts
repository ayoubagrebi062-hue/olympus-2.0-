 ...
 *
 * No mocks. Real behavior. Run: npx vitest integration.test.ts
 */
...
describe('Error Recovery Scenarios', () => {
  test('graceful degradation: partial success', async () => {
    ...
    const generateImages = async () =>
      Err(createError(VisionErrorCode.PROVIDER_ERROR, 'Image API down'));

    const [codeResult, imageResult] = await Promise.all([generateCode(), generateImages()]);
