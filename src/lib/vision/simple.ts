...
export async function visionFull(
  prompt: string,
  ...
  if (!quiet || onProgress) {
    unsub = orch.on((e: GenerationEvent) => {
      ...
      if (e.type === 'completed' && e.success)
        emit('complete', `Done ${e.durationMs}ms`, e.durationMs);
    });
  }

  try {
    const result = await orch.generate(
      { prompt, options: { generateImages: withImages } },
      { timeoutMs: timeout }
    );
    ...
    if (isErr(result)) {
      ...
      throw new Error(
        `${result.error.message}${HELP[result.error.code] ? ` — ${HELP[result.error.code]}` : ''}`
      );
    }
    ...
    return {
      ...
      images: withImages
        ? result.value.images.map(i => ({ url: i.url, prompt: i.prompt }))
        : undefined,
