 ...
 */
export function hasFeatureAccess(planTier: string, feature: keyof typeof FEATURE_FLAGS): boolean {
...
export function getCurrentPhase(): number {
  ...
  for (let i = phases.length - 1; i >= 0; i--) {
    ...
    const allEnabled = phase.features.every(f => isFeatureEnabled(f as keyof typeof FEATURE_FLAGS));
