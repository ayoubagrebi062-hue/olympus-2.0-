 ...
 */
export function extractCriticalDecisions(
  config: ArchonEnhancedOutput
...
export function formatDecisionsForPrompt(decisions: CriticalArchitectureDecisions): string {
  ...
  if (decisions.isMultiTenant) {
    ...
    lines.push(
      `- Add \`tenantId: String\` to: ${decisions.tenantScopedTables.join(', ') || 'all user-facing tables'}`
    );
  ...
  lines.push(
    `- Soft Deletes: ${decisions.softDeletes ? 'YES - add `deletedAt DateTime?`' : 'NO - use hard deletes'}`
  );
  lines.push('');
  ...
  lines.push(
    `- Response Format: ${decisions.responseEnvelope ? '`{ data: T, meta: {...} }`' : 'Direct T response'}`
  );
