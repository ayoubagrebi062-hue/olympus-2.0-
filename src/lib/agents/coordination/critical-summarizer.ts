...
export interface PageDecisions {
  ...
  /** Shared layouts */
  layouts: string[];
  /** Available components from PIXEL (wire must ONLY import these) */
...
function extractArchonDecisions(
  ...
  const parsed = safeJsonParse<Record<string, unknown>>(
    docArtifact.content,
    {},
    'critical-summarizer:archonDecisions'
  );
...
function extractPaletteDecisions(output: AgentOutput): Partial<DesignDecisions> {
  const docArtifact = output.artifacts.find(a => a.type === 'document' || a.type === 'design');
  ...
  const parsed = safeJsonParse<Record<string, any>>(
    docArtifact.content,
    {},
    'critical-summarizer:paletteDecisions'
  );
...
function extractDatumDecisions(output: AgentOutput): Partial<DataDecisions> {
  ...
  const parsed = safeJsonParse<Record<string, any>>(
    docArtifact.content,
    {},
    'critical-summarizer:datumDecisions'
  );
  ...
  const softDeleteEntities =
    parsed.tables
      ?.filter((t: any) => t.columns?.some((c: any) => c.name === 'deletedAt'))
      .map((t: any) => t.name) || [];

  const enums =
    parsed.enums?.map((e: any) => ({
      name: e.name,
      values: e.values || [],
    })) || [];
...
function extractNexusDecisions(output: AgentOutput): Partial<ApiDecisions> {
  ...
  const parsed = safeJsonParse<Record<string, any>>(
    docArtifact.content,
    {},
    'critical-summarizer:nexusDecisions'
  );

  const endpoints =
    parsed.endpoints?.map((e: any) => ({
      method: e.method || 'GET',
      path: e.path || e.url,
  ...
  const publicEndpoints = endpoints.filter((e: any) => !e.requiresAuth).map((e: any) => e.path);
...
function extractSentinelDecisions(output: AgentOutput): Partial<SecurityDecisions> {
  ...
  const parsed = safeJsonParse<Record<string, any>>(
    docArtifact.content,
    {},
    'critical-summarizer:sentinelDecisions'
  );
  ...
  return {
    ...
    auditEvents:
      parsed.security_rules?.filter((r: any) => r.type === 'audit').flatMap((r: any) => r.events) ||
      [],
  };
}
...
function extractCartographerDecisions(output: AgentOutput): Partial<PageDecisions> {
  ...
  const parsed = safeJsonParse<Record<string, any>>(
    docArtifact.content,
    {},
    'critical-summarizer:cartographerDecisions'
  );
...
/**
 * Extract available component paths from PIXEL output
 * WIRE must ONLY import components that PIXEL has generated
 */
function extractPixelComponents(output: AgentOutput): Array<{ name: string; path: string }> {
  const components: Array<{ name: string; path: string }> = [];

  // Extract from code artifacts that are in components/ directory
  for (const artifact of output.artifacts) {
    if (artifact.type === 'code' && artifact.path) {
      // Match paths like src/components/Button.tsx
      if (artifact.path.includes('components/')) {
...
export function updateCriticalDecisions(
  ...
  switch (agentId) {
    ...
    case 'cartographer':
      ...
      break;
    case 'pixel':
      // Extract available components from PIXEL so WIRE knows what it can import
      const pixelComponents = extractPixelComponents(output);
      updated.pages = {
        ...updated.pages,
        availableComponents: pixelComponents,
      };
      break;
  }
...
function formatDesignDecisions(design: Partial<DesignDecisions>): string {
  const lines: string[] = ['## DESIGN CONSTRAINTS (from PALETTE/BLOCKS)', ''];
...
function formatDataDecisions(data: Partial<DataDecisions>): string {
  const lines: string[] = ['## DATA MODEL CONSTRAINTS (from DATUM)', ''];
...
function formatApiDecisions(api: Partial<ApiDecisions>): string {
  const lines: string[] = ['## API CONSTRAINTS (from NEXUS)', ''];
...
function formatSecurityDecisions(security: Partial<SecurityDecisions>): string {
  const lines: string[] = ['## SECURITY CONSTRAINTS (from SENTINEL)', ''];
...
function formatPageDecisions(pages: Partial<PageDecisions>): string {
  const lines: string[] = ['## PAGE STRUCTURE (from CARTOGRAPHER)', ''];
  ...
  // CRITICAL: Available components from PIXEL - WIRE must ONLY import these
  if (pages.availableComponents?.length) {
    lines.push('');
    lines.push('### ⚠️ AVAILABLE COMPONENTS (from PIXEL):');
    lines.push(
      '**YOU MUST ONLY IMPORT COMPONENTS FROM THIS LIST. DO NOT IMPORT COMPONENTS THAT ARE NOT LISTED.**'
    );
    lines.push('');
    pages.availableComponents.forEach(c => {
      // Convert path to import alias: src/components/Button.tsx -> @/components/Button
      const importPath = c.path.replace(/^src\//, '@/').replace(/\.(tsx?|jsx?)$/, '');
      lines.push(`- \`${c.name}\` → import from \`${importPath}\``);
    });
    lines.push('');
    lines.push(
      '**If you need a component that is NOT in this list, you MUST create it inline in your page code.**'
    );
  } else {
    lines.push('');
    lines.push('### ⚠️ NO PIXEL COMPONENTS AVAILABLE');
    lines.push(
      '**PIXEL has not generated any components. You MUST create all components inline in your page code.**'
    );
  }
