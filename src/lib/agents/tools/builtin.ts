...
const qualityCheckTool: ToolDefinition = {
  ...
  name: 'Quality Check',
  description:
    'Run quality gates on generated code to check for TypeScript errors, ESLint violations, and security issues',
  category: 'quality',
  parameters: [
    {
      name: 'files',
...
const qualityCheckHandler: ToolHandler = async (input, context) => {
  ...
  try {
    ...
    const files = typedInput.files.map(
...
const memorySearchTool: ToolDefinition = {
  ...
  name: 'Memory Search',
  description:
    'Search the GraphRAG memory for relevant code snippets, patterns, and user preferences',
  category: 'memory',
  parameters: [
    {
      name: 'query',
...
const memorySearchHandler: ToolHandler = async (input, context) => {
  ...
  try {
    ...
    const results = await searchAllForContext(embedding, typedInput.limit || 5);
