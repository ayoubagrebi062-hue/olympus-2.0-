...
export function validateAgainstFeatureChecklist(
  ...
  const stubPatterns = [
    ...
    // Excludes: placeholder="...", placeholder='...'
    {
      ...
      message: 'Contains placeholder text in content',
    },
...
function getFeaturePatterns(featureId: string): RegExp[] {
  ...
  return words.filter(w => w.length >= 3).map(w => new RegExp(w, 'i'));
}
...
export async function generatePageWithRetry(
  ...
  return generateWithRetry(generateFn, prompt, checklist, options.agent, options.pageType);
}
