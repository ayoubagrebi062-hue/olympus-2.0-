...
export class IntelligentModelRouter {
  ...
  constructor(config: Partial<RouterConfig> = {}) {
    ...
    this.config = {
      ...
      overrides: {
        ...
        forceSonnet: [...defaultOverrides.forceSonnet, ...(config.overrides?.forceSonnet || [])],
      },
    };
  }
    ...
    if (agentId && this.config.overrides) {
      ...
      if (this.config.overrides.forceSonnet?.includes(agentId)) {
        return this.buildAnalysis(
          ...
          'sonnet',
          1.0,
          ['Agent forced to Sonnet (code generator)'],
          promptLength
        );
      }
  ...
  route(prompt: string, agentId?: string): RoutingDecision {
    ...
    const opusCost =
      ...
      (MODEL_PRICING.opus.input + MODEL_PRICING.opus.output);
