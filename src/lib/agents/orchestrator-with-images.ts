 ...
 */
function inferImageRequests(pageType: string, prompt: string): ImageRequest[] {
...
export class OrchestratorWithImages {
  ...
  async generatePage(request: PageGenerationRequest): Promise<PageGenerationResult> {
    ...
    console.log(
      `║ Include Images: ${(request.includeImages ?? this.config.enableImages).toString().padEnd(42)}║`
    );
