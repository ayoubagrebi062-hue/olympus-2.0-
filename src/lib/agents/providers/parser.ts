...
export function parseAgentResponse(
  ...
  try {
    ...
    console.log(
      `[Parser] Agent ${agentId}: parsed.files exists = ${!!parsed.files}, isArray = ${Array.isArray(parsed.files)}, length = ${parsed.files?.length || 0}`
    );
    ...
    if (parsed.files && Array.isArray(parsed.files)) {
      ...
      for (const file of parsed.files) {
        ...
        console.log(
          `[Parser] File: ${file.path}, type=${artifactType}, hasContent=${hasContent}, contentLength=${file.content?.length || 0}`
        );
        artifacts.push({
