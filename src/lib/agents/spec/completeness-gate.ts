...
export class CompletenessGate {
  ...
  check(tracker: RequirementsTracker): CompletenessGateResult {
    if (!tracker.isInitialized()) {
      return this.createFailedResult('RequirementsTracker not initialized', [
...
Build meets all spec requirements.`;
   ...
   */
  private createFailedSummary(failureReasons: string[], missing: MissingRequirements): string {
    const missingCount =
    ...
    const criticalCount = missing.criticalPages.length + missing.criticalComponents.length;
...
${this.config.blockOnFailure ? 'Build cannot proceed until requirements are met.' : 'Warning only - build will continue.'}`;
   ...
   */
  private createFailedResult(summary: string, failureReasons: string[]): CompletenessGateResult {
    return {
      passed: false,
      pageCompletion: 0,
...
export function formatGateResult(result: CompletenessGateResult): string {
  ...
  if (
    !result.passed &&
    result.regenerationInstructions &&
    result.regenerationInstructions.length > 0
  ) {
    output += '\n\n--- REGENERATION INSTRUCTIONS ---\n';
      ...
      for (const instr of critical) {
        ...
        const label =
          typeof req === 'string'
            ? req
            : 'path' in req
              ? (req as PageRequirement).path
              : (req as ComponentRequirement).name;
