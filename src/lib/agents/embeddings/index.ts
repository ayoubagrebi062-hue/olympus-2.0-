...
export async function getDefaultProvider(): Promise<EmbeddingProvider> {
  ...
  if (await isOllamaAvailable()) {
    console.warn(
      '[Embeddings] OpenAI not available, falling back to Ollama. Note: dimension mismatch may occur.'
    );
    return 'ollama';
  }
  throw new Error(
    'No embedding provider available. Configure OPENAI_API_KEY or run Ollama with embedding models.'
  );
}
...
async function getOllamaEmbeddingsBatch(
  ...
  const embeddings = await Promise.all(texts.map(text => getOllamaEmbedding(text, model)));
  return embeddings;
}
...
export class EmbeddingService {
  ...
  static async create(
    ...
    if (options.preferLocal && ollama && ollamaHasEmbeddingModels) {
      ...
      throw new Error(
        'No embedding provider available. Configure OPENAI_API_KEY or install Ollama embedding models.'
      );
    }
