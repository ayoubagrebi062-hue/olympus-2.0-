...
export class BuildIntelligenceEngine extends EventEmitter {
  ...
  private extractProjectType(events: BuildEvent[]): string {
    ...
    return (startEvent?.payload as { projectType?: string })?.projectType || 'unknown';
  }
  ...
  private analyzePhases(events: BuildEvent[]): PhaseProfile[] {
    ...
    for (const event of events) {
      if (event.type === 'PHASE_STARTED') {
        const data = event.payload as { phase: string };
        ...
        const data = event.payload as { phase: string; duration: number };
        ...
        const data = event.payload as { phase: string; agentId: string };
        const phase = phases.get(data.phase);
        if (phase) {
  ...
  private analyzeAgents(events: BuildEvent[]): AgentProfile[] {
    ...
    for (const event of events) {
      if (event.type === 'AGENT_STARTED') {
        const data = event.payload as { agentId: string; agentType: string };
        ...
        if (agent) {
          ...
          agent.performance.tokenEfficiency =
            data.tokensUsed > 0 ? (100 / data.tokensUsed) * 1000 : 0;
        }
      }
    }
  ...
  private calculateMetrics(events: BuildEvent[]): BuildMetrics {
    ...
    for (const event of events) {
      if (event.type === 'BUILD_COMPLETED') {
        const data = event.payload as { duration: number };
        totalDuration = data.duration;
      }
      if (event.type === 'AGENT_COMPLETED') {
        const data = event.payload as { tokensUsed: number; success: boolean };
        totalTokens += data.tokensUsed || 0;
        totalApiCalls++;
        if (data.success) successCount++;
        else failureCount++;
      }
      if (event.type === 'AGENT_RETRIED') {
        totalRetries++;
      }
    }
  ...
  private detectPatterns(events: BuildEvent[]): DetectedPattern[] {
    ...
    const qualityEvents = events.filter(
      e => e.type === 'QUALITY_GATE_PASSED' || e.type === 'QUALITY_GATE_FAILED'
    );
    if (qualityEvents.length > 0) {
      const scores = qualityEvents.map(
        e => (e.payload as { summary?: GateSummary })?.summary?.overallScore || 0
      );
  ...
  private findOptimizations(
    ...
    for (const bottleneck of bottlenecks.filter(b => b.automatable)) {
      opportunities.push({
        ...
        effort: 'easy',
        priority:
          bottleneck.severity === 'critical' ? 10 : bottleneck.severity === 'severe' ? 8 : 5,
      });
    }
  ...
  private buildQualityProfile(events: BuildEvent[]): QualityProfile {
    ...
    const scores = qualityEvents.map(e => {
      const data = e.payload as { summary?: GateSummary };
    ...
    // Calculate trends
    const trends: QualityProfile['trends'] = Object.entries(dimensions).map(([dim, score]) => ({
      dimension: dim,
      direction: score > overallScore ? 'improving' : score < overallScore ? 'degrading' : 'stable',
      velocity: Math.abs(score - overallScore) / overallScore,
    }));

    // Identify risk areas
    const riskAreas: QualityProfile['riskAreas'] = Object.entries(dimensions)
  ...
  private determineBuildHealth(
    ...
    if (
      severeBottlenecks > 2 ||
      metrics.overallSuccessRate < 70 ||
      qualityProfile.overallScore < 50
    )
      return 'poor';
    if (
      severeBottlenecks > 0 ||
      metrics.overallSuccessRate < 85 ||
  ...
  private generateKeyFindings(
    ...
    for (const pattern of patterns) {
      findings.push({
        ...
        evidence: [
          `Detected ${pattern.frequency} occurrences with ${(pattern.confidence * 100).toFixed(0)}% confidence`,
        ],
  ...
  private generatePredictions(
    ...
    if (failurePatterns.length > 0) {
      predictions.push({
        ...
        type: 'failure',
        description:
          'Elevated risk of failures in upcoming builds due to detected instability patterns',
        probability: 0.55,
  ...
  private generateLearnings(patterns: DetectedPattern[], metrics: BuildMetrics): Learning[] {
    ...
    if (metrics.parallelizationEfficiency < 50) {
      learnings.push({
        ...
        type: 'optimization',
        description:
          'Build has low parallelization efficiency - consider restructuring phase dependencies',
        confidence: 0.75,
  ...
  private groupRetrysByAgent(events: BuildEvent[]): Map<string, number> {
    ...
    for (const event of events) {
      if (event.type === 'AGENT_RETRIED') {
        const data = event.payload as { agentId: string };
  ...
  private findUnchanged(a: BuildProfile, b: BuildProfile): string[] {
    ...
    if (
      Math.abs(b.metrics.totalDuration - a.metrics.totalDuration) <
      a.metrics.totalDuration * 0.1
    ) {
  ...
  private generateComparisonSummary(a: BuildProfile, b: BuildProfile): string {
    const durationChange =
      ((b.metrics.totalDuration - a.metrics.totalDuration) / a.metrics.totalDuration) * 100;
  ...
  private analyzePhasePerformance(
    ...
    const agentPerformance = new Map<
      string,
      { success: number; total: number; avgDuration: number }
    >();
    ...
    for (const profile of profiles) {
      ...
      if (phaseProfile) {
        for (const agentId of phaseProfile.agents) {
          ...
          if (agent) {
            const existing = agentPerformance.get(agent.type) || {
              success: 0,
              total: 0,
              avgDuration: 0,
            };
  ...
  private getEventDescription(event: BuildEvent): string {
    const descriptions: Record<string, (data: unknown) => string> = {
      ...
      BUILD_COMPLETED: (d: unknown) =>
        `Build completed in ${this.formatDuration((d as { duration: number }).duration)}`,
      ...
      AGENT_RETRIED: (d: unknown) => `Retry #${(d as { attempt: number }).attempt} for agent`,
      QUALITY_GATE_PASSED: () => 'Quality gate passed',
      QUALITY_GATE_FAILED: () => 'Quality gate failed',
    };

    const descFn = descriptions[event.type];
    return descFn ? descFn(event.payload) : event.type;
  }
