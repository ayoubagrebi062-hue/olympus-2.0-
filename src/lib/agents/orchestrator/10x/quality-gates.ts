...
export class RollbackEngine extends EventEmitter {
  ...
  private buildRollbackSteps(source: CheckpointData, target: CheckpointData): RollbackStep[] {
  ...
  private assessRollbackRisks(source: CheckpointData, target: CheckpointData): RollbackRisk[] {
  ...
  private generateRecommendations(
    ...
    if (regressions.length > 2) {
      recommendations.push(
        'Multiple regressions detected. Consider adding pre-commit quality checks.'
      );
    }
...
export const BUILT_IN_RULES: QualityRule[] = [
  {
    ...
    validator: async context => {
      ...
      const codeArtifacts = Array.from(artifacts.entries()).filter(
        ([key]) => key.endsWith('.ts') || key.endsWith('.tsx') || key.endsWith('.js')
      );
      ...
      return {
        ...
        details: { issues },
        suggestions:
          issues.length > 0 ? ['Replace TODO comments with actual implementations'] : undefined,
        timestamp: new Date(),
      };
    },
  },
  ...
  },
  {
    ...
    validator: async context => {
      ...
      return {
        ...
        suggestions:
          issues.length > 0
            ? ['Add alt text to all images', 'Include aria-labels for interactive elements']
            : undefined,
        timestamp: new Date(),
      };
    },
      ...
      return {
        ...
        message:
          vulnerabilities.length === 0
            ...
            : `Found ${vulnerabilities.length} security concerns`,
      ...
      return {
        ...
        message:
          warnings.length === 0
            ...
            : `Found ${warnings.length} performance warnings`,
        details: { warnings },
        suggestions:
          warnings.length > 0
