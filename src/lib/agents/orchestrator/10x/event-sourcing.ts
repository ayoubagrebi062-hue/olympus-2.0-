...
export class StateProjector {
  ...
  private calculateMetrics(state: ProjectedBuildState): void {
    ...
    state.metrics.agentSuccessRate = agents.length > 0 ? completedAgents.length / agents.length : 0;
  ...
  private getEventDescription(event: BuildEvent): string {
    ...
    switch (event.type) {
      ...
      case 'AGENT_COMPLETED':
        return `Agent "${payload.agentId}" completed (${(payload as unknown as AgentCompletedPayload).qualityScore}/10)`;
      case 'AGENT_FAILED':
        return `Agent "${payload.agentId}" failed: ${(payload as unknown as AgentFailedPayload).error}`;
      case 'CHECKPOINT_CREATED':
  ...
  private getEventSeverity(type: BuildEventType): 'info' | 'warning' | 'error' | 'success' {
...
export class TimeMachine {
  ...
  async diff(
    ...
    const eventsBetween = events.filter(e => e.version > versionA && e.version <= versionB);
  ...
  private computeChanges(stateA: ProjectedBuildState, stateB: ProjectedBuildState): StateChange[] {
