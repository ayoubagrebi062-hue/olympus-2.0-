...
import type { Result, Quality, Progress, Options, TelemetryEvent, PendingItem } from './types';
...
import { VitoError, createError } from './errors';
...
import { streamGeneration, tryLocalGeneration, checkConnection as checkApiConnection } from './api';
import {
  hash,
  sanitize,
  extractCode,
...
export async function vito(task: string, options: Options = {}): Promise<Result> {
  ...
  if (!isOnline) {
    ...
    if (localResult) {
      return handleLocalSuccess(localResult, task, key, continued, startTime, () =>
  ...
  if (circuitOpen) {
    if (now - circuitOpenedAt > CIRCUIT_RESET_MS) {
      ...
      const waitSeconds = Math.ceil((CIRCUIT_RESET_MS - (now - circuitOpenedAt)) / 1000);
  ...
  try {
    ...
    return buildSuccessResult(extracted, raw, quality, task, continued, tokens, startTime);
  } catch (err) {
    if (signal?.aborted) {
      return buildCancelledResult(startTime);
    }
 ...
 */
function buildErrorResult(error: VitoError, queuePosition: number, startTime: number): Result {
  return {
    ok: false,
    code: '',
    raw: '',
