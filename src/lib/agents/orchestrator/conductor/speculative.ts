...
export class SpeculativeEngine {
  ...
  async race(task: string, context?: string): Promise<RaceResult> {
    ...
    const result = await this.raceWithThreshold(racePromises, selectedApproaches, abortController);
    ...
    this.raceHistory.push(finalResult);
  ...
  private estimateComplexity(task: string): number {
    ...
    return Math.min(10, words / 5 + (hasMultiple ? 2 : 0) + (isArchitectural ? 3 : 0));
  }
