...
export class FiftyXOrchestrator {
  ...
  async generate(request: GenerationRequest): Promise<GenerationResult> {
    ...
    const { prompt, framework = 'react', options = {} } = request;
    ...
    try {
      ...
      if (finalOptions.enableVision && pipelineResult.score >= 70) {
        ...
        while (visionIterations < finalOptions.visionIterations) {
          visionIterations++;
          this.emit('vision:iteration', `Vision iteration ${visionIterations}`, {
          ...
          try {
            ...
            this.emit(
              'vision:error',
              `Vision validation failed: ${(visionError as Error).message}`
            );
            break;
          }
        }
