  ...
  from: vi.fn((table: string) => ({
    ...
    select: vi.fn(() => ({
      eq: vi.fn((field: string, value: unknown) => ({
        single: vi.fn(async () => {
          ...
          const record = (records as Record<string, unknown>[]).find(r => r[field] === value);
    ...
    update: vi.fn((data: unknown) => ({
      eq: vi.fn(async (field: string, value: unknown) => {
        const records = mockSupabaseData[table] || [];
        const index = (records as Record<string, unknown>[]).findIndex(r => r[field] === value);
        if (index >= 0) {
          const typedRecords = records as Record<string, unknown>[];
          typedRecords[index] = { ...typedRecords[index], ...(data as object) };
        }
...
import { BuildStateMachine, createStateMachine, type BuildState } from '../state-machine';
  ...
  describe('create()', () => {
    it('should create a new build plan', async () => {
      const input: CreatePlanInput = {
        buildId: 'build-123',
        projectType: 'saas-full',
        phases: [{ id: 'discovery', name: 'Discovery', order: 0, agents: ['oracle'] }],
        agents: [
          {
            agentId: 'oracle',
            phase: 'discovery',
  ...
  describe('validateTransition()', () => {
    ...
    it('should add warnings for non-required conditions', async () => {
      const context = createMockTransitionContext({
        ...
        errors: new Map([['oracle', 'Non-critical error']]),
      });
...
import { PersistentStateMachine, type PersistedState } from '../persistent-state-machine';
  ...
  describe('atomic operations', () => {
    it('should use RPC for atomic plan creation', async () => {
      ...
      const input: TransactionalCreateInput = {
        ...
        agents: [
          { agentId: 'oracle', phaseId: 'discovery', order: 0, isRequired: true, dependencies: [] },
        ],
      };
...
describe('PersistentStateMachine', () => {
          ...
          eq: vi.fn().mockReturnValue({
            eq: vi.fn().mockReturnValue({
              select: vi.fn().mockReturnValue({
                single: vi
                  .fn()
                  .mockResolvedValue({ data: null, error: { message: 'No rows updated' } }),
              }),
            }),
          }),
        }),
  ...
  describe('static methods', () => {
    it('should find orphaned builds', async () => {
      const mockOrphanedBuilds = [{ build_id: 'orphan-1' }, { build_id: 'orphan-2' }];
