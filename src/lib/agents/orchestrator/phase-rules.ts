...
const IMPOSSIBLE_PATTERNS: Array<{ pattern: RegExp; reason: string }> = [
  ...
  {
    pattern: /cryptocurrency|crypto[- ]?payment/i,
    reason: 'Crypto payments require wallet integration',
  },

  // ML/AI training
  {
    pattern: /train.*model|model.*training/i,
    reason: 'ML model training requires GPU infrastructure',
  },
  ...
  // Design -> Frontend (skip architecture for simple projects)
  {
    ...
    conditions: [
      ...
      },
      {
        ...
        required: true,
        validate: async ctx => {
          return ctx.completedAgents.includes('palette') && ctx.completedAgents.includes('blocks');
        },
      },
    ],
  },
    ...
    conditions: [
      ...
      },
      {
        ...
        validate: async ctx => {
          ...
          const smokeOutput = ctx.agentOutputs.get('cypress') as
