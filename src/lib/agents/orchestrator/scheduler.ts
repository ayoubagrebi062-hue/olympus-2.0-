...
export class AgentScheduler {
  ...
  private initializeQueue(): void {
    ...
    for (const phasePlan of this.plan.phases) {
      console.log(
        `[Scheduler] Queueing phase ${phasePlan.phase} with ${phasePlan.agents.length} agents: ${phasePlan.agents.join(', ')}`
      );
  ...
  isPhaseComplete(): boolean {
    ...
    if (!phasePlan) {
      console.log(
        `[Scheduler] isPhaseComplete: Phase ${this.currentPhase} NOT FOUND in plan! Plan phases: ${this.plan.phases.map(p => p.phase).join(', ')}`
      );
      return true;
    }
    ...
    if (allDone) {
      console.log(
        `[Scheduler] Phase ${this.currentPhase} COMPLETE. Status: ${JSON.stringify(completionStatus)}`
      );
    }
  ...
  isComplete(): boolean {
    ...
    const completedRequired = Array.from(this.completed).filter(
      id => !getAgent(id)?.optional
    ).length;
  ...
  markBlockedAsFailed(): void {
    ...
    console.log(
      `[Scheduler] Marking ${blocked.length} blocked agents as failed: ${blocked.join(', ')}`
    );
