...
export class EvolutionEngine extends EventEmitter {
      ...
      switch (action.type) {
        case 'optimize_prompt': {
          ...
          action.result = {
            success: true,
            metrics: {
              before: { quality: 0, efficiency: 0 },
              after: { quality: 0, efficiency: 0 },
              improvement: 0,
            },
        ...
        if (
          ...
          if (evaluation.shouldPromote) {
            if (this.config.automation.autoPromote) {
              ...
              console.log(
                `[EvolutionEngine] Auto-promoted variant from experiment ${experimentId}`
              );
            } else {
  ...
  private findRelatedAgents(gap: string, analyses: AgentPerformanceAnalysis[]): string[] {
    const gapWords = gap.toLowerCase().split(/\s+/);
    const related: string[] = [];

    for (const analysis of analyses) {
      const agentWords = analysis.agentId.toLowerCase().split(/[_-]/);
      const hasOverlap = gapWords.some(w =>
        agentWords.some(aw => aw.includes(w) || w.includes(aw))
      );
