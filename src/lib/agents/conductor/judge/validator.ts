...
export class OutputValidator {
  ...
  private validateSchema(
    ...
    if (schema.properties && typeof schema.properties === 'object') {
      for (const [field, fieldSchema] of Object.entries(schema.properties)) {
        if (field in data) {
          const typeError = this.checkType(data[field], fieldSchema as Record<string, unknown>, [
            field,
          ]);
          if (typeError) {
   ...
   */
  private checkRequiredFields(agentId: string, data: Record<string, unknown>): ValidationError[] {
    const errors: ValidationError[] = [];
   ...
   */
  private checkCommonIssues(agentId: string, data: Record<string, unknown>): ValidationWarning[] {
    const warnings: ValidationWarning[] = [];
