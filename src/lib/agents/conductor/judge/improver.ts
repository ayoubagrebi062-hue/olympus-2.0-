...
export class ImprovementSuggester {
   ...
   */
  private suggestForAgent(agentId: string, score: QualityScore): ImprovementSuggestion[] {
    const suggestions: ImprovementSuggestion[] = [];
...
Priority: CONSISTENCY with previous outputs`;
   ...
   */
  private createDecomposedPrompt(agentId: string, validation: ValidationResult): string {
...
Priority: Address SPECIFIC ISSUES one by one`;
   ...
   */
  private createFocusedPrompt(agentId: string, dimensions: QualityDimensions): string {
