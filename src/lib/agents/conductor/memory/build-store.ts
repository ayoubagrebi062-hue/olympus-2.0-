...
export function createBuildRecord(params: {
  id?: string;
  tenantId: string;
  description: string;
  projectType: ProjectType;
...
export function addOutputToRecord(
  ...
  const inferredQuality =
    typeof (payload as any)?.quality === 'number' ? Number((payload as any).quality) : 0;
  const inferredTokens =
    typeof (payload as any)?.tokens === 'number' ? Number((payload as any).tokens) : 0;
