...
function createTestPattern(overrides: Partial<LearnedPattern> = {}): LearnedPattern {
  return {
    ...
    trigger: {
      conditions: [{ field: 'projectType', operator: 'equals', value: 'saas-app' }],
      operator: 'AND',
    },
    action: {
...
// Default style preferences (satisfy all required properties)
const DEFAULT_CODE_STYLE: import('../types').CodeStylePreferences = {
  framework: null,
  language: null,
  patterns: [],
  avoidPatterns: [],
...
const DEFAULT_DESIGN_STYLE: import('../types').DesignStylePreferences = {
  colorScheme: null,
  primaryColors: [],
  typography: null,
...
const DEFAULT_COMMUNICATION_STYLE: import('../types').CommunicationStylePreferences = {
  verbosity: null,
  technicalLevel: null,
  formatPreference: null,
};

function createTestFeedback(overrides: Partial<UserFeedback> = {}): UserFeedback {
  return {
    rating: 5,
    usedOutput: true,
...
describe('BuildStore', () => {
  ...
  beforeEach(() => {
    store = new BuildStore(DEFAULT_MEMORY_CONFIG.buildHistoryLimit ?? 100);
  });
...
describe('PatternEngine', () => {
  ...
  describe('Pattern Matching', () => {
    ...
    it('should not match patterns with unmet conditions', async () => {
      const pattern = createTestPattern({
        trigger: {
          conditions: [{ field: 'projectType', operator: 'equals', value: 'e-commerce' }],
          operator: 'AND',
        },
      });
      await engine.save(pattern);
    ...
    it('should match patterns with greaterThan operator', async () => {
      const pattern = createTestPattern({
        trigger: {
          conditions: [{ field: 'retryCount', operator: 'greaterThan', value: 2 }],
          operator: 'AND',
        },
      });
      await engine.save(pattern);
  ...
  describe('Effectiveness Queries', () => {
    beforeEach(async () => {
      await engine.save(
        createTestPattern({ successRate: 0.9, confidence: 0.85, type: 'quality_threshold' })
      );
      await engine.save(
        createTestPattern({ successRate: 0.7, confidence: 0.6, type: 'agent_selection' })
      );
      await engine.save(
        createTestPattern({ successRate: 0.95, confidence: 0.9, type: 'retry_strategy' })
      );
    });
    ...
    it('should get most effective patterns', async () => {
      ...
      expect(effective[0].successRate * effective[0].confidence).toBeGreaterThanOrEqual(
        effective[1].successRate * effective[1].confidence
      );
    });
...
describe('VectorStore', () => {
  ...
  describe('Embedding Operations', () => {
    ...
    it('should generate batch embeddings', async () => {
      const texts = ['Build landing page', 'Create e-commerce store', 'Dashboard application'];
  ...
  describe('Statistics', () => {
    beforeEach(async () => {
      await store.indexBuild(
        createTestBuildRecord({ projectType: 'saas-app', overallQuality: 9.0 })
      );
      await store.indexBuild(
        createTestBuildRecord({ projectType: 'saas-app', overallQuality: 8.0 })
      );
      await store.indexBuild(
        createTestBuildRecord({ projectType: 'landing-page', overallQuality: 4.0 })
      );
    });
...
describe('PreferencesManager', () => {
  ...
  describe('CRUD Operations', () => {
    it('should create and retrieve preferences', async () => {
      ...
      const prefs: UserPreferences = {
        ...
        codeStyle: { ...DEFAULT_CODE_STYLE, language: 'typescript', formatting: 'prettier' },
        designStyle: { ...DEFAULT_DESIGN_STYLE, theme: 'dark' },
        communicationStyle: {
  ...
  describe('Feedback Learning', () => {
    it('should record feedback and update preferences', async () => {
      ...
      const feedback: UserFeedback = createTestFeedback({
        rating: 5,
    ...
    it('should calculate average rating over multiple feedbacks', async () => {
      ...
      await manager.recordFeedback(tenantId, createTestFeedback({ rating: 5 }));
      await manager.recordFeedback(tenantId, createTestFeedback({ rating: 4 }));
      await manager.recordFeedback(
        tenantId,
        createTestFeedback({ rating: 3, modifiedOutput: true })
      );
    ...
    it('should update feedback trend', async () => {
      ...
      await manager.recordFeedback(tenantId, createTestFeedback({ rating: 3 }));
      await manager.recordFeedback(tenantId, createTestFeedback({ rating: 4 }));
      await manager.recordFeedback(tenantId, createTestFeedback({ rating: 5 }));
...
describe('MemoryModule', () => {
  ...
  describe('Pattern Operations', () => {
    ...
    it('should record pattern outcomes', async () => {
      ...
      const patterns = await memory.getMostEffectivePatterns('quality_threshold', 10);
    ...
    it('should get most effective patterns', async () => {
      ...
      const effective = await memory.getMostEffectivePatterns('quality_threshold', 1);
  ...
  describe('Analytics', () => {
    beforeEach(async () => {
      ...
      await memory.storeBuild(
        createTestBuildRecord({ tenantId, status: 'completed', overallQuality: 8.5 })
      );
      await memory.storeBuild(
        createTestBuildRecord({ tenantId, status: 'completed', overallQuality: 9.0 })
      );
      await memory.storeBuild(
        createTestBuildRecord({ tenantId, status: 'failed', overallQuality: 3.0 })
      );
    });
  ...
  describe('Event Handling', () => {
    ...
    it('should emit events on pattern add', async () => {
      ...
      const patternCalls = eventHandler.mock.calls.filter(call => call[0].type === 'pattern_added');
