...
export const iterationService = {
  ...
  async iterate(params: IterationParams): Promise<IterationResponse> {
    ...
    if (build.status !== 'completed') {
      return {
        success: false,
        error: { code: 'INVALID_STATUS', message: 'Can only iterate on completed builds' },
      };
    }
    ...
    if (insertError || !iteration) {
      return {
        ...
        error: {
          code: 'CREATE_FAILED',
          message: insertError?.message || 'Failed to create iteration',
        },
      };
    }
  ...
  async startIteration(buildId: string, iterationNumber: number): Promise<IterationResponse> {
    ...
    if (!build || !iteration) {
      return {
        success: false,
        error: { code: 'NOT_FOUND', message: 'Build or iteration not found' },
      };
    }
    ...
    await (supabase.from('build_iterations') as any)
    ...
    orchestrator
      ...
      .catch(error => {
        ...
        console.error(
          `[IterationService] CRITICAL: Failed to finalize iteration ${iteration.id}:`,
          error
        );
        (supabase.from('build_iterations') as any)
          .update({
            status: 'failed',
          ...
          .then(() => {
            console.log(
              `[IterationService] Marked iteration ${iteration.id} as failed after error`
            );
          })
          .catch((dbError: unknown) => {
            console.error(
              `[IterationService] Could not update iteration status for ${iteration.id}:`,
              dbError
            );
          });
      });
