...
export class MinimalInvariantEngine implements InvariantEngine {
  ...
  async verifyAll(identity: AgentIdentity): Promise<VerificationEvent> {
    ...
    console.log(
      `[InvariantEngine] Starting verification ${verificationId} for agent ${identity.agentId}`
    );
    ...
    console.log(
      `[InvariantEngine] Verification ${verificationId} complete: ${allPassed ? 'ALL PASS' : 'SOME FAIL'} (${duration}ms)`
    );
