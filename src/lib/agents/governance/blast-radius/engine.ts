...
export class BlastRadiusEngine implements IBlastRadiusEngine {
  ...
  async assessImpact(
    ...
    console.log(
      `[BlastRadiusEngine] Impact assessed for build ${buildId}: zone=${assessment.estimatedZone}, severity=${assessment.severity}`
    );
  ...
  private async analyzeAffectedResources(buildId: string, tenantId: string): Promise<string[]> {
    return [`build:${buildId}`, `tenant:${tenantId}`, `storage:${tenantId}`, `cache:${tenantId}`];
  }
  ...
  private determineContainmentAction(
    assessment: ImpactAssessment,
    policy: ContainmentPolicy | null
  ): 'ISOLATE' | 'QUARANTINE' | 'ROLLBACK' | 'ESCALATE' {
