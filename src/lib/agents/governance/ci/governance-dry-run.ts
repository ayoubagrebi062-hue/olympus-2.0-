...
class GovernanceDryRun {
  ...
  private isObservationModule(moduleName: string): boolean {
    ...
    const blockingModules = Array.isArray(currentPhase.blockingModules)
      ? currentPhase.blockingModules
      : [];
  ...
  run(): DryRunReport {
    ...
    console.log(
      `Phase Description: ${this.governancePhase?.phases[this.governancePhase.currentPhase].description || 'Unknown'}\n`
    );
      ...
      try {
        ...
        if (learningArtifact.summary.byPriority.high > 0) {
          ...
          report.warnings.push(
            `${learningArtifact.summary.byPriority.high} high-priority recommendations`
          );
        }
      } catch (error) {
        if (error instanceof Error && (error as any).code === 'exit') {
  ...
  private generateReport(modules: ModuleReport[]): DryRunReport {
    ...
    if (this.governancePhase) {
      const currentPhase = this.governancePhase.phases[this.governancePhase.currentPhase];
      blockingModules = Array.isArray(currentPhase.blockingModules)
        ? currentPhase.blockingModules
        : [];
  ...
  private printReport(report: DryRunReport): void {
    ...
    if (this.governancePhase) {
      const phaseDescription =
        this.governancePhase.phases[
          this.governancePhase.currentPhase as keyof typeof this.governancePhase.phases
        ].description;
    ...
    for (const module of report.modules) {
      ...
      console.log(
        `  ${statusEmoji} ${module.moduleName.padEnd(25)} ${module.status.padEnd(10)} ${module.executionTime}ms${marker}`
      );
    }
    ...
    if (report.summary.downgradedFindingsCount > 0) {
      console.log(
        `  Note: ${report.summary.downgradedFindingsCount} findings downgraded from FAIL to WARNING per ${report.governancePhase} phase rules`
      );
    }
    ...
    console.log('\nEnforcement Scope:');
    console.log(
      `  Blocking Modules:    ${report.enforcementScope.blockingModules.join(', ') || 'ALL'}`
    );
    console.log(
      `  Observation Modules:  ${report.enforcementScope.observationModules.join(', ') || 'NONE'}`
    );
