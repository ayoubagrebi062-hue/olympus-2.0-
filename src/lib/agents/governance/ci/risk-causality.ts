...
export class RiskCausalityAnalyzer {
  ...
  constructor() {
    ...
    this.CAUSALITY_OUTPUT_PATH = path.join(
      process.cwd(),
      'data',
      'governance',
      'risk-causality.json'
    ...
    try {
      ...
      try {
        ...
        if (gitDate) {
          ...
          metadata.ageInDays = Math.floor(
            (now.getTime() - gitTime.getTime()) / (1000 * 60 * 60 * 24)
          );
        }
      } catch {
  ...
  private inferCausality(
    ...
    if (acknowledgment?.rationale || acknowledgment?.decision) {
      const text =
        `${acknowledgment.rationale || ''} ${acknowledgment.decision || ''}`.toLowerCase();
    ...
    if (
      acknowledgment?.triggerType?.includes('missing_') ||
    ...
    if (
      gitMetadata.ageInDays > 180 &&
      (risk.stalenessLevel === 'stale' || risk.stalenessLevel === 'fossil')
    ) {
      return {
        causality: Causality.LEGACY_DEBT,
  ...
  private countRecurrences(patternId: string, acknowledgments: AcknowledgmentEntry[]): number {
    ...
    const daySpan = Math.floor(
      (lastDetected.getTime() - firstDetected.getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  analyze(): CausalityReport {
    ...
    for (const risk of attentionRisks) {
      ...
      const { causality, reason } = this.inferCausality(
        risk,
        acknowledgment,
        decisions,
        gitMetadata
