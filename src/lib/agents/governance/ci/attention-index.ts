...
export class AttentionIndexGenerator {
  ...
  constructor() {
    ...
    this.ATTENTION_INDEX_PATH = path.join(
      process.cwd(),
      'data',
      'governance',
      'attention-index.json'
    );
  }
  ...
  private computeAttentionScore(acknowledgment: ExtendedAcknowledgmentEntry): number {
    ...
    const driftMultiplier =
      acknowledgment.aging?.driftDetected === true
        ? this.DRIFT_MULTIPLIERS.drifted
        : this.DRIFT_MULTIPLIERS.stable;

    const ownershipPenalty =
      !acknowledgment.acknowledgedBy || !acknowledgment.acknowledged
  ...
  generate(): AttentionIndexReport {
    ...
    const fossilCriticalCount = topRisks.filter(
      r => r.riskLevel === 'critical' && r.stalenessLevel === 'fossil'
    ).length;
    const driftedHighRiskCount = topRisks.filter(
      r => r.riskLevel === 'high' && r.driftDetected === true
    ).length;
  ...
  printTop5Attention(report: AttentionIndexReport): void {
    ...
    top5.forEach((risk, index) => {
      const scoreIcon =
        risk.attentionScore >= 80
          ? '🔴'
          : risk.attentionScore >= 60
            ? '🟠'
            : risk.attentionScore >= 40
              ? '🟡'
              : '🟢';
      const stalenessIcon =
        risk.stalenessLevel === 'fossil' ? '🦴' : risk.stalenessLevel === 'stale' ? '⏰' : '';
      ...
      console.log(
        `   ${stalenessIcon}${driftIcon}${ownerIcon} ${risk.stalenessLevel.toUpperCase()} | ${risk.driftDetected ? 'DRIFTED' : 'STABLE'} | ${risk.ownershipRisk ? 'NO OWNER' : 'OWNED'}`
      );
