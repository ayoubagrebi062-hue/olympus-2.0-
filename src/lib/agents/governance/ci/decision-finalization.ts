...
export class DecisionFinalizationGate {
  ...
  enforce(): EnforcementReport {
    ...
    const report: EnforcementReport = {
      ...
      decisionsWithoutOutcome: referencedDecisions.filter(d => !d.outcome || !d.outcome.result)
        .length,
      violations,
      summary: {
        byClass: summary,
  ...
  private getReferencedDecisions(): Array<DecisionSchema['decisions'][number]> {
    ...
    return this.decisionSchema.decisions.filter(decision =>
      this.referencedDecisionIds.has(decision.id.toLowerCase())
    );
  }
  ...
  private validateDecision(decision: DecisionSchema['decisions'][number]): Violation[] {
    ...
    if (!outcome.result) {
      violations.push({
        ...
        description:
          'Outcome missing result field. Must be one of: success, failure, partial, blocked, pending.',
        severity: 'error',
    ...
    if (
      ...
      (!outcome.reversalMethod || outcome.reversalMethod.trim() === '')
    ) {
      violations.push({
        decisionId: decision.id,
        decisionClass: decision.decisionClass,
        ...
        description:
          'Outcome marked as reversible but missing reversalMethod. Must describe how to reverse.',
        severity: 'error',
    ...
    if (
      ...
      (!outcome.unintendedConsequences || outcome.unintendedConsequences.length === 0)
    ) {
      violations.push({
        decisionId: decision.id,
        decisionClass: decision.decisionClass,
  ...
  private calculateResultBreakdown(
