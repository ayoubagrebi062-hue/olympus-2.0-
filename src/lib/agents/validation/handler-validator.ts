...
const FAKE_HANDLER_PATTERNS: Array<{ pattern: RegExp; reason: string }> = [
  ...
  // Console.log only
  {
    pattern: /^\s*\(\s*\)\s*=>\s*console\.(log|info|warn|debug)\([^)]*\)\s*;?\s*$/gm,
    reason: 'Only console.log',
  },
  {
    pattern: /^\s*\(\s*\)\s*=>\s*\{\s*console\.(log|info|warn|debug)\([^)]*\)\s*;?\s*\}\s*$/gm,
    reason: 'Only console.log in body',
  },

  // Alert only
  { pattern: /^\s*\(\s*\)\s*=>\s*alert\([^)]*\)\s*$/gm, reason: 'Only alert()' },
  {
    pattern: /^\s*\(\s*\)\s*=>\s*\{\s*alert\([^)]*\)\s*;?\s*\}\s*$/gm,
    reason: 'Only alert() in body',
  },

  // TODO/FIXME only
  {
    pattern: /^\s*\([^)]*\)\s*=>\s*\{\s*\/\/\s*(TODO|FIXME|HACK)[^}]*\}\s*$/gim,
    reason: 'Only contains TODO comment',
  },
  {
    pattern: /^\s*\([^)]*\)\s*=>\s*\{\s*\/\*[\s\S]*?(TODO|FIXME)[\s\S]*?\*\/\s*\}\s*$/gim,
    reason: 'Only contains TODO comment',
  },
...
function extractHandlers(
  code: string
): Array<{ location: string; body: string; fullMatch: string }> {
  ...
  const standalonePattern =
    /const\s+(handle\w+|on\w+)\s*=\s*((?:async\s+)?\([^)]*\)\s*=>\s*(?:\{[\s\S]*?\}|[^;\n]+))/g;
