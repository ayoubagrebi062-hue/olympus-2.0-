...
function suggestColorToken(hex: string): string {
  ...
  if (
    normalizedHex.startsWith('7c') ||
    normalizedHex.startsWith('8b') ||
    normalizedHex.startsWith('6d')
  ) {
    return 'Use primary or violet-* variant';
  }
  if (
    normalizedHex.startsWith('ef') ||
    normalizedHex.startsWith('dc') ||
    normalizedHex.startsWith('f8')
  ) {
    return 'Use destructive or red-* variant';
  }
  if (
    normalizedHex.startsWith('22') ||
    normalizedHex.startsWith('16') ||
    normalizedHex.startsWith('4a')
  ) {
...
export function validateDesignTokens(code: string): DesignValidationResult {
  ...
  for (const match of classNameMatches) {
    ...
    for (const cls of classes) {
      ...
      const spacingMatch = cls.match(
        /(?:p|m|gap|space|w|h|top|right|bottom|left|inset)-\[(\d+)px\]/
      );
      if (spacingMatch) {
        hardcodedSpacing++;
  ...
  const score =
    totalClasses > 0 ? Math.max(0, Math.round(100 - (totalViolations / totalClasses) * 500)) : 100;
