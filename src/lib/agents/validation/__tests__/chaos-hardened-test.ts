...
async function runHardenedChaosTest() {
  ...
  for (const input of garbageInputs) {
    ...
    if (result.success || result.error) {
      ...
      console.log(
        `  ${input.name.padEnd(15)} → ${result.success ? '✅ Valid' : `⚠️  Rejected: ${result.error?.code}`}`
      );
      garbagePass++;
    } else {
  ...
  for (const s of sizes) {
    ...
    if (s.size <= 1024 * 1024) {
      console.log(
        `  ${s.name.padEnd(10)} → ${result.success ? '✅ Accepted' : `⚠️  ${result.error?.code}`}`
      );
    } else {
      console.log(
        `  ${s.name.padEnd(10)} → ${result.error?.code === 'INPUT_TOO_LARGE' ? '✅ Rejected (expected)' : '❌ Should have rejected'}`
      );
    }
  }
  ...
  for (let i = 0; i < 150; i++) {
    const result = hardenedQuickValidate('const x = 1;', {
  ...
  console.log(
    `  ${rateLimited > 0 ? '✅ Rate limiting works' : '⚠️  Rate limiting not triggered'}`
  );
