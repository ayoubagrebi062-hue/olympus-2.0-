...
export function validatePrompt(prompt: string): PromptValidationResult {
  ...
  if (foundMalicious.length > 0) {
    console.warn(
      `[PromptValidator] REJECTED: Malicious intent detected - ${foundMalicious.join(', ')}`
    );
    return {
      valid: false,
  ...
  if (wordCount < 10) {
    ...
    return {
      valid: false,
      error:
        'Your description is too vague. Please include: what the app does, who will use it, and key features.',
      suggestion:
        'Example: "Build a todo app where users can create tasks, mark them complete, and organize by category. Target users are busy professionals."',
      category: 'vague',
  ...
  const foundComplexity = COMPLEXITY_MARKERS.filter(marker => lower.includes(marker.toLowerCase()));

  if (foundComplexity.length >= 3) {
    console.warn(
      `[PromptValidator] WARNING: Over-complex - ${foundComplexity.length} major features`
    );
    return {
  ...
  // 5. Valid prompt
  console.info(
    `[PromptValidator] ACCEPTED: ${wordCount} words, ${foundComplexity.length} complexity markers`
  );
  return {
    valid: true,
    category: 'valid',
