  ...
  pixel: [
    {
      ...
      validate: (output: AgentOutput) => {
        ...
        const hasComponents =
          output.artifacts.some(a => a.type === 'schema' && a.id === 'components') ||
          output.decisions.some(d => d.type === 'component' || d.choice?.includes('component'));

        if (!hasComponents) {
          errors.push(
            'BLOCKS output missing component definitions. PIXEL cannot generate UI without component specs.'
          );
        }

        // Check for design tokens
        const hasDesignTokens = output.artifacts.some(
          a => a.id === 'design_tokens' || a.content?.includes('tokens')
        );
      ...
      validate: (output: AgentOutput) => {
        ...
        const hasTables =
          output.artifacts.some(
            a => a.type === 'schema' && (a.id === 'tables' || a.id === 'schema')
          ) || output.decisions.some(d => d.type === 'schema' || d.choice?.includes('table'));

        if (!hasTables) {
          errors.push(
            'DATUM output missing database tables. FORGE cannot generate backend without schema.'
          );
        }

        // Check for relationships
        const hasRelationships = output.artifacts.some(
          a => a.id === 'relationships' || a.content?.includes('relationship')
        );

        if (!hasRelationships) {
          warnings.push(
            'DATUM output missing table relationships. Data integrity may be compromised.'
          );
        }
  ...
  engine: [
    {
      ...
      validate: (output: AgentOutput) => {
        ...
        // Check for endpoints
        const hasEndpoints =
          output.artifacts.some(a => a.type === 'schema' && a.id === 'endpoints') ||
          output.decisions.some(d => d.type === 'api' || d.choice?.includes('endpoint'));

        if (!hasEndpoints) {
          errors.push(
            'NEXUS output missing API endpoints. ENGINE cannot implement business logic without API contract.'
          );
        }

        // Check for request/response schemas
        const hasSchemas = output.artifacts.some(
          a =>
        ...
        if (!hasSchemas) {
          warnings.push(
            'NEXUS output missing request/response schemas. Type safety may be compromised.'
          );
        }
  ...
  wire: [
    {
      ...
      validate: (output: AgentOutput) => {
        ...
        // Check for pages
        const hasPages =
          output.artifacts.some(a => a.type === 'schema' && a.id === 'pages') ||
          output.decisions.some(d => d.type === 'page' || d.choice?.includes('page'));

        if (!hasPages) {
          errors.push(
            'CARTOGRAPHER output missing page definitions. WIRE cannot implement routing without pages.'
          );
        }

        // Check for navigation
        const hasNavigation = output.artifacts.some(
          a => a.id === 'navigation' || a.content?.includes('nav')
        );

        if (!hasNavigation) {
          warnings.push(
            'CARTOGRAPHER output missing navigation structure. User experience may suffer.'
          );
        }
  ...
  keeper: [
    {
      ...
      validate: (output: AgentOutput) => {
        ...
        const hasTables = output.artifacts.some(
          a => a.type === 'schema' && (a.id === 'tables' || a.id === 'schema')
        );

        if (!hasTables) {
          errors.push(
            'DATUM output missing database schema. KEEPER cannot implement data layer without tables.'
          );
        }
...
export class EnhancedAgentExecutor {
   ...
   */
  private injectEnhancedContext(input: AgentInput, context: EnhancedContext): AgentInput {
    ...
    const designPreferences =
      context.userContext.preferences.stylePreference || input.context.designPreferences;
   ...
   * Record execution for learning
   */
  private async recordExecution(input: AgentInput, result: ExecutionResult): Promise<string[]> {
