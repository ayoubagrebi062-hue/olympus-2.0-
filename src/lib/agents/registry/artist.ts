...
const STYLE_TEMPLATES: Record<string, { positive: string; negative: string }> = {
  hero: {
    positive:
      'professional, high-quality, modern, clean, corporate, tech, cinematic lighting, 8k resolution',
  ...
  product: {
    positive:
      'product photography, studio lighting, clean white background, detailed, commercial quality',
...
export class ArtistAgent {
  ...
  async generatePrompts(requirements: ImageRequirement[]): Promise<LeonardoPrompt[]> {
    ...
    for (const req of requirements) {
      ...
      const dimensions =
        req.dimensions || DIMENSION_PRESETS[req.type] || DIMENSION_PRESETS.illustration;
  ...
  generatePlaceholders(requirements: ImageRequirement[]): Record<string, string> {
    ...
    for (const req of requirements) {
      const dimensions = req.dimensions ||
        DIMENSION_PRESETS[req.type] || { width: 800, height: 600 };

      // Use placeholder service
      placeholders[req.id] =
        `https://placehold.co/${dimensions.width}x${dimensions.height}/1a1a2e/eaeaea?text=${encodeURIComponent(req.type)}`;
    }
...
export function extractImageRequirements(strategosOutput: any): ImageRequirement[] {
  ...
  for (const feature of features) {
    ...
    if (
      ...
      description.includes('user profile')
    ) {
      requirements.push({
