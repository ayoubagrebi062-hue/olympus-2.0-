...
export async function saveCriticalDecisions(
  ...
  try {
    ...
    const { error } = await (supabase.from('builds') as any)
      .update({
 ...
 */
export async function loadCriticalDecisions(buildId: string): Promise<CriticalDecisions | null> {
  try {
    const supabase = createServiceRoleClient();
    ...
    const decisions = safeJsonParse<CriticalDecisions | null>(
      row.critical_decisions,
      null,
      'persistence:criticalDecisions'
    );
    if (!decisions) {
      return await loadCriticalDecisionsBackup(buildId);
    }
 ...
 */
async function loadCriticalDecisionsBackup(buildId: string): Promise<CriticalDecisions | null> {
  try {
    const supabase = createServiceRoleClient();
 ...
 */
async function loadCriticalDecisionsFromFile(buildId: string): Promise<CriticalDecisions | null> {
  try {
    const fs = await import('fs/promises');
    const path = await import('path');
