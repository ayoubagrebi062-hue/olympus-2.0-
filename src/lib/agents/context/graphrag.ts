...
export class GraphRAGContextManager {
   ...
   */
  async getUserContext(userId: string, options: ContextOptions = {}): Promise<UserContext> {
    ...
    const [neo4jContext, buildHistory, similarPrompts] = await Promise.all([
   ...
   */
  private async getMongoDBContext(userId: string, limit: number): Promise<BuildSummary[]> {
    try {
  ...
  async getContextSummary(userId: string): Promise<string> {
    ...
    if (context.totalBuilds > 0) {
      parts.push(
        `Build History: ${context.totalBuilds} builds, ${(context.successRate * 100).toFixed(0)}% success rate`
      );
    }
   ...
   */
  async ensureUser(userId: string, email: string, displayName?: string): Promise<void> {
...
export async function recordBuild(
  ...
  return await getContextManager().recordBuild(userId, projectId, buildId, prompt, embedding);
}
