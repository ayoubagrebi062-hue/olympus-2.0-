...
export async function analyzeFeedback(feedback: string): Promise<FeedbackAnalysis> {
  ...
  if (designKeywords.some(k => lowercaseFeedback.includes(k))) {
    aspects.push({
      category: 'design',
      sentiment: determineAspectSentiment(
        feedback,
        designKeywords,
        positiveKeywords,
        negativeKeywords
...
export async function learnFromBuild(
  ...
  console.log(
    `[PreferenceLearning] Learned: ${analysis.techUsed.length} tech, ${analysis.designPatterns.length} patterns, ${analysis.components.length} components`
  );
}
...
export async function learnFromFeedback(
  ...
  console.log(
    `[PreferenceLearning] Feedback analyzed: ${analysis.sentiment}, ${analysis.aspects.length} aspects`
  );
}
