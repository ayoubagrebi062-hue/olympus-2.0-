...
export class SemanticLayer implements GuardrailLayerHandler {
  ...
  private assessQuality(prompt: string): QualityAssessment {
    ...
    const score = clarity * 0.25 + specificity * 0.3 + completeness * 0.25 + relevance * 0.2;
  ...
  private assessSpecificity(prompt: string, issues: string[], suggestions: string[]): number {
    ...
    const techTerms =
      /\b(react|vue|angular|typescript|javascript|python|node|next\.?js|express|django|flask|postgres|mongodb|redis|docker|kubernetes|aws|gcp|azure)\b/gi;
  ...
  private assessCompleteness(prompt: string, issues: string[], suggestions: string[]): number {
    ...
    const actionVerbs =
      /\b(build|create|make|implement|add|remove|fix|update|deploy|configure|analyze|generate)\b/i;
  ...
  private analyzeScope(prompt: string): ScopeAnalysis {
    ...
    const complexityIndicators: Array<{ pattern: RegExp; weight: number; concern: string }> = [
      {
        pattern: /\b(microservice|distributed|scalable)\b/i,
        weight: 2,
        concern: 'Distributed architecture adds complexity',
      },
      {
        pattern: /\b(machine learning|ml|ai|neural)\b/i,
        weight: 2,
        concern: 'ML integration requires specialized expertise',
      },
      {
        pattern: /\b(blockchain|crypto|web3)\b/i,
        weight: 2,
        concern: 'Blockchain integration is complex',
      },
      {
        pattern: /\b(multi-tenant|saas)\b/i,
        weight: 1.5,
        concern: 'Multi-tenancy adds architectural complexity',
      },
      {
        pattern: /\b(enterprise|large-scale)\b/i,
        weight: 1.5,
        concern: 'Enterprise requirements increase scope',
      },
    ];
