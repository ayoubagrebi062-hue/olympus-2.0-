...
export class GuardrailEngine implements IGuardrailEngine {
   ...
   */
  async validate(context: GuardrailContext, input: GuardrailInput): Promise<GuardrailResult> {
  ...
  private getLayerConfig(
    layer: GuardrailLayer
  ): ApiLayerConfig | SecurityLayerConfig | SemanticLayerConfig | AgentLayerConfig {
    switch (layer) {
  ...
  private getLayerHandler(
    layer: GuardrailLayer
  ): ApiLayer | SecurityLayer | SemanticLayer | AgentLayer {
    switch (layer) {
      case GUARDRAIL_LAYERS.API:
  ...
  private computeFinalResult(context: GuardrailContext, startTime: number): GuardrailResult {
    // Collect all results
 ...
 */
export function createGuardrailEngine(config?: Partial<GuardrailPipelineConfig>): GuardrailEngine {
