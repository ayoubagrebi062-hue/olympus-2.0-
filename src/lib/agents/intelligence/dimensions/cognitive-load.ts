...
function calculateMetrics(content: string): CognitiveMetrics {
  ...
  const workingMemoryLoad = Math.min(
    10,
    avgSentenceLength / 5 + maxDepth * 1.5 + (jargonCount / wordCount) * 100
  );
  ...
  const shortParagraphs =
    paragraphs.filter(p => p.split(/\s+/).length < 50).length / paragraphCount;
  const scanningEase = Math.min(
    10,
    (hasHeadings ? 3 : 0) + (hasBullets ? 2 : 0) + shortParagraphs * 5
  );
