...
export function analyzeEmotionalJourney(content: string): EmotionalJourneyResult {
  ...
  const confidence = Math.min(
    0.9,
    0.4 + (intensityMap.length / 30) * 0.3 + (presentEmotions.size / 6) * 0.2
  );
...
function analyzeArc(arc: EmotionalState[]): ArcAnalysis {
  ...
  for (let i = 0; i < arc.length; i++) {
    ...
    if (
      positiveEmotions.includes(arc[i]) &&
      firstNegativeIndex !== -1 &&
      firstPositiveAfterNegative === -1
    ) {
      firstPositiveAfterNegative = i;
    }
  }

  const hasNegativeBeforePositive =
    firstNegativeIndex !== -1 && firstPositiveAfterNegative > firstNegativeIndex;

  // Check for emotional peak (high intensity moment)
  const hasEmotionalPeak =
    arc.includes('excited') || (arc.includes('hopeful') && arc.includes('confident'));
