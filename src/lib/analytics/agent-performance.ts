...
export class AgentPerformanceTracker {
  ...
  recordExecution(data: {
    ...
    agent.retryRate = agent.executions > 0 ? data.retries / agent.executions : 0;
  ...
  completeBuild(
    ...
    if (outputs) {
      build.efficiency.tokensPerLine =
        outputs.linesOfCode > 0 ? build.totalTokens / outputs.linesOfCode : 0;
      build.efficiency.tokensPerComponent =
        outputs.components > 0 ? build.totalTokens / outputs.components : 0;
      build.efficiency.costPerFeature =
        outputs.features > 0 ? build.totalCost / outputs.features : 0;
    }

    // Calculate quality per dollar
    const avgQuality = this.calculateBuildAvgQuality(build);
    build.efficiency.qualityPerDollar = build.totalCost > 0 ? avgQuality / build.totalCost : 0;

    // Compare to baseline and best
    const allBuilds = Array.from(this.buildData.values()).filter(b => b.status === 'completed');
    if (allBuilds.length > 0) {
      ...
      build.efficiency.vsBest =
        build.efficiency.qualityPerDollar / Math.max(0.01, bestBuild.efficiency.qualityPerDollar);
    }
  ...
  private createAgentData(agentId: string, agentName: string, role: string): AgentPerformanceData {
    return {
      agentId,
      agentName,
      role,
  ...
  private updateBuildData(buildId: string, phaseId: string, metrics: AgentBuildMetrics): void {
  ...
  private takeSnapshots(): void {
    for (const agent of this.agentData.values()) {
      const snapshot: PerformanceSnapshot = {
        ...
        successRate: agent.executions > 0 ? agent.successfulExecutions / agent.executions : 0,
  ...
  private calculateTrend(history: PerformanceSnapshot[]): 'improving' | 'stable' | 'degrading' {
  ...
  private calculateSummary(
    ...
    return {
      ...
      avgQualityScore:
        agents.length > 0 ? agents.reduce((s, a) => s + a.avgQualityScore, 0) / agents.length : 0,
  ...
  private generateSuggestions(
    ...
    const avgBuildCost =
      builds.length > 0 ? builds.reduce((s, b) => s + b.totalCost, 0) / builds.length : 0;
  ...
  private calculateBudgetStatus(agents: AgentPerformanceData[]): BudgetStatus {
    ...
    const percentUsed =
      this.budgetConfig.allocated > 0 ? (spent / this.budgetConfig.allocated) * 100 : 0;

    const daysElapsed = Math.max(
      1,
      (Date.now() - this.budgetConfig.startDate) / (24 * 60 * 60 * 1000)
    );
    const daysTotal = Math.max(
      1,
      (this.budgetConfig.endDate - this.budgetConfig.startDate) / (24 * 60 * 60 * 1000)
    );
...
export interface ExportedData {
  agents: AgentPerformanceData[];
  builds: Array<
    Omit<BuildPerformance, 'agentBreakdown' | 'phaseBreakdown'> & {
