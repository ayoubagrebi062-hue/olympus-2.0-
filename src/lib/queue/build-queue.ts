...
// Type definitions for Supabase client stub
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type SupabaseQueryResult = { data: any; error: Error | null };
// eslint-disable-next-line @typescript-eslint/no-explicit-any
interface SupabaseQueryBuilder {
  ...
  eq: (column: string, value: string) => SupabaseQueryBuilder & Promise<SupabaseQueryResult>;
  lt: (column: string, value: unknown) => SupabaseQueryBuilder & Promise<SupabaseQueryResult>;
...
export class BuildQueue {
  ...
  async claim(queueId: string, userId: string, tenantId: string): Promise<any> {
    ...
    if (new Date(data.expires_at) < new Date()) {
      await supabase.from('build_queue').update({ status: 'expired' }).eq('queue_id', queueId);
  ...
  async getStats(): Promise<{
    ...
    try {
      const { data, error } = await supabase.from('build_queue').select('status');
      ...
      const stats = {
        ...
        claimed: data.filter((r: { status: string }) => r.status === 'claimed').length,
        expired: data.filter((r: { status: string }) => r.status === 'expired').length,
        total: data.length,
      };
