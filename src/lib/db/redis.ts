 ...
 * Refresh session TTL
 */
export async function refreshSession(sessionId: string, ttlSeconds: number = 86400): Promise<void> {
  const redis = getClient();
 ...
 */
export async function setBuildStatus(buildId: string, status: BuildStatusCache): Promise<void> {
  const redis = getClient();
  await redis.setex(
  ...
  await redis.publish(`${PREFIXES.PUBSUB}build:${buildId}`, JSON.stringify(status));
}
...
export async function setCache(key: string, value: any, ttlSeconds: number = 300): Promise<void> {
  const redis = getClient();
  await redis.setex(`${PREFIXES.CACHE}${key}`, ttlSeconds, JSON.stringify(value));
}
