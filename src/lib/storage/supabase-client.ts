...
export class StorageClient {
  ...
  async upload(
    ...
    const cacheControl =
      ...
      (options.contentType
        ? CACHE_BY_CATEGORY[getCategoryFromMime(options.contentType)]
        : CACHE_CONTROL.private);

    const { data: result, error } = await this.supabase.storage.from(bucket).upload(path, data, {
   ...
   * Download a file
   */
  async download(bucket: StorageBucket, path: string, options?: DownloadOptions): Promise<Buffer> {
    this.validateBucket(bucket);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data, error } = await this.supabase.storage.from(bucket).download(path, options as any);
   ...
   */
  async listAll(bucket: StorageBucket, folder: string = ''): Promise<StorageObject[]> {
   ...
   * Move a file
   */
  async move(bucket: StorageBucket, fromPath: string, toPath: string): Promise<void> {
    this.validateBucket(bucket);
   ...
   * Copy a file
   */
  async copy(bucket: StorageBucket, fromPath: string, toPath: string): Promise<void> {
    this.validateBucket(bucket);
