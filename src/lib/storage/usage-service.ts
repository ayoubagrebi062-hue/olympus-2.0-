...
import { QuotaExceededError, BandwidthExceededError, ProviderError } from './errors';
...
export class UsageServiceImpl implements UsageService {
  ...
  async assertQuota(tenantId: string, fileSize: number): Promise<void> {
    ...
    if (!check.allowed) {
      throw new QuotaExceededError(tenantId, check.currentUsage, check.limit, fileSize);
    }
  }
   ...
   * Track file upload
   */
  async trackUpload(tenantId: string, size: number, category: FileCategory): Promise<void> {
   ...
   * Track file deletion
   */
  async trackDeletion(tenantId: string, size: number, category: FileCategory): Promise<void> {
    const categoryColumn = CATEGORY_COLUMNS[category] || 'other_bytes';
  ...
  async getUsage(tenantId: string): Promise<UsageStats> {
    ...
    return {
      storage: {
        ...
        limit: usage.limit_bytes,
        percentage: usage.limit_bytes > 0 ? (usage.total_bytes / usage.limit_bytes) * 100 : 0,
        byCategory: {
  ...
  private async manualIncrement(
    ...
    await this.supabase.from('storage_usage').update(updates).eq('tenant_id', tenantId);
  }
