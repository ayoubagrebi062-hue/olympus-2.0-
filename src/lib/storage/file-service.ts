...
export class FileServiceImpl implements FileService {
  ...
  async getSignedUrl(fileId: string, expiresIn: number = 3600): Promise<string> {
    ...
    const url = await this.storage.createSignedUrl(file.bucket as StorageBucket, file.path, {
      expiresIn,
    });
  ...
  async getDownloadUrl(fileId: string, filename?: string): Promise<string> {
    ...
    return this.storage.createSignedUrl(file.bucket as StorageBucket, file.path, {
  ...
  async download(fileId: string): Promise<Buffer> {
    ...
    const buffer = await this.storage.download(file.bucket as StorageBucket, file.path);
  ...
  async listFiles(params: ListFilesParams): Promise<PaginatedResult<FileWithUrls>> {
    ...
    const enrichedFiles = await Promise.all(files.map(f => this.enrichWithUrls(f)));
  ...
  async searchFiles(params: SearchParams): Promise<FileWithUrls[]> {
    const { tenantId, query, category, mimeTypes, minSize, maxSize, limit = 20 } = params;
   ...
   */
  async updateMetadata(fileId: string, metadata: Record<string, unknown>): Promise<FileWithUrls> {
    const file = await this.getFileRecord(fileId);
    if (!file) {
      throw new FileNotFoundError(fileId);
    }
  ...
  private async enrichWithUrls(file: DbFile): Promise<FileWithUrls> {
    ...
    // Generate main URL
    if (file.is_public) {
      const publicUrl = this.storage.getPublicUrl(file.bucket as StorageBucket, file.path);
      result.url = publicUrl;
      result.publicUrl = publicUrl;
    } else {
      result.url = await this.storage.createSignedUrl(file.bucket as StorageBucket, file.path);
    }
    ...
    if (file.variants && Object.keys(file.variants).length > 0) {
      ...
      for (const [name, path] of Object.entries(file.variants)) {
        if (file.is_public) {
          result.variantUrls[name] = this.storage.getPublicUrl(file.bucket as StorageBucket, path);
        } else {
