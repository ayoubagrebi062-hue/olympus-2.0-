...
export class ImageProcessorImpl implements ImageProcessor {
  ...
  async process(fileId: string): Promise<ProcessResult> {
    ...
    try {
      // 1. Download original
      const originalBuffer = await this.storage.download(file.bucket as StorageBucket, file.path);
      ...
      for (const variant of variants) {
        try {
          const variantPath = await this.generateVariant(file, rotatedBuffer, variant);
  ...
  async generateVariant(
    ...
    try {
      ...
      // Upload variant
      await this.storage.upload(file.bucket as StorageBucket, variantPath, outputBuffer, {
  ...
  async optimize(fileId: string, options: OptimizeOptions = {}): Promise<OptimizeResult> {
    ...
    await this.storage.upload(file.bucket as StorageBucket, optimizedPath, optimizedBuffer, {
      contentType: `image/${format}`,
      cacheControl: 'public, max-age=31536000, immutable',
    });
  ...
  async applyWatermark(
    ...
    const resizedWatermark = await sharp
      .default(watermarkBuffer)
  ...
  async processAvatar(
    ...
    for (const variant of AVATAR_VARIANTS) {
      const variantBuffer = await sharp
        .default(processed)
   ...
   * Update file record
   */
  private async updateFileRecord(fileId: string, updates: Partial<DbFile>): Promise<void> {
