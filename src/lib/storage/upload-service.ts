...
export class UploadServiceImpl implements UploadService {
  ...
  async getUploadUrl(params: GetUploadUrlParams): Promise<PresignedUrl> {
    ...
    if (this.usageService) {
      ...
      if (!quota.allowed) {
        throw new StorageError('Storage quota exceeded', STORAGE_ERROR_CODES.QUOTA_EXCEEDED, 507, {
          tenantId,
          size,
  ...
  async confirmPresignedUpload(fileId: string): Promise<UploadResult> {
    ...
    // Track usage
    if (this.usageService) {
      await this.usageService.trackUpload(
        file.tenant_id,
        file.size_bytes,
  ...
  async completeUpload(sessionId: string): Promise<UploadResult> {
    ...
    if (session.uploaded_chunks.length !== totalChunks) {
      throw new UploadIncompleteError(sessionId, session.uploaded_chunks.length, totalChunks);
    }
  ...
  async uploadMultiple(files: UploadParams[]): Promise<UploadResult[]> {
    ...
    for (let i = 0; i < files.length; i += concurrency) {
      ...
      const batchResults = await Promise.allSettled(
        batch.map((params, idx) => this.upload(params).then(result => ({ index: i + idx, result })))
      );
    ...
    if (errors.length > 0 && errors.length === files.length) {
      throw new StorageError(`All uploads failed`, STORAGE_ERROR_CODES.UPLOAD_FAILED, 500, {
  ...
  private async validateFile(
    ...
    if (!filename || filename.length > 255) {
      throw new StorageError('Invalid filename', STORAGE_ERROR_CODES.INVALID_FILENAME, 400);
    }
  }
  ...
  private async createFileRecord(
    data: Partial<DbFile> & { id: string; tenant_id: string }
  ): Promise<DbFile> {
