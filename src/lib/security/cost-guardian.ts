...
export function checkMonthlyTokens(tokensUsed: number, tier: PlanTier): LimitCheckResult {
  const limits = TIER_LIMITS[tier];
...
export function checkDailyBuilds(buildsToday: number, tier: PlanTier): LimitCheckResult {
  const limits = TIER_LIMITS[tier];
...
export function checkHourlySpend(hourlySpend: number, tier: PlanTier): LimitCheckResult {
  const limits = TIER_LIMITS[tier];
...
export function costGuardianCheck(input: CostGuardianCheckInput): CostGuardianResult {
  ...
  if (customBudget && limits.allowCustomBudget) {
    if (
      customBudget.dailyLimit &&
      currentUsage.dailySpend + estimatedCost > customBudget.dailyLimit
    ) {
      customBudgetCheck = {
        allowed: false,
  ...
  if (tokensAfter >= limits.tokensPerMonth * 0.8) {
    warnings.push(
      `Used ${Math.round((tokensAfter / limits.tokensPerMonth) * 100)}% of monthly tokens`
    );
  }
