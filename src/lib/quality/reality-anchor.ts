...
export async function executeAllAnchors(
  ...
  for (const summary of Object.values(intentSummary)) {
    summary.externalOutcomeScore =
      summary.anchorsTotal > 0 ? summary.anchorsPassed / summary.anchorsTotal : 1.0; // No anchors = assume pass
  ...
  const criticalFailures = results.filter(
    r => r.critical && (r.status === 'FAILED' || r.status === 'ERROR' || r.status === 'TIMEOUT')
  ).length;
...
export async function executeAllAnchorsGoverned(
  ...
  for (const anchor of anchors) {
    ...
    const result: AnchorExecutionResult = {
      ...
      status: quorum.finalVerdict
        ? 'PASSED'
        : quorum.samplesTimedOut === quorum.samplesExecuted
          ? 'TIMEOUT'
          : quorum.samplesSucceeded === 0
            ? 'ERROR'
            : 'FAILED',
      critical: anchor.critical,
      governance: {
  ...
  for (const summary of Object.values(intentSummary)) {
    ...
    summary.externalOutcomeScore =
      summary.anchorsTotal > 0 ? summary.anchorsPassed / summary.anchorsTotal : 1.0;
  ...
  console.log(
    `[RGL] Passed: ${passed}/${anchors.length} (${((passed / anchors.length) * 100).toFixed(0)}%)`
  );
...
export function logExternalValidationReport(report: ExternalValidationReport): void {
  ...
  for (const [intentId, summary] of Object.entries(report.intentSummary)) {
    console.log(
      `[ERA]   ${intentId}: ${summary.anchorsPassed}/${summary.anchorsTotal} (${(summary.externalOutcomeScore * 100).toFixed(0)}%)`
    );
  }
