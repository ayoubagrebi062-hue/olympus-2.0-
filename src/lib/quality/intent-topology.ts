...
function extractGuarantees(chain: IntentCausalChain): IntentGuarantee[] {
  ...
  // Latency guarantees
  if (
    requirement.includes('fast') ||
    requirement.includes('quick') ||
    requirement.includes('instant')
  ) {
    guarantees.push({
      type: 'latency',
      name: 'response_time',
 ...
 */
function detectConstraintConflict(a: IntentConstraint, b: IntentConstraint): string | null {
  ...
  for (const [resource, nodes] of Array.from(exclusiveByResource.entries())) {
    if (nodes.length > 1) {
      violations.push({
        ...
        suggestedResolution:
          'Designate one intent as the exclusive owner, or implement a locking mechanism',
...
function checkConstraintMonotonicity(graph: IntentInteractionGraph): InvariantViolation[] {
  ...
  const constraintsByName = new Map<
    string,
    Array<{ node: IntentNode; constraint: IntentConstraint }>
  >();
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (isStricterConstraint(lower.constraint, higher.constraint)) {
        violations.push({
          ...
          suggestedResolution:
            'Elevate the constraint to the higher-priority intent, or reduce the requirement in the lower-priority intent',
          requiresMultiIntentChange: true,
        });
      }
    }
...
function checkResourceFeasibility(graph: IntentInteractionGraph): InvariantViolation[] {
  ...
  const guaranteeRequirements = new Map<
    string,
    Array<{ node: IntentNode; guarantee: IntentGuarantee }>
  >();
  ...
  if (latencyReqs.length > 1) {
    ...
    const strictest = latencyReqs.reduce((min, curr) => {
      const currValue =
        typeof curr.guarantee.requiredValue === 'number' ? curr.guarantee.requiredValue : Infinity;
      const minValue =
        typeof min.guarantee.requiredValue === 'number' ? min.guarantee.requiredValue : Infinity;
      return currValue < minValue ? curr : min;
    });

    if (
      typeof strictest.guarantee.requiredValue === 'number' &&
      strictest.guarantee.requiredValue < 100
    ) {
 ...
 */
export function detectGlobalViolations(graph: IntentInteractionGraph): GlobalConsistencyResult {
  console.log('[ITGCL] ==========================================');
 ...
 */
function generateCrossIntentContradictions(
  violations: InvariantViolation[]
): CrossIntentContradiction[] {
 ...
 */
function generateCrossIntentClarifications(
  violations: InvariantViolation[]
): CrossIntentClarification[] {
