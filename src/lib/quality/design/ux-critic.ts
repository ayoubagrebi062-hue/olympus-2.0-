...
export function formatUXCriticResult(result: UXCriticResult): string {
  ...
  // Verdict
  const verdictIcon =
    result.verdict === 'APPROVED' ? '✓' : result.verdict === 'NEEDS_WORK' ? '⚠' : '✗';
  ...
  lines.push(
    `  Clarity:    [${scoreBar(result.scores.clarity.score)}] ${result.scores.clarity.score}/5`
  );
  lines.push(`              "${result.scores.clarity.reasoning}"`);
  lines.push(
    `  Efficiency: [${scoreBar(result.scores.efficiency.score)}] ${result.scores.efficiency.score}/5`
  );
  lines.push(`              "${result.scores.efficiency.reasoning}"`);
  lines.push(
    `  Simplicity: [${scoreBar(result.scores.simplicity.score)}] ${result.scores.simplicity.score}/5`
  );
  lines.push(`              "${result.scores.simplicity.reasoning}"`);
  lines.push(
    `  Trust:      [${scoreBar(result.scores.trust.score)}] ${result.scores.trust.score}/5`
  );
 ...
 * NOW WITH 50X UPGRADE: Includes actual benchmark code for comparison
 */
export function createUXCriticPrompt(input: UXCriticInput): string {
  const benchmarkKey = getBenchmarkForPageType(input.pageType);
  const benchmark = BENCHMARKS[benchmarkKey];

  return `
## TASK
Review this ${input.pageType} UI for quality and taste.
Compare against ${benchmark.name} standards.

${formatBenchmarkForCritic(input.pageType)}
...
1. Compare the submitted code against the Reference Implementation above
2. For EACH pattern in "Patterns to Verify":
   - If present and correct: Note as strength
   - If missing or wrong: Note as concern
3. For EACH anti-pattern in "Anti-Patterns to Reject":
   - If found: Flag as high-severity concern
4. Score 1-5 where:
   - 1 = Significantly worse than ${benchmark.name}
   - 3 = Comparable to ${benchmark.name} (minimum to pass)
   - 5 = Exceeds ${benchmark.name} quality

## YOUR REVIEW
Provide evaluation in JSON format. Be specific - reference the benchmark patterns directly.
`;
}
