...
function validateNoCustomComponents(content: string, path: string): GateIssue[] {
  ...
  const componentPattern =
    /(?:function|const)\s+([A-Z][a-zA-Z0-9]*)\s*(?:=\s*(?:\([^)]*\)|[^=])*=>|[:(])/g;
  let match;
...
function validateButtons(content: string, path: string): GateIssue[] {
  ...
  while ((adjacentMatch = adjacentPattern.exec(content)) !== null) {
    const surrounding = content.substring(
      Math.max(0, adjacentMatch.index - 200),
      adjacentMatch.index + 100
    );
...
const FORBIDDEN_GENERIC_AI_PATTERNS = [
  // Generic blue (EVERY AI USES THIS - FORBIDDEN)
  {
    ...
    message: 'Generic AI blue - use bg-violet-600 instead',
    expected: 'bg-violet-600',
  },
  {
    pattern: /bg-blue-600/g,
    message: 'Generic AI blue - use bg-violet-600 or bg-violet-700',
    expected: 'bg-violet-600/700',
  },
  {
    pattern: /text-blue-500/g,
    message: 'Generic AI blue text - use text-violet-400',
    expected: 'text-violet-400',
  },
  {
    pattern: /border-blue-500/g,
    message: 'Generic AI blue border - use border-violet-500',
    expected: 'border-violet-500',
  },
  {
    pattern: /ring-blue-500/g,
    message: 'Generic AI blue ring - use ring-violet-500',
    expected: 'ring-violet-500',
  },
  {
    pattern: /focus:ring-blue/g,
    message: 'Generic AI blue focus - use focus:ring-violet-500',
    expected: 'focus:ring-violet-500',
  },

  // Dead links (QUALITY VIOLATION)
  {
    pattern: /href=["']#["']/g,
    message: 'Dead link href="#" - use real route or button',
    expected: 'Valid href or convert to button',
  },
  {
    pattern: /href=["']["']/g,
    message: 'Empty href - use real route or button',
    expected: 'Valid href or convert to button',
  },
  ...
  // Empty handlers (BROKEN FUNCTIONALITY)
  {
    ...
    expected: 'Meaningful handler',
  },
  {
    pattern: /onClick=\{?\s*\(\)\s*=>\s*console\.log/g,
    message: 'console.log onClick - no visible result',
    expected: 'Handler with visible feedback',
  },

  // Missing transitions (STATIC = GENERIC)
  {
    pattern: /hover:[^t]*(?!transition)/g,
    message: 'hover: without transition - feels static',
    expected: 'Add transition-all duration-200',
  },
];
...
function validate50XQuality(content: string, path: string): GateIssue[] {
  ...
  while ((cardMatch = cardPattern.exec(content)) !== null) {
    ...
    const hasGlass =
      context.includes('backdrop-blur') ||
  ...
  if (content.includes('className') && content.length > 1000) {
    const hasViolet =
      content.includes('violet-') || content.includes('purple-') || content.includes('#7c3aed');
    const hasBlue =
      content.includes('blue-500') || content.includes('blue-600') || content.includes('#3B82F6');
