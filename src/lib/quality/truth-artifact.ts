...
export function generateTruthArtifact(inputs: TruthArtifactInputs): string {
  ...
  lines.push('|--------|-------|--------|');
  lines.push(
    `| W-ISS-D (Intent Satisfaction) | ${inputs.metrics.wissd}% | ${inputs.metrics.wissd >= 95 ? 'PASS' : 'FAIL'} |`
  );
  lines.push(
    `| SSI (Stability) | ${(inputs.metrics.ssi * 100).toFixed(1)}% | ${inputs.metrics.ssi >= 0.7 ? 'PASS' : 'FAIL'} |`
  );
  lines.push(
    `| UVD (User Value) | ${(inputs.metrics.uvd * 100).toFixed(1)}% | ${inputs.metrics.uvd >= 0.6 ? 'PASS' : 'FAIL'} |`
  );
  lines.push(
    `| IAS (Adequacy) | ${(inputs.metrics.ias * 100).toFixed(1)}% | ${inputs.metrics.ias >= 0.7 ? 'PASS' : 'FAIL'} |`
  );
  lines.push(
    `| Hostile Blocked | ${inputs.metrics.hostileBlocked}/${inputs.metrics.hostileTotal} | ${inputs.metrics.hostileBlocked === inputs.metrics.hostileTotal ? 'PASS' : 'FAIL'} |`
  );
  lines.push('');
  ...
  for (const gate of inputs.gates) {
    ...
    const details =
      gate.blocker || (gate.score !== undefined ? `${(gate.score * 100).toFixed(1)}%` : '-');
  ...
  lines.push(
    `- **Critical Intents:** ${inputs.intents.criticalSatisfied}/${inputs.intents.critical} satisfied`
  );
  lines.push('');
  ...
  if (inputs.fateResult) {
    ...
    if (inputs.fateResult.evolutionViolations.length > 0) {
      ...
      for (const e of inputs.fateResult.evolutionViolations) {
        const symbol =
          e.evolutionType === 'PROMOTION'
            ? ''
            : e.evolutionType === 'DEMOTION'
              ? ''
              : e.evolutionType === 'VIOLATION'
                ? ''
                : '';
  ...
  if (shipped) {
    ...
    if (inputs.fateResult && inputs.fateResult.summary.acceptedWithDebt > 0) {
      lines.push(
        '**Note:** Some intents were accepted with technical debt. These should be addressed in future builds.'
      );
    }
  } else {
    ...
    if (failedGates.some(g => g.name.includes('UVD') || g.name.includes('value'))) {
      lines.push(
        '- **Low user value**: Add observable outcomes, user feedback, or behavioral indicators'
      );
    }

    if (!inputs.constitutionalResult.passed) {
      lines.push(
        '- **Constitutional violation**: Cannot be overridden. Must fix the underlying issue.'
      );
    }
...
export function logCTELResult(result: CTELResult): void {
  ...
  console.log('[CTEL] ==========================================');
  console.log(
    `[CTEL] Constitutional Tests: ${result.constitutionalTests.passed ? 'PASSED' : 'FAILED'}`
  );
