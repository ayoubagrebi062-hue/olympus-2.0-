    ...
    while ((match = pattern.exec(content)) !== null) {
      ...
      if (!handler.hasVisibleEffect) {
        ...
        if (handler.stateChanges.length > 0) {
          ...
          handler.stateChanges.length === 0 &&
          handler.apiCalls.length === 0 &&
          handler.navigations.length === 0
        ) {
...
export function runCausalAnalysis(buildDir: string): CausalReport {
  ...
  console.log(
    `[CausalAnalyzer] State: ${summary.stateUsedInRender} rendered, ${summary.stateNotRendered} orphaned`
  );
  console.log(
    `[CausalAnalyzer] Handlers: ${summary.handlersWithEffect} with effect, ${summary.handlersWithoutEffect} without`
  );
