...
export const DEFAULT_CONSTRAINTS: GovernanceConstraints = Object.freeze({
  minStability: 0.7, // SSI must be >= 70%
  minTrust: 0.7, // External anchors must be >= 70% trusted
  maxCouplingDensity: 0.8, // Coupling density must be < 80%
  criticalMandatory: true, // Critical intents cannot be excluded
  minIntentUVD: 0.4, // Intents with UVD < 40% can be excluded
...
export function selectOptimalSubset(
  ...
  const initialMetrics = computeSubsetMetrics(
    selected,
    constraints,
    currentStability,
    currentTrust
  ...
  for (const candidate of sortedByRatio) {
    ...
    const testMetrics = computeSubsetMetrics(
      testSelected,
      constraints,
      currentStability,
      currentTrust
    ...
    const currentMetrics = computeSubsetMetrics(
      selected,
      constraints,
      currentStability,
      currentTrust
...
export function runIntentGovernance(
  ...
  console.log(
    `[IGE] Constraints: stability>=${(constraints.minStability * 100).toFixed(0)}%, trust>=${(constraints.minTrust * 100).toFixed(0)}%`
  );
  ...
  for (const chain of chains) {
    ...
    const violatesTrust =
      intentTrust < constraints.minTrust && chain.outcomeScores.external !== null;
    ...
    console.log(
      `[IGE]   ${intentId}: V=${value.totalValue.toFixed(2)} C=${cost.totalCost.toFixed(2)} V/C=${valueToCostratio.toFixed(2)}`
    );
  }
  ...
  // Final ship decision
  let allowsShip =
    stabilityConstraintMet && trustConstraintMet && feasibilityConstraintMet && criticalUVDOk;
  ...
  console.log(
    `[IGE] Constraints: stability=${stabilityConstraintMet}, trust=${trustConstraintMet}, feasibility=${feasibilityConstraintMet}`
  );
...
function generateJustification(
  ...
  if (allowsShip) {
    ...
    parts.push(
      `Selected ${selected.length} intents with total value ${(totalValue * 100).toFixed(1)}% and cost ${(totalCost * 100).toFixed(1)}%`
    );
...
export const GOVERNANCE_TEST_CASES: readonly GovernanceTestCase[] = Object.freeze([
  {
    ...
    scenario:
      'System has 3 intents: 2 high-value, 1 low-value. The low-value intent has high cost (many external anchors, low trust). Excluding it brings total cost under budget.',
    expectedOutcome: 'SHIP_AFTER_EXCLUSION',
    intents: [
      { id: 'intent-high-1', priority: 'high', uvdScore: 0.85, cost: 0.3 },
      { id: 'intent-high-2', priority: 'high', uvdScore: 0.8, cost: 0.35 },
      { id: 'intent-low-1', priority: 'low', uvdScore: 0.35, cost: 0.9 }, // Low UVD, high cost
    ],
    constraints: {
      maxTotalCost: 0.5,
      minIntentUVD: 0.4,
  ...
  },
  {
    ...
    scenario:
      'System has 4 intents with varying coupling. Including all creates high coupling density that destabilizes the system. But critical intent cannot be excluded.',
    expectedOutcome: 'BLOCKED_BY_INCLUSION',
    intents: [
      { id: 'intent-critical-1', priority: 'critical', uvdScore: 0.9, cost: 0.4 },
      { id: 'intent-high-1', priority: 'high', uvdScore: 0.75, cost: 0.5 },
      { id: 'intent-medium-1', priority: 'medium', uvdScore: 0.6, cost: 0.55 },
      { id: 'intent-medium-2', priority: 'medium', uvdScore: 0.55, cost: 0.6 },
    ],
    constraints: {
...
export function runGovernanceTests(): {
  ...
  for (const testCase of GOVERNANCE_TEST_CASES) {
    ...
    if (excluded.length > 0 && improvementAchieved) {
      ...
      const metrics = computeSubsetMetrics(
        selected,
        constraints,
        testCase.currentStability,
        testCase.currentTrust
    ...
    if (testPassed) {
      ...
      failed++;
      console.log(
        `[IGE] ✗ ${testCase.id}: Expected ${testCase.expectedOutcome}, got ${actualOutcome}`
      );
    }
 ...
 */
export function saveFateHistory(fsOutputDir: string, entry: FateHistoryEntry): string {
  const olympusDir = path.join(fsOutputDir, '.olympus');
  const historyPath = path.join(olympusDir, 'intent-fates.json');
 ...
 */
export function countQuarantineStrikes(history: FateHistoryEntry[], intentId: string): number {
  let strikes = 0;
...
export function assignIntentFate(
  ...
  else if (uvdScore >= FATE_THRESHOLDS.DEBT_UVD_MIN && uvdScore < FATE_THRESHOLDS.DEBT_UVD_MAX) {
 ...
 */
export function isValidTransition(previousFate: IntentFate, newFate: IntentFate): boolean {
...
export function checkEvolutionRules(
  ...
  for (const assignment of assignments) {
    if (assignment.previousFate !== null) {
      const evolutionType = getEvolutionType(assignment.previousFate, assignment.fate);
...
export function assignIntentFates(
  ...
  console.log('[IGE-FATE] ------------------------------------------');
  console.log(
    `[IGE-FATE] Summary: A=${summary.accepted} D=${summary.acceptedWithDebt} Q=${summary.quarantined} F=${summary.forbidden}`
  );

  // Check if any FORBIDDEN intent was supposed to be selected
  const forbiddenSelected = fates.filter(f => f.fate === IntentFate.FORBIDDEN && f.wasSelected);
...
export function logIGEResult(result: IntentGovernanceResult): void {
  ...
  console.log('[IGE] ==========================================');
  console.log(
    `[IGE] Selected: ${result.optimizationStats.selectedCount}/${result.optimizationStats.totalIntents}`
  );
