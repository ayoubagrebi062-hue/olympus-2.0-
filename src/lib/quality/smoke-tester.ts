 ...
 * Test a single route
 */
async function testRoute(route: string, port: number, timeout: number): Promise<RouteTestResult> {
...
export async function runSmokeTests(
  ...
  try {
    ...
    for (const route of routes.filter(r => r.type === 'page')) {
      ...
      for (let attempt = 1; attempt <= cfg.retries; attempt++) {
        ...
        if (attempt < cfg.retries) {
          console.log(
            `[SmokeTester] Retrying ${route.path} (attempt ${attempt + 1}/${cfg.retries})`
          );
    ...
    console.log(
      `[SmokeTester] Critical failures: ${criticalFailures}, Optional failures: ${optionalFailures}`
    );
