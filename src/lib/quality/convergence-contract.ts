...
export class ConvergenceContractManager {
  ...
  createContract(
    intent: IntentSpec,
    overrides?: Partial<ConvergenceContract>
  ): ConvergenceContract {
   ...
   */
  recordRepairAttempt(intentId: string, axis: AxisType, issDelta: number, success: boolean): void {
    const state = this.states.get(intentId);
    if (!state) return;
  ...
  checkTermination(
    ...
    if (state.repairAttempts >= contract.maxRepairs && currentISS < contract.minAcceptableISS) {
      ...
      const reason = this.generateImpossibilityReason(
        state,
        contract,
        currentAxisScores,
        blockingDependency
  ...
  private detectBlockingDependency(
    ...
    const mostFailedAxis = Object.entries(axisCounts).sort(([, a], [, b]) => b - a)[0];
    ...
    if (mostFailedAxis && mostFailedAxis[1] >= 2) {
      ...
      if (axis === 'trigger') {
        return {
          ...
          evidence: [
            `Axis ${axis} score: ${axisScores[axis] || 0}`,
            `Failed repairs: ${mostFailedAxis[1]}`,
          ],
        };
      }
...
export function logConvergenceStatus(status: GlobalConvergenceStatus): void {
  ...
  console.log(
    `[Convergence] Converged: ${status.converged} (${(status.convergenceRate * 100).toFixed(1)}%)`
  );
