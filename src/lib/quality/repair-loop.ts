...
`,
  ...
  state: (intent: IntentSpec, stateName: string, isArray: boolean) =>
    isArray
...
`,
  ...
  outcome: (stateName: string, intent: IntentSpec, isArray: boolean) =>
    isArray
      ? `
...
export function generateGradientRepairPlan(
  ...
  plan: GradientRepairPlan | null;
  axisOrderViolation: GradientRepairResult['axisOrderViolation'];
} {
  // Log state
  ...
  console.log(
    `[IGDE]   Axis to repair: ${axisToRepair} (weight: ${AXIS_DOMINANCE_WEIGHTS[axisToRepair]})`
  );
  ...
  if (!fs.existsSync(targetPath)) {
    ...
    if (!fs.existsSync(pixelPath)) {
      ...
      fs.writeFileSync(
        path.join(
          repairDir,
          `repair-${plan.selectedIntent.intentId}-${plan.repairAction.axis}.tsx`
        ),
      ...
      return {
        success: true,
        filesModified: [
          `repairs/repair-${plan.selectedIntent.intentId}-${plan.repairAction.axis}.tsx`,
        ],
      };
    }
  }
  ...
  switch (action.insertionPoint) {
    ...
    case 'handler':
      ...
      if (returnMatch && returnMatch.index !== undefined) {
        return (
          content.slice(0, returnMatch.index) +
          '\n' +
          action.codeTemplate +
...
export async function runGradientDescent(
  ...
  if (wiss.score >= WISS_THRESHOLDS.HARD_FAIL) {
    console.log(
      `[IGDE] W-ISS-D ${wiss.score}% >= ${WISS_THRESHOLDS.HARD_FAIL}% - repair not mandatory`
    );
  ...
  console.log(
    `[IGDE] Convergence: ${result.convergence.converged}/${result.convergence.totalIntents} converged`
  );
  console.log('[IGDE] ==========================================');
