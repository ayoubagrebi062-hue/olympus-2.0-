...
const INTENT_EXTRACTION_PATTERNS: Record<IntentCategory, RegExp[]> = {
  ...
  filtering: [/filter\s+(?:by\s+)?(\w+)/gi, /sort\s+(?:by\s+)?(\w+)/gi, /(\w+)\s+filter/gi],
  search: [/search\s+(?:for\s+)?(\w+)/gi, /find\s+(\w+)/gi, /(\w+)\s+search/gi],
  form_submission: [/submit\s+(\w+)\s+form/gi, /(\w+)\s+form/gi, /contact\s+form/gi],
  authentication: [
    /log\s*in/gi,
    /sign\s*in/gi,
    /log\s*out/gi,
  ...
  feedback: [/show\s+(?:success\s+)?message/gi, /notification/gi, /toast/gi, /alert/gi],
  loading: [/loading\s+(?:state|indicator|spinner)/gi, /show\s+loading/gi],
  error_handling: [
  ...
  for (const artifact of oracleArtifacts) {
    ...
    for (const pattern of goalPatterns) {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        intents.push(
          createIntentFromMatch(`intent_${++intentId}`, requirement, category, match, 'oracle')
        );
      }
    }
  }
  ...
  switch (category) {
    case 'navigation':
      return {
        type: 'click',
        target: `${entity}Link`,
        targetPattern: new RegExp(`href.*${entity}`, 'i'),
      };
    case 'data_mutation':
      return {
        type: 'click',
        target: `Add${entity}Button`,
        targetPattern: new RegExp(`add.*${entity}|create.*${entity}`, 'i'),
      };
    case 'filtering':
      return {
        type: 'click',
        target: `${entity}Filter`,
        targetPattern: new RegExp(`filter.*${entity}|${entity}.*filter`, 'i'),
      };
    case 'search':
      return {
        type: 'change',
        target: `${entity}Search`,
        targetPattern: new RegExp(`search.*${entity}`, 'i'),
      };
    case 'form_submission':
      return {
        type: 'submit',
        target: `${entity}Form`,
        targetPattern: new RegExp(`form.*${entity}|${entity}.*form`, 'i'),
      };
    case 'state_toggle':
      return {
        type: 'click',
        target: `${entity}Toggle`,
        targetPattern: new RegExp(`toggle.*${entity}`, 'i'),
      };
    default:
      return undefined;
  }
...
function buildExpectedState(category: IntentCategory, entity: string): StateSpec | undefined {
  ...
  switch (category) {
    case 'data_display':
      return {
        stateName: `${entityLower}s`,
        statePattern: new RegExp(`\\[${entityLower}s?,\\s*set`, 'i'),
      };
    case 'data_mutation':
      return {
        stateName: `${entityLower}s`,
        statePattern: new RegExp(`\\[${entityLower}s?,\\s*set`, 'i'),
        expectedTransition: 'list grows',
      };
    case 'filtering':
      return {
        stateName: 'filter',
        statePattern: /\[filter.*,\s*set/i,
        derivedState: [`filtered${entity}s`],
      };
    case 'search':
    ...
    case 'state_toggle':
      return {
        stateName: `is${entity}`,
        statePattern: new RegExp(`\\[is${entity},\\s*set`, 'i'),
        expectedTransition: 'boolean toggle',
      };
    case 'loading':
...
export function validateIntentChain(
  ...
  if (stateExpected) {
    if (foundState) {
      ...
      stateScore =
        foundState.usedInHandler && foundState.usedInRender
          ? 1.0
          : foundState.usedInHandler || foundState.usedInRender
            ? 0.5
            : 0.25;
  ...
  for (const file of codeFiles) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (useStateMatch) {
        ...
        if (
          foundStateName.toLowerCase().includes(stateName.toLowerCase()) ||
...
function extractHandlerCode(handlerRef: string, content: string): string {
  ...
  const funcPattern = new RegExp(
    `(?:const|function)\\s+${handlerRef}\\s*=?[^{]*\\{([\\s\\S]*?)\\n\\s*\\}`,
    'm'
  );
 ...
 */
export function runICGAnalysis(buildDir: string, agentOutputs: Map<string, any>): ICGReport {
  ...
  console.log(
    `[ICG] Extracted ${allIntents.length} intents (${intentsFromScope.length} from scope, ${intentsFromOracle.length} from oracle)`
  );
  ...
  const wissScore = totalWeight > 0 ? Math.round((achievedWeight / totalWeight) * 100) : 100;
  ...
  const criticalBlocker =
    criticalIntentsTotal > 0 && criticalIntentsSatisfied < criticalIntentsTotal;
  ...
  const axisAverages = {
    trigger:
      chains.length > 0
        ? chains.reduce((sum, c) => sum + c.axisScores.trigger, 0) / chains.length
        : 1,
    state:
      chains.length > 0
        ? chains.reduce((sum, c) => sum + c.axisScores.state, 0) / chains.length
        : 1,
    effect:
      chains.length > 0
        ? chains.reduce((sum, c) => sum + c.axisScores.effect, 0) / chains.length
        : 1,
    outcome:
      chains.length > 0
        ? chains.reduce((sum, c) => sum + c.axisScores.outcome, 0) / chains.length
        : 1,
  };
  ...
  const intentSatisfactionScore =
    allIntents.length > 0 ? Math.round((satisfiedIntents / allIntents.length) * 100) : 100;
  ...
  if (criticalBlocker) {
    console.log(
      `[ICG]   ⛔ CRITICAL BLOCKER: ${blockers.length} critical intent(s) not fully satisfied`
    );
    for (const blocker of blockers) {
      console.log(
        `[ICG]      - "${blocker.requirement.slice(0, 50)}..." (${Math.round(blocker.score * 100)}%)`
      );
    ...
    for (const match of mapMatches) {
      ...
      if (!['children', 'items', 'options'].includes(listName.toLowerCase())) {
        intents.push({
          ...
          expectedState: {
            stateName: listName,
            statePattern: new RegExp(`\\[${listName},\\s*set`),
          },
          expectedOutcome: {
            type: 'list_change',
...
export function applyExternalValidation(
  ...
  return chains.map(chain => {
    ...
    if (
      !validation ||
      validation.externalScore === null ||
      validation.externalScore === undefined
    ) {
