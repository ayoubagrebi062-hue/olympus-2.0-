...
export class RealityPolicyRegistry {
  ...
  setPolicy(
    ...
    const policy: RealityPolicy = {
      ...
      successThreshold:
        config.successThreshold ?? existing?.successThreshold ?? DEFAULT_POLICY.successThreshold,
      ...
      minTrustScore:
        config.minTrustScore ?? existing?.minTrustScore ?? DEFAULT_POLICY.minTrustScore,
      varianceThreshold:
        config.varianceThreshold ?? existing?.varianceThreshold ?? DEFAULT_POLICY.varianceThreshold,
    ...
    console.log(
      `[RGL] Policy set for anchor ${anchorId}: mode=${policy.mode}, samples=${policy.samples}`
    );
    return policy;
  }
...
export function calculateTrustScore(
  ...
  const variance =
    successfulSamples.length > 0 ? (uniqueHashes.size - 1) / successfulSamples.length : 0;
...
export function aggregateQuorum(
  ...
  const successfulSamples = samples.filter(s => s.success && !s.timedOut);
  const uniqueHashes = new Set(successfulSamples.map(s => s.payloadHash));
  const payloadVariance =
    successfulSamples.length > 1 ? (uniqueHashes.size - 1) / (successfulSamples.length - 1) : 0;
...
export function generateGovernanceReport(
  ...
  const averageTrustScore =
    trustScores.length > 0 ? trustScores.reduce((a, b) => a + b, 0) / trustScores.length : 1.0;
...
export function logQuorumResult(quorum: QuorumResult): void {
  ...
  console.log(
    `[RGL]   Samples: ${quorum.samplesSucceeded}/${quorum.samplesExecuted} passed (${quorum.samplesTimedOut} timeouts)`
  );
  console.log(
    `[RGL]   Success Rate: ${(quorum.successRate * 100).toFixed(0)}% (threshold: ${(quorum.policy.successThreshold * 100).toFixed(0)}%)`
  );
  console.log(
    `[RGL]   Trust Score: ${(quorum.trustScore * 100).toFixed(0)}% (min: ${(quorum.policy.minTrustScore * 100).toFixed(0)}%)`
  );
  console.log(
    `[RGL]   Deterministic: ${quorum.isDeterministic ? 'Yes' : 'No'} (variance: ${quorum.payloadVariance.toFixed(3)})`
  );
...
export function logGovernanceReport(report: GovernanceReport): void {
  ...
  console.log(
    `[RGL] Trust: avg=${(report.averageTrustScore * 100).toFixed(0)}%, min=${(report.lowestTrustScore * 100).toFixed(0)}%, max=${(report.highestTrustScore * 100).toFixed(0)}%`
  );
