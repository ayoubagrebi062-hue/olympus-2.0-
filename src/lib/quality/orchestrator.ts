...
export class QualityOrchestrator {
  ...
  async runAllGates(
    ...
    for (const [type, gate] of this.gates) {
      ...
      try {
        ...
        console.log(
          `[QualityOrchestrator] ${type}: ${result.passed ? '✅ PASSED' : '❌ FAILED'} (${result.issues.length} issues)`
        );
      } catch (error) {
    ...
    if (belowThreshold && this.config.stopOnFailure) {
      console.log(
        `[QualityOrchestrator] QUALITY THRESHOLD FAILED: score ${overallScore} < minScore ${this.config.minScore}`
      );
    ...
    console.log(
      `[QualityOrchestrator] Quality check complete: ${overallStatus} (score: ${overallScore}, threshold: ${this.config.minScore}, stopOnFailure: ${this.config.stopOnFailure})`
    );
  ...
  private generateRecommendations(results: GateResult[]): string[] {
    ...
    if (issuesByRule.has('missing-page')) {
      recommendations.push(
        'Create missing page files for all linked routes, or update links to existing pages'
      );
    }

    if (issuesByRule.has('empty-data-array') || issuesByRule.has('empty-datasets')) {
      recommendations.push(
        'Add sample/mock data for charts and tables - never render empty arrays'
      );
    }

    if (issuesByRule.has('adjacent-buttons-no-gap')) {
      recommendations.push(
        'Wrap adjacent buttons in flex container with gap-2 or gap-3 for proper spacing'
      );
    }
  ...
  formatReport(report: QualityReport): string {
    ...
    for (const gate of report.gates) {
      ...
      lines.push(
        `║ ${icon} ${gate.gate.padEnd(15)} ${gate.status.padEnd(10)} ${issues.padEnd(16)} ║`
      );
    }
    ...
    // Summary
    lines.push(
      `║ Errors: ${String(report.summary.totalErrors).padEnd(5)} Warnings: ${String(report.summary.totalWarnings).padEnd(20)} ║`
    );
 ...
 * Check a single file
 */
export async function checkFile(file: FileToCheck): Promise<GateIssue[]> {
