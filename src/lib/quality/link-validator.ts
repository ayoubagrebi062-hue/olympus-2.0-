...
export async function validateLinks(files: FileToCheck[]): Promise<{
  ...
  return {
    ...
    summary: {
      placeholderLinks: result.issues.filter(
        i => i.rule === 'placeholder-href' || i.rule === 'empty-href'
      ).length,
      ...
      spacingIssues: result.issues.filter(
        i => i.rule?.includes('gap') || i.rule?.includes('spacing')
      ).length,
    },
  };
}
