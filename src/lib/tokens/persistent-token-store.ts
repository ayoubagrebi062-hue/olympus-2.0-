...
class RedisTokenStore implements TokenStore {
  ...
  async get(id: string): Promise<TokenData | null> {
    try {
      ...
      logger.warn('Redis get failed, data may be stale', {
...
export class PersistentTokenService {
  ...
  constructor(redis?: { get: Function; set: Function; del: Function; exists: Function }) {
    ...
    if (redis) {
      ...
      logger.warn(
        'PersistentTokenService: No Redis, using memory store (not recommended for production)'
      );
    }
  }
  ...
  private calculateQuota(token: TokenData): TokenQuota {
    ...
    return {
      ...
      apiCalls: {
        ...
        remaining:
          limits.maxApiCallsPerHour === -1 ? Infinity : limits.maxApiCallsPerHour - usage.apiCalls,
      },
    };
  }
