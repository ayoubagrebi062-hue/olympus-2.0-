...
const NI_06: NegativeInvariant = {
  ...
  consequence: 'SYSTEM_INVALID',
  check: s =>
    ['CONSTITUTION', 'INVARIANTS', 'PROOFS', 'OPERATIONAL_RECORD', 'SELF'].includes(
      s.legitimacySource
    ),
};
...
const NI_18: NegativeInvariant = {
  ...
  consequence: 'TERMINATION_REQUIRED',
  check: (_, a) =>
    a.providedStatement !== null &&
    a.verifiedTruth !== null &&
    a.providedStatement !== a.verifiedTruth,
};
...
const NI_20: NegativeInvariant = {
  ...
  consequence: 'SYSTEM_INVALID',
  check: s =>
    ['SELF', 'INHERENT', 'NATURAL', 'NECESSARY', 'INTERNAL_DOCUMENT'].includes(
      s.rightToExistSource
    ),
};
...
function createSystemState(partial: Partial<SystemState>): SystemState {
  return {
    ...
    authorityGrant: partial.authorityGrant ?? {
      grantedBy: null,
      grantedAt: null,
      documentRef: null,
      isExternal: false,
...
function createContext(
  timestamp: number,
  examinationInitiated: boolean,
  externalAuthorityPresent: boolean
): ExecutionContext {
