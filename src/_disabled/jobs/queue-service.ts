...
export class QueueService {
   ...
   */
  async enqueue<T = Record<string, unknown>>(job: JobInput<T>): Promise<EnqueueResult> {
   ...
   */
  async enqueueMany<T = Record<string, unknown>>(jobs: JobInput<T>[]): Promise<EnqueueResult[]> {
  ...
  async deleteSchedule(name: string): Promise<void> {
    ...
    await this.supabase.from('scheduled_jobs').delete().eq('name', name);
  }
  ...
  async listSchedules(): Promise<ScheduledJob[]> {
    const { data, error } = await this.supabase.from('scheduled_jobs').select('*').order('name');
   ...
   */
  async resolveDeadLetter(dlqId: string, notes: string, resolvedBy?: string): Promise<void> {
    await this.supabase
      .from('dead_letter_queue')
      .update({
        resolved: true,
