...
class HandlerRegistry {
   ...
   */
  registerFunction<T, R>(type: JobType, handler: JobHandler<T, R>, config?: HandlerConfig): void {
...
export class JobProcessor {
  ...
  async process(request: {
    ...
    try {
      ...
      const err = error as Error;
      context.log('error', `Job processing infrastructure error: ${err.message}`, {
        stack: err.stack,
      });
...
export function createHandler<T, R>(
  ...
  handlerRegistry.registerFunction(
    type,
